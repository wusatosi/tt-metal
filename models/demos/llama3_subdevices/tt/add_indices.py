...
class TTAddIndices(LightweightModule):
    ...
    def forward(self, x: ttnn.Tensor):
        x = ttnn.typecast(x, dtype=ttnn.uint32, sub_core_grids=self.sub_core_grids)
        x = ttnn.typecast(x, dtype=ttnn.int32, sub_core_grids=self.sub_core_grids)

        topk_indices_gathered_sharded = ttnn.to_memory_config(x, self.args.model_config["DECODE_SAMPLING_INPUT_MEMCFG"])
