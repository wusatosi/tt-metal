...
class TTSampling(LightweightModule):
    ...
    def forward(self, x: ttnn.Tensor):
        ...
        topk_indices_gathered_sharded = ttnn.to_memory_config(
            topk_indices_gathered, self.args.model_config["DECODE_SAMPLING_INPUT_MEMCFG"], dtype=ttnn.int32
        )
        ttnn.deallocate(topk_indices_gathered)

        # Add device offsets for global indices
        topk_global_indices = ttnn.add(
            self.tt_indices_device_offsets, topk_indices_gathered_sharded, dtype=ttnn.int32
        )
        ttnn.deallocate(topk_indices_gathered)
