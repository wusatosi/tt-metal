...
class TtModelArgs:
    ...
    def __init__(
        ...
        if device is not None:  # Avoid issue with test_llama_torch.py not having a device
            ...
            # lambda seq_len: self.matmul_config(
            #     m=min(seq_len, 1024),
            #     k=self.dim // self.cluster_shape[0],
            #     n=self.hidden_dim // self.cluster_shape[1],
            #     grid_size=(7, 10) if self.is_galaxy else ((8, 8) if seq_len >= 1024 else (8, 4)),
            # )
            self.model_config[
                ...
                per_core_N=math.ceil(self.dim / self.cluster_shape[0] / 32 / 8),  # N / TILE_WIDTH / grid width
                transpose_mcast=False,
                fused_activation=None,
                fuse_batch=seq_len <= 2048,
            )
            ...
            self.model_config["XQKV_PREFILL_PROGCFG"] = lambda seq_len: ttnn.MatmulMultiCoreReuseMultiCastProgramConfig(
                ...
                per_core_N=math.ceil(self.qkv_size / self.cluster_shape[1] / 32 / 10),  # N / TILE_WIDTH / grid width
                transpose_mcast=False,
                fused_activation=None,
                fuse_batch=seq_len <= 2048,
            )
