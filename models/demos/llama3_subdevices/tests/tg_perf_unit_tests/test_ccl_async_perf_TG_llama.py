...
@pytest.mark.parametrize(
    "rs_type, warmup_iters, perf_target_us",
    [
        ("mlp", 10, 25),
    ],
)
@pytest.mark.models_device_performance_bare_metal
def test_reduce_scatter_mlp_perf(
    rs_type,
    warmup_iters,
    perf_target_us,
):
    profiler = BenchmarkProfiler()
    benchmark_data = BenchmarkData()
    step_name = f"rs_mlp_test"

    subdir = "llama_ccl_perf"
    command = f"pytest tests/ttnn/unit_tests/operations/ccl/test_ccl_async_TG_llama.py::test_reduce_scatter_tg_llama -k {rs_type}"
    cols = ["DEVICE KERNEL"]
