...
class TtLlamaDecoder_optimized:
    ...
    def decode_forward(
        ...
        attn_resid_replicated = ttnn.empty(shape=[1,1,32,8192], layout=ttnn.TILE_LAYOUT,device=self.mesh_device, memory_config = self.model_config["HIDDEN_WIDTH_16_CORES_MEMCFG"])

        print (f"Tensor properties {attn_resid_replicated.dtype} {attn_resid_replicated.shape} {attn_resid_replicated.layout} {dir(attn_resid_replicated)}")
