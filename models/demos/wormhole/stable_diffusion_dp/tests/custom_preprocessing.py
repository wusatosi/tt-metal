...
def preprocess_conv_parameter(parameter, *, dtype, mesh_mapper=None):
    parameter = ttnn.from_torch(parameter, dtype=dtype, mesh_mapper=mesh_mapper)
    return parameter
...
def custom_preprocessor(model, name, mesh_mapper=None):
    ...
    if isinstance(model, nn.Conv2d):
        ...
        parameters["weight"] = preprocess_conv_parameter(model.weight, dtype=ttnn.float32, mesh_mapper=mesh_mapper)
        if model.bias is not None:
            bias = model.bias
            while len(bias.shape) < 4:
                bias = bias.unsqueeze(0)
        parameters["bias"] = preprocess_conv_parameter(bias, dtype=ttnn.float32, mesh_mapper=mesh_mapper)
