...
def _save_image_and_latents(latents, iter, vae, mesh_composer, pre_fix="", pre_fix2=""):
    ...
    # Save images and latents for each prompt
    for idx, pil_image in enumerate(pil_images):
        ttnn_output_image_path = f"{pre_fix}{pre_fix2}image_prompt_{idx + 1}_iter_{iter}.png"
        ttnn_output_latents_path = f"{pre_fix}{pre_fix2}latents_prompt_{idx + 1}_iter_{iter}.pt"
        pil_image.save(ttnn_output_image_path)
        torch.save(latents[idx], ttnn_output_latents_path)
        print(f"Saved image to {ttnn_output_image_path} and latents to {ttnn_output_latents_path}")
...
def run_interactive_demo_inference(device, num_prompts, num_inference_steps, image_size=(256, 256)):
    ...
    _tlist = []
    for t in ttnn_scheduler.timesteps:
        _t = constant_prop_time_embeddings(t, rand_latents, unet.time_proj)
        _t = _t.unsqueeze(0).unsqueeze(0)
        _t = _t.permute(2, 0, 1, 3)  # pre-permute temb
        _t = ttnn.from_torch(
            _t, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device, mesh_mapper=weights_mesh_mapper
    ...
    while 1:
        ...
        for index in tqdm(range(len(time_step))):
            ...
            noise_pred = tt_guide(ttnn_output, guidance_scale)
            ttnn_latents = ttnn_scheduler.step(noise_pred, t, ttnn_latents).prev_sample
            _save_image_and_latents(ttnn_latents, iter, vae, output_mesh_composer, pre_fix=f"", pre_fix2="")
        ...
        for idx, pil_image in enumerate(pil_images):
            ttnn_output_path = f"input_data_{idx + 1}_{height}x{width}.png"
            pil_image.save(ttnn_output_path)
