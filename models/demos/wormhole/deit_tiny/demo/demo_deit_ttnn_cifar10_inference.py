...
def test_deit(device):
    ...
    for iter in range(iterations):
        ...
        for i in range(batch_size):
            predictions.append(cifar_label_dict[prediction[i].item()])
            logger.info(
                f"Iter: {iter} Sample: {i} - Expected Label: {cifar_label_dict[labels[i]]} -- Predicted Label: {predictions[-1]}"
            )
            print(predictions)
            if cifar_label_dict[labels[i]] == predictions[-1]:
                correct += 1

        # del tt_output, tt_inputs, inputs, labels, predictions

    accuracy = correct / (batch_size * iterations)
    print("Cifar10 Inference ACCURACY=", accuracy)