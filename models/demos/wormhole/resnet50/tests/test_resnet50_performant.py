...
@run_for_wormhole_b0()
@pytest.mark.parametrize(
    "device_params", [{"l1_small_size": RESNET50_L1_SMALL_SIZE, "trace_region_size": 845824}], indirect=True
)
@pytest.mark.parametrize(
    "batch_size, act_dtype, weight_dtype, math_fidelity",
    ((16, ttnn.bfloat8_b, ttnn.bfloat8_b, ttnn.MathFidelity.LoFi),),
...
@pytest.mark.parametrize(
    "device_params",
    [{"l1_small_size": RESNET50_L1_SMALL_SIZE, "trace_region_size": 845824, "num_command_queues": 2}],
    indirect=True,
)
@pytest.mark.parametrize(
    "batch_size, act_dtype, weight_dtype, math_fidelity",
