...
def update_model_config(config, batch_size):
    core_grid = ttnn.CoreGrid(y=batch_size, x=3)
    seqL_t = int(224 / 32)  # 7
    dim_t = int(192 / 32)  # 6
    ...
    program_configs = {
        ...
        "query_by_key_matmul_program_config": ttnn.MatmulMultiCoreReuseProgramConfig(
            compute_with_storage_grid_size=(core_grid.x, core_grid.y),
            in0_block_w=int(dim_t__x),  # 2,
            out_subblock_h=1,
            out_subblock_w=seqL_t,  # 7,
            per_core_M=int(seqL_t),  # 14,
            per_core_N=int(head_seqL_t),  # 7,
        ),
        ...
        "softmax_program_config": ttnn.SoftmaxShardedMultiCoreProgramConfig(
            compute_with_storage_grid_size=(core_grid.x, core_grid.y),
            subblock_w=int(head_seqL_t),  # 7,
            block_h=int(seqL_t),  # 14,
            block_w=int(head_seqL_t),  # 7,
        ),
    }
...
def deit_patch_embeddings(config, pixel_values, *, parameters, unittest_check=False):
    ...
    patch_embedding_output = ttnn.reshape(patch_embedding_output, (batch_size, patch_count_all, embedding_dim))
...
def deit(
    ...
    embeddings_output = deit_embeddings(
        config, pixel_values, cls_token, distillation_token, position_embeddings, parameters=parameters
    )
...
def custom_preprocessor(torch_model, name):
    ...
    if isinstance(torch_model, transformers.models.vit.modeling_vit.ViTEmbeddings):
        ...
        three_times_hidden_size, c, patch_h, patch_w = weight.shape
        pad_value = 4 - c
        ...
        preprocessed_weight = torch.reshape(preprocessed_weight, ((4 * patch_h * patch_w), three_times_hidden_size))
