...
def test_llama_model_inference(
    ...
    if batch_size == 1:
        ...
        pcc = 0.7  # TODO Miguel: Investigate lower PCC with batch_size > 1

    instruct = False if weights == "instruct" else False
    dummy_weights = True if weights == "random" else False
    model_args = TtModelArgs(
        mesh_device, instruct=instruct, dummy_weights=dummy_weights, max_seq_len=max_seq_len, max_batch_size=batch_size
    )
