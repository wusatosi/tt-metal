...
def ResnetBlock2D(
    ...
    # if (C == 1920 and H == 64) or ( C == 960 and H == 128):
    # use_torch_silu = True
    #    use_torch_conv = True
    # if:
    # use_torch_conv = True
    use_torch_conv = False
    if use_torch_gn:
        print("Groupnorm1 :", hidden_states.shape)
    ...
    if non_linearity == "silu":
        if use_torch_silu:
            print("Silu1 :", hidden_states.shape)
    ...
    # if use_torch_conv:
    if (hidden_states.shape[1] == 1920 and hidden_states.shape[2] == 64) or (
        hidden_states.shape[1] == 960 and hidden_states.shape[2] == 128
    ):
        ...
        print("Conv1 :", hidden_states.shape)
    ...
    if use_torch_gn:
        print("groupnorm2 :", hidden_states.shape)
        hidden_states = ttnn.to_torch(hidden_states).to(torch.float)
    ...
    if non_linearity == "silu":
        if use_torch_silu:
            print("Silu2 :", hidden_states.shape)
            torch_silu = torch.nn.SiLU()
            hidden_states = ttnn.to_torch(hidden_states)
            hidden_states = torch_silu(hidden_states)
            hidden_states = ttnn.from_torch(hidden_states, device=device, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT)
    ...
    if (hidden_states.shape[1] == 1920 and hidden_states.shape[2] == 64) or (
        ...
        hidden_states = ttnn.to_torch(hidden_states).to(torch.float)
        hidden_states = torch.permute(hidden_states, (0, 3, 1, 2))
        print("Torch Conv2", hidden_states.shape)
        hidden_states = conv(hidden_states)
        hidden_states = ttnn.from_torch(hidden_states, device=device, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT)
    ...
    if parameters.conv2.conv_shortcut:
        if use_torch_conv:
            # if (input_tensor.shape[1] == 1920 and input_tensor.shape[2] == 64) or (input_tensor.shape[1] == 960 and input_tensor.shape[2] == 128):
            print("Torch conv shortcut :", input_tensor.shape)
