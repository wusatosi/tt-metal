...
def get_aug_embed(emb, text_embeds, time_ids, parameters, device, breaks=False):
    time_ids = ttnn.to_torch(time_ids)

    time_ids = ttnn.from_torch(time_ids.flatten(), dtype=ttnn.bfloat16, device=device)
    time_embeds = Timesteps(time_ids, 256, True, 0, device=device, breaks=breaks)
    # return time_embeds
...
def stable_diffusion_xl_turbo(
    ...
    aug_emb = get_aug_embed(
        emb=emb, text_embeds=text_embeds, time_ids=time_ids, parameters=parameters, device=device, breaks=True
    )
    # return aug_emb
    ...
    input_mask_tensor = parameters.conv_norm_out.input_mask_tensor
    ...
    # )
    gamma_t, beta_t = parameters.conv_norm_out.tt_weight, parameters.conv_norm_out.tt_bias
