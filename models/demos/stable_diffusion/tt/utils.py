...
def get_timestep_embedding(
    ...
    print("Shape of timestpes :", timesteps.shape, " ", emb.shape)
    timesteps = ttnn.to_torch(timesteps)
    emb = ttnn.to_torch(emb)
    emb = timesteps * emb
    emb = ttnn.from_torch(emb, device=device, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT)
    # if breaks:

    #    print("My return :", emb.shape)
...
def Timesteps(
    timesteps, num_channels: int, flip_sin_to_cos: bool, freq_shift: float, scale: int = 1, device=None, breaks=False
) -> Tuple[int, int]:
    t_emb = get_timestep_embedding(
        timesteps, num_channels, flip_sin_to_cos, freq_shift, scale, device=device, breaks=breaks
    )
    return t_emb
