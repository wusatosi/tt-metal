...
from models.demos.stable_diffusion_xl_turbo.tt.tt_upblock_2d import up_block_2d
...
def test_up_block_2d_1024x1024(
    ...
    assert_with_pcc(torch_output, resnet_block, 0.88)
...
def test_up_block_2d_512x512(
    ...
    input = torch.randn(hidden_states_shape)
    temb = torch.randn(temb_shape)
    input1 = torch.randn(1, 320, 64, 64)
    input_tuple = (input1, input1, input1)
    torch_output = resnet(input, input_tuple, temb)
    ...
    resnet_block = ttnn.to_torch(resnet_block)
    assert_with_pcc(torch_output, resnet_block, 0.91)
