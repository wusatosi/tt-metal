...
def test_downsample_1024x1024(device, N, C, H, W, index, reset_seeds):
    pipe = AutoPipelineForText2Image.from_pretrained(
        "stabilityai/sdxl-turbo", torch_dtype=torch.float32, variant="fp16"
    )
    model = pipe.unet
    model.eval()
    model = model.down_blocks[index].downsamplers[0]
    ...
    output = tt_downsample_2d.downsample(ttnn_hidden_state, parameters, device)
...
def test_downsample_512x512(device, N, C, H, W, index, reset_seeds):
    pipe = AutoPipelineForText2Image.from_pretrained(
        "stabilityai/sdxl-turbo", torch_dtype=torch.float32, variant="fp16"
    )
    model = pipe.unet
