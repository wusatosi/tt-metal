...
import ttnn
import torch
from models.experimental.centernet.tt.common import Conv, batch_norm
...
class TtCTResNetNeck:
    ...
    def _make_deconv_layer(self) -> nn.Sequential:
        ...
        for i in range(6):
            ...
            bn = batch_norm(self.device, f"neck.deconv_layers.{i}.bn", self.parameters)
    ...
    def forward(self, x) -> Tuple[torch.Tensor]:
        ...
        for i, module in enumerate(self.deconv_layers):
            if i == 0 or i == 6 or i == 12:
                temp = outs
                outs = module(outs)
                outs = ttnn.to_torch(outs)
                o1, o2, mask = torch.chunk(outs, 3, dim=1)
                o1 = ttnn.from_torch(o1, device=self.device, dtype=ttnn.bfloat16)
                o2 = ttnn.from_torch(o2, device=self.device, dtype=ttnn.bfloat16)
                mask = ttnn.from_torch(mask, device=self.device, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT)
                offset = ttnn.concat([o1, o2], dim=1)

                mask = ttnn.sigmoid_accurate(mask)
                if i == 0:
                    weight = self.weight1
