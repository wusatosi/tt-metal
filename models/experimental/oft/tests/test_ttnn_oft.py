...
def test_basic_block(device, path):
    ...
    # print(f"param : {parameters.downsample[0]}")
...
def test_oft_module(device, cell_size, grid_height, reset_seeds):
    ...
    y_corners = ttnn.from_torch(y_corners, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device)

    ttnn_input = ttnn.from_torch(features, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device)
    ttnn_calib = ttnn.from_torch(calib, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device)
    ttnn_grid = ttnn.from_torch(grid, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device)
...
def test_oftnet(device, grid_height, cell_size):
    ...
    mean = ttnn.from_torch(mean, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device)
    std = ttnn.from_torch(std, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device)
    y_corners = torch.arange(0, grid_height, cell_size) - grid_height / 2.0
    y_corners = F.pad(y_corners.view(-1, 1, 1, 1), [1, 1])
    y_corners = ttnn.from_torch(y_corners, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device)
