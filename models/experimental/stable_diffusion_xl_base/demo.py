...
def run_demo_inference(
    ...
    # 0. Set up height and width for unet
    ...
    # - pooled prompt_embeds = none
    # - pooled negative_prompt_embeds = none
    # - guidance_rescale = 0
    # - clip skip = none
    # - cross attention kwargs = none
    # - denoising end = none
    ...
    prompt_embeds = prompt_embeds.to(cpu_device)
    ...
    if has_latents_mean and has_latents_std:
        latents_mean = torch.tensor(pipeline.vae.config.latents_mean).view(1, 4, 1, 1).to(latents.device, latents.dtype)
        latents_std = torch.tensor(pipeline.vae.config.latents_std).view(1, 4, 1, 1).to(latents.device, latents.dtype)
...
# For classifier free guidance do:
...
@pytest.mark.parametrize(
    "vae_scale_factor",
    ((8),),  # Image size is going to be default_sample_size * vae_scale_factor
)
@pytest.mark.parametrize(
