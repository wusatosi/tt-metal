...
class ttnn_PETRHead:
    ...
    def position_embeding(self, img_feats, img_metas, masks=None, device=None):
        ...
        D = coords_d.shape[0]
        coords_h = ttnn.reshape(coords_h, (-1, 1, 1))
        coords_w = ttnn.reshape(coords_w, (1, -1, 1))
        coords_d = ttnn.reshape(coords_d, (1, 1, -1))

        coords_h = ttnn.repeat(coords_h, ttnn.Shape([1, coords_w.shape[1], coords_d.shape[2]]))
        coords_w = ttnn.repeat(coords_w, ttnn.Shape([coords_h.shape[0], 1, coords_d.shape[2]]))
        coords_d = ttnn.repeat(coords_d, ttnn.Shape([coords_h.shape[0], coords_w.shape[1], 1]))

        coords_h = ttnn.reshape(coords_h, (1, coords_h.shape[0], coords_h.shape[1], coords_h.shape[2]))
        coords_w = ttnn.reshape(coords_w, (1, coords_w.shape[0], coords_w.shape[1], coords_w.shape[2]))
        coords_d = ttnn.reshape(coords_d, (1, coords_d.shape[0], coords_d.shape[1], coords_d.shape[2]))
        ones = ttnn.ones_like(coords_d, dtype=ttnn.bfloat16)
        coords = ttnn.concat((coords_h, coords_w, coords_d, ones), dim=0)
        coords = ttnn.permute(coords, (2, 1, 3, 0))
        coords = ttnn.to_layout(coords, ttnn.ROW_MAJOR_LAYOUT)
        # coords[..., :2] = coords[..., :2] * ttnn.maximum(coords[..., 2:3], (ttnn.full_like(coords[..., 2:3], 1))* eps)
        ...
        for img_meta in img_metas:
            ...
            for i in range(len(img_meta["lidar2img"])):
                # img_meta_tensor = ttnn.to_torch(img_meta_tensor)
                img2lidar.append(np.linalg.inv(img_meta["lidar2img"][i]))
            img2lidars.append(np.asarray(img2lidar))
        img2lidars = np.asarray(img2lidars)
        img2lidars = torch_coords.new_tensor(img2lidars)

        print("ttnn: ", torch_coords.shape)
    ...
    def __call__(self, mlvl_feats, img_metas, device=None):
        ...
        masks = F.interpolate(masks, size=ttnn.to_torch(x).shape[-2:]).to(torch.bool)
        ...
        if self.with_position:
            ...
            if self.with_multiview:
                ...
                sin_embed = ttnn.from_torch(sin_embed, device=device)
