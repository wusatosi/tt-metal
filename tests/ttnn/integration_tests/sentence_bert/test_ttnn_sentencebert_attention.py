...
def test_ttnn_sentence_bert_attention(device, inputs):
    ...
    ttnn_hidden_states = ttnn.from_torch(
        hidden_states.unsqueeze(dim=1), layout=ttnn.TILE_LAYOUT, dtype=ttnn.bfloat8_b, device=device
    )
    sharded_input = ttnn.to_memory_config(
        ttnn_hidden_states,
        memory_config=ttnn.create_sharded_memory_config(
            ttnn_hidden_states.shape,
            core_grid=ttnn.CoreGrid(y=8, x=6),  # device.core_grid,
    ...
    # l1 = torch.load("/home/ubuntu/venkatesh_latest/tt-metal/models/experimental/sentence_bert/ttnn/dumps/q_pipeline").squeeze(dim=1)
    # l2 = torch.load(
