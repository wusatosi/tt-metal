...
def test_petr_head(device, reset_seeds):
    mlvl_feats = torch.load(
        "/home/ubuntu/punith/tt-metal/models/experimental/functional_petr/reference/golden_mlvl_feats_petr_head.pt"
    )
    img_metas = torch.load(
        "/home/ubuntu/punith/tt-metal/models/experimental/functional_petr/reference/golden_img_metas_petr_head.pt"
    )
    ...
    for i in range(len(mlvl_feats)):
        mlvl_feats[i] = ttnn.from_torch(mlvl_feats[i], device=device)
