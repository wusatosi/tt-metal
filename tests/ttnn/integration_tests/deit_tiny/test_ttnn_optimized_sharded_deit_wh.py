...
@pytest.mark.parametrize("model_name", ["facebook/deit-tiny-patch16-224"])
...
def test_deit_patch_embeddings(device, model_name, batch_size, image_size, image_channels):
    ...
    model = transformers.ViTForImageClassification.from_pretrained("facebook/deit-tiny-patch16-224")
...
def test_deit_encoder(device, model_name, batch_size, sequence_size):
    ...
    config.num_hidden_layers = 12
    model = transformers.ViTForImageClassification.from_pretrained(
        "facebook/deit-tiny-patch16-224", config=config
    ).vit.encoder
    model.classifier = nn.Linear(192, 10, bias=True)
...
def test_deit(device, model_name, batch_size, image_size, image_channels, sequence_size):
    ...
    assert_with_pcc(torch_output, output[0, 0, :1000], 0.66)
