...
def test_ada_layernorm_zero(device, x_shape, reset_seeds):
    ...
    reference_model.load_state_dict(pipe.transformer.transformer_blocks[0].norm1_context.state_dict())
    reference_model.eval()
    torch_input_x = torch.load(
        "models/experimental/functional_stable_diffusion3_5/demo/512_padded_inputs_torch/torch_encoder_hidden_states_layer_0.pt",
        map_location=torch.device("cpu"),
    )
    torch_innput_emb = torch.load(
        "models/experimental/functional_stable_diffusion3_5/demo/512_padded_inputs_torch/torch_temb_layer_0.pt",
        map_location=torch.device("cpu"),
    )
    ...
    for i in range(len(torch_output)):
        ...
        if len(torch_output_shape) > 2:
            print("torch_output[i].unsqueeze(1)", torch_output[i].unsqueeze(1).shape)
            print("ttnn.to_torch(ttnn_output[i])", ttnn.to_torch(ttnn_output[i]).shape)
            assert_with_pcc(torch_output[i].unsqueeze(1), ttnn.to_torch(ttnn_output[i]), pcc=1)  # pcc 0.92
        else:
            print("torch_output[i].unsqueeze(1)", torch_output[i].unsqueeze(1).unsqueeze(1).shape)
            print("ttnn.to_torch(ttnn_output[i])", ttnn.to_torch(ttnn_output[i]).shape)
            assert_with_pcc(
