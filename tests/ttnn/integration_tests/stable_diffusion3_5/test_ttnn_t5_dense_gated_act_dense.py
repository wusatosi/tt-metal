...
@skip_for_grayskull()
def test_t5_dense_gated_act_dense(device, reset_seeds):
    pipe = StableDiffusion3Pipeline.from_pretrained(
        "stabilityai/stable-diffusion-3.5-medium", torch_dtype=torch.bfloat16
    )
    config = pipe.text_encoder_3.config

    reference_model = T5DenseGatedActDense(config).to(dtype=torch.bfloat16)
    ...
    ttnn_model = ttnn_T5DenseGatedActDense(config)
