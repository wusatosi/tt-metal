...
def test_t5_attention(device, reset_seeds, idx):
    ...
    reference_model = T5Attention(config, has_relative_attention_bias=has_relative_attention_bias, layer_idx=idx).to(
    ...
    mask = torch.randn(1, 1, 1, 256, dtype=torch.bfloat16)
    ...
    ttnn_model = ttnn_T5Attention(config, has_relative_attention_bias, idx)
    ...
    assert_with_pcc(torch_output[0], ttnn.to_torch(ttnn_output[0]), pcc=0.98)
    if idx != 0:
