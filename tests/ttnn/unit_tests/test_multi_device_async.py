...
def test_ttnn_to_and_from_multi_device_shard(t3k_mesh_device, layout, memory_config, dtype):
    ...
    for i in range(100):
        ...
        ttnn_tensor = ttnn.from_torch(
            torch_tensor, dtype=dtype, layout=layout, mesh_mapper=ShardTensorToMesh(t3k_mesh_device, dim=3)
        )
        ttnn_tensor = ttnn.to_device(ttnn_tensor, t3k_mesh_device, memory_config=memory_config)
        ttnn_loop_back_tensor = ttnn.from_device(ttnn_tensor)
        torch_loop_back_tensor = ttnn.to_torch(
            ttnn_loop_back_tensor, mesh_composer=ConcatMeshToTensor(t3k_mesh_device, dim=3)
        )
...
@pytest.mark.parametrize("memory_config", [ttnn.DRAM_MEMORY_CONFIG, ttnn.L1_MEMORY_CONFIG])
@pytest.mark.parametrize("dtype", [ttnn.bfloat16, ttnn.bfloat8_b])
def test_multi_device_check_per_device_shard(t3k_mesh_device, layout, memory_config, dtype):
    ...
    for i in range(num_loops):
        ...
        shard_offset, shard_size = 0, int(1024 / len(t3k_mesh_device.get_device_ids()))
...
@pytest.mark.parametrize("layout", [ttnn.TILE_LAYOUT, ttnn.ROW_MAJOR_LAYOUT])
@pytest.mark.parametrize("memory_config", [ttnn.DRAM_MEMORY_CONFIG, ttnn.L1_MEMORY_CONFIG])
def test_multi_device_replicate(t3k_mesh_device, shape, layout, memory_config):
    ...
    for i in range(100):
        ...
        loopback_replicated_tensors = ttnn.to_torch(
            ttnn_loop_back_tensor, mesh_composer=ListMeshToTensor(t3k_mesh_device)
        )
...
def test_ttnn_to_multi_device_tilized_parallel(t3k_mesh_device, layout, memory_config, dtype):
    ...
    for loop in range(20):
        ...
        ttnn_tensor = ttnn.from_torch(
            torch_tensor,
            mesh_mapper=ShardTensorToMesh(t3k_mesh_device, dim=shard_dim),
            layout=layout,
            memory_config=memory_config,
            device=t3k_mesh_device,
        )
        if loop < 10:
            ...
            readback_tensor = ttnn.to_torch(
                ttnn_tensor, mesh_composer=ConcatMeshToTensor(t3k_mesh_device, dim=shard_dim), device=t3k_mesh_device
            )
        else:
            # Test Mesh Composer
            readback_tensors = ttnn.to_torch(
                ttnn_tensor, mesh_composer=ListMeshToTensor(t3k_mesh_device), device=t3k_mesh_device
            )
...
def test_multi_device_unary_binary_op_chain(t3k_mesh_device, program_cache, shape):
    ...
    for i in range(50):
        ...
        ttnn_torch_output_tensor = ttnn.to_torch(
            ttnn_output_tensor, mesh_composer=ConcatMeshToTensor(t3k_mesh_device, dim=3)
        )
...
def test_multi_device_data_parallel_op_chain(t3k_mesh_device, program_cache, input_a_shape):
    ...
    for i in range(5):
        ...
        ttnn_torch_output_tensor = ttnn.to_torch(
            ttnn_output_tensor, mesh_composer=ConcatMeshToTensor(t3k_mesh_device, dim=0)
        )
...
def test_multi_device_argmax(t3k_mesh_device, layout, mem_config):
    t3k_mesh_device.enable_async(True)
    ...
    tt_out_1B = ttnn.to_torch(tt_out_1B, mesh_composer=ttnn.ConcatMeshToTensor(t3k_mesh_device, dim=0))[0]
...
@pytest.mark.parametrize("t3k_mesh_device", [2], indirect=True)
def test_multi_device_explicit_dealloc(t3k_mesh_device):
...
@pytest.mark.parametrize("t3k_mesh_device", [2], indirect=True)
def test_add_1D_tensor_and_scalar(t3k_mesh_device, scalar, size):
    torch.manual_seed(0)
    ...
    output_tensors = ttnn.to_torch(output_tensor, mesh_composer=ttnn.ListMeshToTensor(t3k_mesh_device))
