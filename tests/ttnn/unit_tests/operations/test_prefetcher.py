...
def test_run_prefetcher(
    ...
    for i in range(num_layers * num_tensors):
        ...
        outputs_t.append(output_t)  # FIXME: Moving the ttnn.to_torch here causes a hang in matmul

    # In the order that matmul will produce results
    for l in range(num_layers):
        for t in range(num_tensors):
            tt_out = ttnn.to_torch(outputs_t[l * num_tensors + t])
            pt_out = in0 @ pt_tensors[l * num_tensors + t]
