...
@pytest.mark.parametrize(
    "N, C, H, W, num_groups",
    (
        (1, 512, 64, 64, 32),  # Failed with ERROR: a.get_legacy_shape()[3] == gamma.value().get_legacy_shape()[3]
        (
            1,
            512,
            1,
            4096,
            32,
        ),  # Actual Input shape (1,512,4096) is unsqueezed to (1, 512, 1, 4096) Failed with ERROR: a.get_legacy_shape()[3] == gamma.value().get_legacy_shape()[3]
        (1, 512, 128, 128, 32),  # Failed with ERROR: a.get_legacy_shape()[3] == gamma.value().get_legacy_shape()[3]
        (1, 512, 256, 256, 32),  # Failed with OOM
        (1, 256, 256, 256, 32),  # Failed with ERROR: a.get_legacy_shape()[3] == gamma.value().get_legacy_shape()[3]
        (1, 256, 512, 512, 32),  # Failed with OOM
        (1, 128, 512, 512, 32),  # Failed with OOM
    ),
)
def test_sd35_vae_512(device, N, C, H, W, num_groups):
    grid_size = ttnn.CoreGrid(y=8, x=8)
    # torch input tensor
    torch_input_tensor = torch.rand((N, C, H, W), dtype=torch.bfloat16)
    torch_weight = torch.ones((C,), dtype=torch.bfloat16)
