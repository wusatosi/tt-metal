...
shapes_and_output_sizes_yolov9c = [((1, 256, 160, 160), (159, 159))]


@pytest.mark.parametrize("dtype", [ttnn.float32, ttnn.bfloat16])
@pytest.mark.parametrize("input_shape, output_size", shapes_and_output_sizes_yolov9c)
def test_run_adaptive_avg_pool2d_yolov9c(device, input_shape, output_size, dtype, reset_seeds):
    torch_input_tensor = torch.randn(input_shape)
    torch_output_tensor = torch.nn.functional.adaptive_avg_pool2d(torch_input_tensor, output_size[0])
    torch_input_tensor = torch.permute(torch_input_tensor, (0, 2, 3, 1))
    input_tensor = ttnn.from_torch(torch_input_tensor, dtype=dtype, device=device)
    output_tensor = ttnn.adaptive_avg_pool2d(input_tensor, ttnn.Shape(output_size))
    output_tensor = ttnn.to_torch(output_tensor)
