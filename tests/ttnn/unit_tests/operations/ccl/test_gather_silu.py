...
def run_line_all_gather_on_TG_with_mesh_tensor_along_rows(
    ...
    # buffer tensor
    full_buffer_tensor_unfractured = torch.zeros(full_mesh_input_shape, dtype=torch.bfloat16)
    ...
    buffer_mem_config = ttnn.MemoryConfig(
    ...
    buffer_tensor = ttnn.from_torch(
        full_buffer_tensor_unfractured,
        tile=ttnn.Tile(tile),
        dtype=input_dtype,
        device=mesh_device,
        layout=layout,
        memory_config=buffer_mem_config,
        mesh_mapper=ttnn.ShardTensor2dMesh(mesh_device, mesh_shape=mesh_shape, dims=shard_dims),
    )
    buffer_tensor = ttnn.to_device(buffer_tensor, mesh_device)
    buffer_tensor = ttnn.to_memory_config(buffer_tensor, buffer_mem_config)
    ...
    tt_output_tensor = ttnn.to_torch(
        ttnn_tensor_out, mesh_composer=ttnn.ConcatMesh2dToTensor(mesh_device, mesh_shape=mesh_shape, dims=concat_dims)
    )
    ...
    print(full_input_tensor_unfractured.shape)
    output_golden[:, :, :, :] = full_input_tensor_unfractured[:, :, :, :3584].repeat(repeat_factor)
