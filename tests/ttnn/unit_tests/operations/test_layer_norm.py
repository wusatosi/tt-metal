...
def test_sdxl_turbo_layer_norm_1024x1024_dram(device, h, w):
    torch.manual_seed(0)
...
def test_sdxl_turbo_layer_norm_512x512_dram(device, h, w):
    torch.manual_seed(0)
...
def test_sdxl_turbo_layer_norm_1024x1024_l1(device, h, w):
    torch.manual_seed(0)
    ...
    input_tensor = ttnn.from_torch(torch_input_tensor, layout=ttnn.TILE_LAYOUT, device=device)
    output_tensor = ttnn.layer_norm(input_tensor, memory_config=ttnn.L1_MEMORY_CONFIG)
    output_tensor = ttnn.to_layout(output_tensor, ttnn.ROW_MAJOR_LAYOUT)
    output_tensor = ttnn.from_device(output_tensor)
    output_tensor = ttnn.to_torch(output_tensor)
...
def test_sdxl_turbo_layer_norm_512x512_l1(device, h, w):
    torch.manual_seed(0)
