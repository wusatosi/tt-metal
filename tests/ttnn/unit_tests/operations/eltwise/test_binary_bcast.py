...
@pytest.mark.parametrize(
    "input_shapes",
    (
        (torch.Size([3, 2, 23, 23]), torch.Size([1, 2, 1, 1])),
...
def test_binary_scalar_ops(input_shapes, device):
    ...
    cq_id = 0
    out_tt = ttnn.experimental.add(a_tt, b_tt)
    print(out_tt.shape)
    out_pt = a_pt - b_pt
...
#     torch_output_tensor = torch_input_tensor_a + torch_input_tensor_b.view(1, 3, 1, 1)
