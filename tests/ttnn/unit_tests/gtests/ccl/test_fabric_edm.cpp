...
int main(int argc, char** argv) {
    ...
    // std::string devices_str = argv[arg_idx++];

    // std::vector<std::pair<size_t, size_t>> devices;
    // std::stringstream ss(devices_str);
    // std::string dev_str;
    // while (std::getline(ss, dev_str, '-')) {
    //     size_t x, y;
    //     char discard;
    //     std::stringstream dev_stream(dev_str);
    //     dev_stream >> discard >> x >> discard >> y >> discard;
    ...
    // test_params.params.devices = devices;
