...
std::vector<std::tuple<int, int, int, bool, bool, int, int, int>> matmul_shapes_bfloat16_HiFi2 = {
...
std::vector<std::tuple<int, int, int, bool, bool, int, int, int>> matmul_shapes_bfloat16_HiFi4 = {
    // {512, 512, 512, true, true, 1, 1, 1},
    // {512, 1024, 1024, true, true, 1, 1, 1},
    {512, 1024, 2048, true, true, 1, 1, 1},
    // {1024, 1024, 1024, true, true, 1, 1, 1},
...
std::vector<std::tuple<DataType, MathFidelity, bool>> matmul_configs = {
    {DataType::BFLOAT16, MathFidelity::HiFi2, false}, {DataType::BFLOAT16, MathFidelity::HiFi4, false},
...
TEST_P(Matmul2DHostPerfTestFixture, Matmul2DHostPerfTest) {
    ...
    for (const auto& config : matmul_configs) {
        ...
        if (dtype == DataType::BFLOAT16) {
            if (math_fidelity == MathFidelity::HiFi2) {
                matmul_shapes = matmul_shapes_bfloat16_HiFi2;
            } else if (math_fidelity == MathFidelity::HiFi4) {
                matmul_shapes = matmul_shapes_bfloat16_HiFi4;
            }
