...
INSTANTIATE_TEST_SUITE_P(
    ...
    [](const testing::TestParamInfo<std::tuple<ttnn::TensorSpec, ttnn::compiler_interface::ResourceUsage>> &info) {
        ...
        ss << "_" << std::get<ttnn::TensorSpec>(info.param).second;

        // print tensor shape; operator<< exists but is too long to be used here
        ss << "_";
        detail::operator<<(ss, std::get<ttnn::TensorSpec>(info.param).first);
        return ss.str();
    });
...
INSTANTIATE_TEST_SUITE_P(
    ...
    [](const testing::TestParamInfo<
        ...
        // print tensor layout
        using detail::operator<<;
        ss << "_" << std::get<0>(info.param).second;

        // print tensor shape; operator<< exists but is too long to be used here
        ss << "_";
        detail::operator<<(ss, std::get<0>(info.param).first);

        ss << "_";

        // print tensor layout
        using detail::operator<<;
        ss << "_" << std::get<1>(info.param).second;

        // print tensor shape; operator<< exists but is too long to be used here
        ss << "_";
        detail::operator<<(ss, std::get<1>(info.param).first);
    ...
    // Run the test
    {
        ...
        const auto output_spec = ttnn::TensorSpec(
            ttnn::SimpleShape(tt::tt_metal::Array4D{
                input_spec_a.first[0], input_spec_a.first[1], input_spec_a.first[-2], input_spec_b.first[-1]}),
            tt::tt_metal::TensorLayout(
                tt::tt_metal::DataType::BFLOAT16,
                tt::tt_metal::PageConfig(tt::tt_metal::Layout::TILE),
                ttnn::L1_MEMORY_CONFIG));

        std::cout << input_spec_a.first << "\n" << input_spec_b.first << "\n" << output_spec.first << std::endl;
