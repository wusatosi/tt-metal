...
def run_nlp_create_qkv_heads_sd35_test(batch, seq_len, hidden_dim, dtype, in0_mem_config, out_mem_config, device):
    ...
    q = ttnn.experimental.nlp_create_qkv_heads_sd35(in0_t, memory_config=out_mem_config)[0]
...
@pytest.mark.parametrize(
    ...
    ((2, 4096, 1536), (2, 1024, 1536)),  # (1, 256, 256)),
    ids=[
        "batch1_seq4k",
        "batch1_seq1k",
    ],
)
def test_nlp_create_qkv_heads_sd35_test(
    batch, seq_len, hidden_dim, dtype, in0_mem_config, out_mem_config, request, device
):
    if is_grayskull() and dtype == ttnn.float32:
        pytest.skip("Skipping float32 tests on Grayskull")
    run_nlp_create_qkv_heads_sd35_test(batch, seq_len, hidden_dim, dtype, in0_mem_config, out_mem_config, device)


def test_nlp_create_qkv_heads_sd35_with_program_cache(device, use_program_cache):
    dtype = ttnn.bfloat8_b
    mem_config = ttnn.DRAM_MEMORY_CONFIG
    for _ in range(2):
        run_nlp_create_qkv_heads_sd35_test(2, 4096, 1536, dtype, mem_config, mem_config, device)
        dummy_shape = [2, 1, 4096, 1536]
