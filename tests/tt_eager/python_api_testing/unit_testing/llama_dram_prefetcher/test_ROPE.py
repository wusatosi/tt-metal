...
@pytest.mark.parametrize("activation_shape, shard_cores", [[(1, 8, 8, 128), 8]])
def test_ROPE(device, pcc, activation_shape, shard_cores):
    ...
    trans_mat = torch.ones((1, 1, 32 * activation_shape[1] * 2, 32), dtype=torch.bfloat16)
    trans_mat_core_range_set = get_compact_core_range_set(1, row_wise=True)
    cos_mat = torch.ones((1, 1, 32 * activation_shape[1] * 2, 32), dtype=torch.bfloat16)
    sin_mat = torch.ones((1, 1, 32 * activation_shape[1] * 2, 32), dtype=torch.bfloat16)
    cos_mat_core_range_set = get_compact_core_range_set(1, row_wise=True)
