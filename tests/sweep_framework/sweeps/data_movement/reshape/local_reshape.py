...
def run(
    ...
    if tuple(input_shape) not in computed_shapes:
        shape = gen_reshape_shape(input_shape)
        num_tries = 50
        i = 0

        # If we store result to L1 required volume should not be larger than 1Mb
        while input_a_memory_config == ttnn.L1_MEMORY_CONFIG and max_volume(shape) > 1024 * 1024 and i < num_tries:
            shape = gen_reshape_shape(input_shape)
            i += 1
        computed_shapes[tuple(input_shape)] = shape

    # Use the precomputed output shape
    rehape_shape = computed_shapes[tuple(input_shape)]
    ...
    # print(pcc)
    return [pcc, e2e_perf, rehape_shape]
...
nightly_params = parameters["nightly"]

# Generate all combinations of parameters
param_combinations = list(
    itertools.product(
        nightly_params["input_shape"],
        nightly_params["input_a_dtype"],
        nightly_params["input_a_layout"],
        nightly_params["input_a_memory_config"],
    )
)
...
for input_shape, input_a_dtype, input_a_layout, input_a_memory_config in tqdm(
    param_combinations, desc="Running tests", unit="test"
):
    output = run(
        ...
        device=device,  # Ensure 'device' is defined in your environment
    )
    ...
    results.append(
        {
            "input_shape": input_shape,
            "output_shape": output[2],  # Third item from run() output
            "input_a_dtype": str(input_a_dtype),  # Convert to string for readability
            "input_a_layout": str(input_a_layout),
            "input_a_memory_config": str(input_a_memory_config),
            "pcc": output[0],  # First item from run() output
            "e2e_perf": output[1],  # Second item from run() output
        }
    )
...
# Save to CSV
df_results.to_csv("reshape_sweeps_local.csv", index=False)

print("Run completed. Results saved to 'reshape_sweeps_local.csv'.")
