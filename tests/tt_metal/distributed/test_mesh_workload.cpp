...
TEST_F(MeshWorkloadTest, DataCopyOnSubDevices) {
    ...
    auto syncer_coord = sub_device_2.cores(HalProgrammableCoreType::TENSIX).ranges().at(0).start_coord;
    ...
    auto datacopy_coord = sub_device_1.cores(HalProgrammableCoreType::TENSIX).ranges().at(0).start_coord;
    ...
    auto datacopy_kernel = CreateKernel(
        ...
        datacopy_core,
        DataMovementConfig{.processor = DataMovementProcessor::RISCV_0, .noc = NOC::RISCV_0_default});
    std::array<uint32_t, 5> datacopy_rt_args = {
        global_sem.address(), 0, 0, input_buf->address(), output_buf->address()};
    SetRuntimeArgs(datacopy_program, datacopy_kernel, datacopy_core, datacopy_rt_args);
    constexpr uint32_t src0_cb_index = CBIndex::c_0;
    CircularBufferConfig cb_src0_config = CircularBufferConfig(single_tile_size, {{src0_cb_index, DataFormat::UInt32}})
                                              .set_page_size(src0_cb_index, single_tile_size);
    ...
    for (std::size_t logical_x = 0; logical_x < input_buf->device()->num_cols(); logical_x++) {
        for (std::size_t logical_y = 0; logical_y < input_buf->device()->num_rows(); logical_y++) {
            ...
            ReadShard(mesh_device_->mesh_command_queue(), dst_vec, input_buf, Coordinate(logical_y, logical_x));
        }
    }
