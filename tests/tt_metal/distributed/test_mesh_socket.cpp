...
TEST_F(MeshSocketTest, MultiConnectionMultiDeviceTest) {
    ...
    for (const auto& device_coord : MeshCoordinateRange(md0->shape())) {
        ...
        ReadShard(md0->mesh_command_queue(), sender_configs, send_socket.config_buffer, device_coord);
        ReadShard(md1->mesh_command_queue(), recv_configs, recv_socket.config_buffer, device_coord);
