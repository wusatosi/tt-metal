...
// Register MeshSubDevice Test suite for T3K and N300
template <typename T>
using MeshSubDeviceTestSuite = MeshTestSuite<T>;
TYPED_TEST_SUITE(MeshSubDeviceTestSuite, MeshFixtureTypes);

TYPED_TEST(MeshSubDeviceTestSuite, SyncWorkloadsOnSubDevice) {
    SubDevice sub_device_1(std::array{CoreRangeSet(CoreRange({0, 0}, {2, 2}))});
    SubDevice sub_device_2(std::array{CoreRangeSet(std::vector{CoreRange({3, 3}, {3, 3}), CoreRange({4, 4}, {4, 4})})});

    uint32_t num_iters = 5;
    auto sub_device_manager = this->mesh_device_->create_sub_device_manager({sub_device_1, sub_device_2}, 3200);
    this->mesh_device_->load_sub_device_manager(sub_device_manager);

    auto [waiter_program, syncer_program, incrementer_program, global_sem] =
        create_basic_sync_program(this->mesh_device_.get(), sub_device_1, sub_device_2);
    ...
    for (uint32_t i = 0; i < num_iters; i++) {
        EnqueueMeshWorkload(this->mesh_device_->mesh_command_queue(), waiter_mesh_workload, false);
        this->mesh_device_->set_sub_device_stall_group({SubDeviceId{0}});
        EnqueueMeshWorkload(this->mesh_device_->mesh_command_queue(), syncer_mesh_workload, true);
        EnqueueMeshWorkload(this->mesh_device_->mesh_command_queue(), incrementer_mesh_workload, false);
        this->mesh_device_->reset_sub_device_stall_group();
    }
    Finish(this->mesh_device_->mesh_command_queue());
}

TYPED_TEST(MeshSubDeviceTestSuite, DataCopyOnSubDevices) {
    ...
    // Create IO Buffers
    auto input_buf = MeshBuffer::create(global_buffer_config, per_device_buffer_config, this->mesh_device_.get());
    auto output_buf = MeshBuffer::create(global_buffer_config, per_device_buffer_config, this->mesh_device_.get());

    // Create and Load SubDeviceConfig on the mesh
    auto sub_device_manager =
        this->mesh_device_->create_sub_device_manager({sub_device_1, sub_device_2, sub_device_3}, 3200);
    this->mesh_device_->load_sub_device_manager(sub_device_manager);
    ...
    auto syncer_core_phys = this->mesh_device_->worker_core_from_logical_core(syncer_coord);
    ...
    auto datacopy_core_phys = this->mesh_device_->worker_core_from_logical_core(datacopy_coord);

    auto all_cores = syncer_core.merge(datacopy_core);
    auto global_sem = CreateGlobalSemaphore(this->mesh_device_.get(), all_cores, 0);
    ...
    for (int i = 0; i < 50; i++) {
        this->mesh_device_->set_sub_device_stall_group({SubDeviceId{2}});
        EnqueueMeshWorkload(this->mesh_device_->mesh_command_queue(), syncer_mesh_workload, false);
        EnqueueMeshWorkload(this->mesh_device_->mesh_command_queue(), datacopy_mesh_workload, false);
        ...
        // buffer data
        EnqueueWriteMeshBuffer(this->mesh_device_->mesh_command_queue(), input_buf, src_vec, true);
        ...
        for (std::size_t logical_x = 0; logical_x < output_buf->device()->num_cols(); logical_x++) {
            for (std::size_t logical_y = 0; logical_y < output_buf->device()->num_rows(); logical_y++) {
                ...
                ReadShard(
                    this->mesh_device_->mesh_command_queue(), dst_vec, output_buf, Coordinate(logical_y, logical_x));
...
TYPED_TEST(MeshSubDeviceTestSuite, SubDeviceSwitching) {
    ...
    auto sub_device_manager_0 = this->mesh_device_->create_sub_device_manager({sub_device_1, sub_device_2}, 3200);
    auto sub_device_manager_1 = this->mesh_device_->create_sub_device_manager({sub_device_3, sub_device_4}, 3200);
    ...
    auto [waiter_program_1, syncer_program_1, incrementer_program_1, global_sem_1] =
        create_basic_sync_program(this->mesh_device_.get(), sub_device_3, sub_device_4);
    ...
    for (uint32_t i = 0; i < num_iters; i++) {
        this->mesh_device_->load_sub_device_manager(sub_device_manager_0);
        EnqueueMeshWorkload(this->mesh_device_->mesh_command_queue(), waiter_mesh_workload, false);
        this->mesh_device_->set_sub_device_stall_group({SubDeviceId{0}});
        EnqueueMeshWorkload(this->mesh_device_->mesh_command_queue(), syncer_mesh_workload, true);
        EnqueueMeshWorkload(this->mesh_device_->mesh_command_queue(), incrementer_mesh_workload, false);
        ...
        this->mesh_device_->load_sub_device_manager(sub_device_manager_1);
        EnqueueMeshWorkload(this->mesh_device_->mesh_command_queue(), waiter_mesh_workload_1, false);
        this->mesh_device_->set_sub_device_stall_group({SubDeviceId{0}});
        EnqueueMeshWorkload(this->mesh_device_->mesh_command_queue(), syncer_mesh_workload_1, true);
        EnqueueMeshWorkload(this->mesh_device_->mesh_command_queue(), incrementer_mesh_workload_1, false);
        this->mesh_device_->reset_sub_device_stall_group();
    }
    Finish(this->mesh_device_->mesh_command_queue());
}
