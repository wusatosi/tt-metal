...
void RunDelayTestOnCore(WatcherDelayFixture* fixture, Device* device, CoreCoord &core) {
    tt_metal::Program program = tt_metal::CreateProgram();
        ...
        CoreCoord worker_core =
            fixture->delayed_cores[CoreType::TENSIX][0];  // Just check that the first delayed core has the feedback set
        CoreCoord phys_core = device->virtual_core_from_logical_core({0, 0}, CoreType::TENSIX);
