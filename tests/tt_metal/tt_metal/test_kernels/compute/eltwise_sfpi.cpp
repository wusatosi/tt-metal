...
void MAIN {
    ...
    for (uint32_t block_index = 0; block_index < per_core_block_cnt; block_index++) {
        ckernel::cb_reserve_back(CBIndex::c_16, per_core_block_dim);
        for (uint32_t tile_index = 0; tile_index < per_core_block_dim; ++tile_index) {
            ckernel::acquire_dst();

            // Pop tile after tile, copy to DST and pack
            ckernel::cb_wait_front(CBIndex::c_0, 1);

            ckernel:: copy_tile(CBIndex::c_0, 0, 0);
            ...
            // "sfpu_gelu(0); ckernel:: pack_tile(0, CBIndex::c_16);"

            SFPI_OP_AND_PACK
            // comes from add_define in kernel config
            // Also is epxected to include ckernel:: pack_tile(0, CBIndex::c_16); for non-relu
            // For relu it expects the hlk_pack_relu variant

            ckernel::cb_pop_front(CBIndex::c_0, 1);

            ckernel:: release_dst();
        }
        ckernel::cb_push_back(CBIndex::c_16, per_core_block_dim);
    }
    DEINIT_RELU
