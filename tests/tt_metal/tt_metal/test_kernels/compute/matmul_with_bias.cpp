...
void MAIN {
    ...
    for (uint32_t b = 0; b < block_cnt; ++b) {
        ckernel::cb_wait_front(tt::CBIndex::c_0, in0_block_tile_cnt);
        ckernel::cb_wait_front(tt::CBIndex::c_1, in1_block_tile_cnt);
        ...
        ckernel::cb_pop_front(tt::CBIndex::c_0, in0_block_tile_cnt);
        ckernel::cb_pop_front(tt::CBIndex::c_1, in1_block_tile_cnt);
    }

    // add bias in2 to intermed0 and load to dst
    if (with_bias) {
        // Pack out
        ckernel::cb_reserve_back(tt::CBIndex::c_24, out_block_tile_cnt);
        for (uint32_t i = 0; i < out_block_tile_cnt; ++i) {
            ckernel:: pack_tile(i, tt::CBIndex::c_24);
        }
        ckernel::cb_push_back(tt::CBIndex::c_24, out_block_tile_cnt);
        ckernel:: release_dst();

        ckernel::acquire_dst();

        add_bcast_rows_init_short(tt::HlkOperand::intermed0, tt::HlkOperand::in2);
        ckernel::cb_wait_front(tt::CBIndex::c_24, out_block_tile_cnt);
        ckernel::cb_wait_front(tt::CBIndex::c_2, dst_tile_cols);
        int dst_tile_index = 0;
        for (uint32_t r = 0; r < dst_tile_rows; ++r) {
            for (uint32_t c = 0; c < dst_tile_cols; ++c) {
        ...
        ckernel::cb_pop_front(tt::CBIndex::c_2, dst_tile_cols);
    }

    // Pack to c_out0
    ckernel::cb_reserve_back(tt::CBIndex::c_16, out_block_tile_cnt);
    for (uint32_t i = 0; i < out_block_tile_cnt; ++i) {
        ckernel:: pack_tile(i, tt::CBIndex::c_16);
    }

    ckernel::cb_push_back(tt::CBIndex::c_16, out_block_tile_cnt);
    ckernel:: release_dst();
}
}  // namespace NAMESPACE
