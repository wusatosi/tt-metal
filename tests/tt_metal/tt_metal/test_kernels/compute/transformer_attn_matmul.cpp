            ...
            for (uint32_t nt_C = 0; nt_C < Nt; ++nt_C)  // output tile index of C
            {
                for (uint32_t tile_row_id = 0; tile_row_id < num_rows_in_one_tile; tile_row_id++) {
                    ...
                    for (uint32_t kt = 0; kt < Kt; kt++) {
                        if (tile_row_id == 0) {
                            ckernel::cb_wait_front(tt::CBIndex::c_0, kt + 1);
                        }
                    ...
                    ckernel::cb_reserve_back(cb_intermed1, 1);
                    untilize_block(cb_intermed0, 1, cb_intermed1);
                    ckernel::cb_push_back(cb_intermed1, 1);

                    ckernel::cb_pop_front(cb_intermed0, 1);
                    untilize_uninit(cb_intermed0);

                    mm_init_short(tt::CBIndex::c_0, tt::CBIndex::c_1, transpose_hw);
                }
                ckernel::cb_pop_front(tt::CBIndex::c_0, Kt);

                // cb_intermed2 comes from reader; untilized row-major tile
                ckernel::cb_wait_front(cb_intermed2, 1);
                ...
                ckernel::cb_pop_front(cb_intermed2, 1);
