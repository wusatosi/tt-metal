...
void kernel_main() {
    ...
    volatile tt_l1_ptr uint32_t* l1_valid_value_addr_ptr =
        reinterpret_cast<volatile tt_l1_ptr uint32_t*>(l1_valid_value_addr);
    *(l1_valid_value_addr_ptr) = VALID;
    ...
    for (uint32_t j = 0; j < num_repetitions; j++) {
        for (uint32_t i = 0; i < num_tiles; i += block_size_tiles) {
            // wait until receiver has set the sender's semaphore_addr value to 1, which means receiver has reserved
            // space in the CB
