...
FORCE_INLINE void send_uni_dir(
    ...
    while (sender_num_messages_send < total_msgs) {
        update_sender_state<sync_reg_id>(
            buffer_slot_addrs,
            full_payload_size,
...
void kernel_main() {
    ...
    init_ptr_val<sync_reg_id>(NUM_BUFFER_SLOTS);  // sending path
    init_ptr_val<bidir_sync_reg_id>(0);           // receiving path
    ...
    switch (benchmark_type) {
        ...
        case EthOnlyBiDir: {
            DeviceZoneScopedN("MAIN-TEST-BODY");
            send_receiver_bi_dir<sync_reg_id, bidir_sync_reg_id, false>(
        ...
        case EthEthTensixBiDir: {
            DeviceZoneScopedN("MAIN-TEST-BODY");
            send_receiver_bi_dir<sync_reg_id, bidir_sync_reg_id, true>(
                sender_buffer_slot_addrs,
                receiver_buffer_slot_addrs,
                full_payload_size,
                message_size,
