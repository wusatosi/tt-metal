...
constexpr uint32_t bidir_sync_reg_id = 1;
...
template <uint32_t directional_stream_id>
FORCE_INLINE void write_receiver(uint32_t buffer_slot_addr, uint32_t full_payload_size) {
    increment_local_update_ptr_val<directional_stream_id>(-1);
    ...
    remote_update_ptr_val<directional_stream_id>(1);
}
...
FORCE_INLINE void check_buffer_full_and_send_packet(
    ...
    bool buffer_not_full = (get_ptr_val<directional_stream_id>() != 0);

    if (buffer_not_full && num_messages_send != total_msgs) {
        write_receiver<directional_stream_id>(buffer_slot_addrs[write_ptr], full_payload_size);
...
FORCE_INLINE void update_sender_state(
    ...
    check_buffer_full_and_send_packet<directional_stream_id>(
...
template <uint32_t directional_stream_id>
FORCE_INLINE bool has_incoming_packet() {
    return get_ptr_val<directional_stream_id>() != 0;
}
...
template <uint32_t directional_stream_id, bool write_to_worker>
FORCE_INLINE void check_incoming_packet_and_write_worker(
    ...
    if (has_incoming_packet<directional_stream_id>()) {
        increment_local_update_ptr_val<directional_stream_id>(-1);
...
FORCE_INLINE void check_write_worker_done_and_send_ack(
    ...
    if (send_ack_condition) {
        ack_complete<directional_stream_id>(buffer_slot_addrs[read_ptr], full_payload_size);
...
FORCE_INLINE void update_receiver_state(
    ...
    check_incoming_packet_and_write_worker<directional_stream_id, write_to_worker>(
    ...
    check_write_worker_done_and_send_ack<directional_stream_id, write_to_worker>(
...
template <uint32_t dir0_stream_id, uint32_t dir1_stream_id, bool write_to_worker>
FORCE_INLINE void send_receiver_bi_dir(
    ...
    uint64_t worker_noc_addr) {
    uint32_t total_msgs_per_dir;
    if constexpr (measurement_type == MeasurementType::Latency) {
        total_msgs_per_dir = num_messages;
    } else {
        total_msgs_per_dir = num_messages * NUM_BUFFER_SLOTS;
    }
    ...
    while ((receiver_num_messages_ack + sender_num_messages_send) < total_msgs_per_dir * 2) {
        update_sender_state<dir0_stream_id>(
        ...
        update_receiver_state<dir1_stream_id, write_to_worker>(
    ...
    while (get_ptr_val<dir0_stream_id>() != NUM_BUFFER_SLOTS) {
        switch_context_if_debug();
    }
}
