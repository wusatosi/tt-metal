...
std::tuple<std::vector<tt_metal::Program>, std::unique_ptr<tt::tt_metal::experimental::GlobalCircularBuffer>> create_programs(
    ...
    auto writer_cb = tt_metal::experimental::CreateCircularBuffer(sender_program, dram_reader_core, in1_writer_cb_config, *global_cb);
    ...
    in1_receiver_cb_config.remote_index(in1_receiver_cb_index).set_page_size(single_tile_size).set_data_format(tile_format);
    in1_receiver_cb_config.index(in1_pusher_cb_index).set_page_size(single_tile_size).set_data_format(tile_format);
    auto in1_receiver_cb = tt_metal::experimental::CreateCircularBuffer(receiver_program, l1_receiver_cores, in1_receiver_cb_config, *global_cb);
    ...
    std::vector<uint32_t> in1_receiver_compile_time_args = {
        (std::uint32_t) num_layers,
        (std::uint32_t) in1_receiver_cb_index
    };
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        //                      Application Setup
        ////////////////////////////////////////////////////////////////////////////
        auto [programs, global_cb] = create_programs(device, dram_reader_core, l1_receiver_core, sender_receiver_core_mapping, single_tile_size, tile_format, m, k, n, num_blocks, cb_num_blocks, num_receivers, num_layers, cb_padding, in0_buffer, in1_buffers[0], output_buffer, use_sub_devices);
