...
void kernel_main() {
    ...
    const tt_l1_ptr uint32_t* num_blocks =
        (const tt_l1_ptr uint32_t*)(get_arg_addr(increment_arg_idx(rt_args_idx, num_layers)));
    const tt_l1_ptr uint32_t* in0_block_tiles =
        (const tt_l1_ptr uint32_t*)(get_arg_addr(increment_arg_idx(rt_args_idx, num_layers)));
    const tt_l1_ptr uint32_t* out_block_num_tiles =
        (const tt_l1_ptr uint32_t*)(get_arg_addr(increment_arg_idx(rt_args_idx, num_layers)));
