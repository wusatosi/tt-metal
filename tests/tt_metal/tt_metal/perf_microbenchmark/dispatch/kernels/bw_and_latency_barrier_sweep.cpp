...
void kernel_main() {
    ...
    uint64_t noc_addresses[PAGE_COUNT];
    for (int j = 0; j < PAGE_COUNT; j++) {
        noc_addresses[j] = get_dram_noc_addr(j, page_size, 0); 
    }
    ...
    for (int i = 0; i < ITERATIONS; i++) {
        ...
        for (int j = 0; j < PAGE_COUNT; j++) {
            ...
            uint64_t noc_addr = noc_addresses[j];
#else
...
#endif
            if (j > 0 && (j % READ_BARRIER_PERIOD) == 0) { noc_async_write_barrier(); }
#else
            if (j > 0 && (j % READ_BARRIER_PERIOD) == 0) { noc_async_read_barrier(); }
#endif
#endif
            read_ptr += page_size;
            write_ptr += page_size;
