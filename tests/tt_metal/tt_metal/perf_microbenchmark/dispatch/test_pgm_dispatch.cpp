...
bool initialize_program(
    ...
    CoreRange total_kg = {info.workers.start_coord, {info.workers.end_coord.x - info.n_kgs + 1, info.workers.end_coord.y}};
...
static int pgm_dispatch(T& state, TestInfo info) {
    ...
    try {
        ...
        if (info.n_subdevice_ranges > 1) {
            ...
            auto grid_size = device->compute_with_storage_grid_size();
            CoreRange all_core_range{{0, 0}, {grid_size.x - 1, grid_size.y - 1}};
            core_ranges[static_cast<size_t>(HalProgrammableCoreType::TENSIX)] = get_subdevice_core_range_set(info, all_core_range);
...
BENCHMARK_CAPTURE(
    ...
    256_bytes_brisc_only_left_processors_trace,
