...
std::tuple<std::vector<tt_metal::Program>, std::unique_ptr<tt_metal::experimental::GlobalCircularBuffer>> create_programs(
    ...
    auto global_cb = tt_metal::experimental::CreateGlobalCircularBuffer(device, sender_receiver_core_mapping, padded_global_cb_size, tt_metal::BufferType::L1);
    ...
    auto writer_cb = tt_metal::experimental::CreateCircularBuffer(sender_program, dram_reader_core, writer_cb_config, *global_cb);
    ...
    auto receiver_cb = tt_metal::experimental::CreateCircularBuffer(receiver_program, l1_receiver_cores, receiver_cb_config, *global_cb);
    ...
    std::vector<uint32_t> receiver_compile_time_args = {
        (std::uint32_t) num_mixed_df_layers,
        (std::uint32_t) receiver_cb_index
    };
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        TT_FATAL(num_mixed_df_layers == 1, "this test does not support mixed precision testing");
        TT_FATAL(num_mixed_df_layers % 2 == 1, "currently only support odd number of layers testing, due to issue with validation");
        ...
        //                      Application Setup
        ////////////////////////////////////////////////////////////////////////////
        auto [programs, global_cb] = create_programs(device, dram_reader_core, l1_receiver_core, sender_receiver_core_mapping, single_tile_size, tile_format, k, n, num_blocks, cb_num_blocks, num_receivers, num_mixed_df_layers, cb_padding, input_buffers[0], use_sub_devices);
        if (tile_format == tt::DataFormat::Bfp8_b) {
            // output
            vector<uint32_t> outputs = create_constant_vector_of_bfp8(output_size, 0, true);
            output_buffer = create_and_transfer_data_sharded_cb(device, outputs, kt / num_blocks * cb_num_blocks, nt, tt_metal::BufferType::L1, tt::DataFormat::Bfp8_b, l1_receiver_core, num_receivers, global_cb->buffer_address());

        } else {
            // output
            vector<uint32_t> outputs = create_constant_vector_of_bfloat16(output_size, 0);
            output_buffer = create_and_transfer_data_sharded_cb(device, outputs, kt / num_blocks * cb_num_blocks, nt, tt_metal::BufferType::L1, tt::DataFormat::Float16_b, l1_receiver_core, num_receivers, global_cb->buffer_address());
        }
