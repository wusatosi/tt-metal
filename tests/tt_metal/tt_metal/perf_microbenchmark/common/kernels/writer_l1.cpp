...
void kernel_main() {
    ...
    for (uint32_t l = 0; l < num_layers; ++l) {
        ...
        for (uint32_t block = 0; block < curr_num_blocks; ++block) {
            ...
            experimental::remote_cb_reserve_back(remote_cb_id, curr_receiver_block_num_tiles);
            experimental::remote_cb_push_back_and_write_pages(remote_cb_id, local_cb_addr, curr_receiver_block_num_tiles, curr_num_tile_rows, curr_coalesced_num_pages, curr_coalesced_page_size, noc);
