...
// uint32_t get_noc_distance(uint32_t noc_idx, CoreCoord src, CoreCoord dest, uint32_t grid_size_x, uint32_t
// grid_size_y) {
//     uint32_t x_dist = 0;
//     uint32_t y_dist = 0;
...
//                     auto physical_mcast_chip_ids = this->get_physical_mcast_chip_ids(physical_chip_ids[i],
//                     mcast_depth); if (!physical_mcast_chip_ids.empty()) {
...
//         uint32_t num_routers = router_logical_cores.size();
//         uint32_t sync_addr = gk_interface_addr + offsetof(gatekeeper_info_t, router_sync) + offsetof(sync_word_t,
//         val); while (num_routers != gk_status) {
...
//                     // need to check if the next high priority worker for this router has already been picked by
//                     another
//                     // router
//                     auto it =
...
//         rx_workers = rx_devices[0]->select_worker_cores(dest_routers, num_links_to_use, num_rx_workers,
//         num_cores_to_skip);
...
//                 while (true) {
//                     auto tx_status =
//                         tt::llrt::read_hex_vec_from_core(rx_device->physical_chip_id, rx_core, test_results_address,
//                         4);
...
//     // TODO: use eth_l1_mem::address_map::ERISC_L1_UNRESERVED_BASE which should be 0x19900, set test results size
//     back
//     // to 0x7000
//     constexpr uint32_t default_tunneler_queue_size_bytes = 0x8000;   // maximum queue (power of 2)
//     constexpr uint32_t default_tunneler_test_results_addr = 0x39000; // 0x8000 * 4 + 0x19000; 0x10000 * 4 + 0x19000 =
//     0x59000 > 0x40000 (256kB) constexpr uint32_t default_tunneler_test_results_size = 0x6000;  // 256kB total L1 in
//     ethernet core - 0x39000
...
//         log_info(LogTest, "  --data_kb_per_tx: Total data in KB per TX endpoint, default = {}",
//         default_data_kb_per_tx); log_info(LogTest, "  --tx_x: X coordinate of the starting TX core, default = {}",
//         default_tx_x); log_info(LogTest, "  --tx_y: Y coordinate of the starting TX core, default = {}",
//         default_tx_y); log_info(LogTest, "  --rx_x: X coordinate of the starting RX core, default = {}",
//         default_rx_x); log_info(LogTest, "  --rx_y: Y coordinate of the starting RX core, default = {}",
//         default_rx_y); log_info(
//             LogTest,
//             "  --routing_table_start_addr: Routing Table start address, default = 0x{:x}",
//             default_routing_table_start_addr);
//         log_info(LogTest, "  --tx_queue_start_addr: TX queue start address, default = 0x{:x}",
//         default_tx_queue_start_addr); log_info(LogTest, "  --tx_queue_size_bytes: TX queue size in bytes, default =
//         0x{:x}", default_tx_queue_size_bytes); log_info(LogTest, "  --rx_queue_start_addr: RX queue start address,
//         default = 0x{:x}", default_rx_queue_start_addr); log_info(
//             LogTest, "  --rx_queue_size_bytes: RX queue size in bytes, default = 0x{:x}",
//             default_rx_queue_size_bytes);
//         log_info(LogTest, "  --test_results_addr: test results buf address, default = 0x{:x}",
//         default_test_results_addr); log_info(LogTest, "  --test_results_size: test results buf size, default =
//         0x{:x}", default_test_results_size); log_info(LogTest, "  --timeout_mcycles: Timeout in MCycles, default =
//         {}", default_timeout_mcycles); log_info(LogTest, "  --check_txrx_timeout: Check if timeout happens during tx
//         & rx (if enabled, timeout_mcycles will also be used), default = {}", default_check_txrx_timeout);
//         log_info(LogTest, "  --rx_disable_data_check: Disable data check on RX, default = {}",
//         default_rx_disable_data_check); log_info(LogTest, "  --rx_disable_header_check: Disable header check on RX,
//         default = {}", default_rx_disable_header_check); log_info(LogTest, "  --tx_skip_pkt_content_gen: Skip packet
//         content generation during tx, default = {}", false); log_info(LogTest, "  --tx_pkt_dest_size_choice: choice
//         for how packet destination and packet size are generated, default = {}", default_tx_pkt_dest_size_choice); //
//         pkt_dest_size_choices_t log_info(LogTest, "  --tx_data_sent_per_iter_low: the criteria to determine the
//         amount of tx data sent per iter is low (unit: words); if both 0, then disable counting it in tx kernel,
//         default = {}", default_tx_data_sent_per_iter_low); log_info(LogTest, "  --tx_data_sent_per_iter_high: the
//         criteria to determine the amount of tx data sent per iter is high (unit: words); if both 0, then disable
//         counting it in tx kernel, default = {}", default_tx_data_sent_per_iter_high); log_info(LogTest, "
//         --dump_stat_json: Dump stats in json to output_dir, default = {}", default_dump_stat_json); log_info(LogTest,
//         "  --output_dir: Output directory, default = {}", default_output_dir); log_info(
//             LogTest, "  --device_id: Device on which the test will be run, default = {}", default_test_device_id_l);
//         log_info(
//             LogTest, "  --device_id_r: Device on which the test will be run, default = {}",
//             default_test_device_id_r);
//         return 0;
//     }
...
//     uint32_t tx_queue_start_addr = test_args::get_command_option_uint32(input_args, "--tx_queue_start_addr",
//     default_tx_queue_start_addr); uint32_t tx_queue_size_bytes = test_args::get_command_option_uint32(input_args,
//     "--tx_queue_size_bytes", default_tx_queue_size_bytes); uint32_t rx_queue_start_addr =
//     test_args::get_command_option_uint32(input_args, "--rx_queue_start_addr", default_rx_queue_start_addr); uint32_t
//     rx_queue_size_bytes =
//         test_args::get_command_option_uint32(input_args, "--rx_queue_size_bytes", default_rx_queue_size_bytes);
//     uint32_t tunneler_queue_size_bytes = test_args::get_command_option_uint32(input_args,
//     "--tunneler_queue_size_bytes", default_tunneler_queue_size_bytes); uint32_t test_results_addr =
//     test_args::get_command_option_uint32(input_args, "--test_results_addr", default_test_results_addr); uint32_t
//     test_results_size = test_args::get_command_option_uint32(input_args, "--test_results_size",
//     default_test_results_size); uint32_t tunneler_test_results_addr =
//     test_args::get_command_option_uint32(input_args, "--tunneler_test_results_addr",
//     default_tunneler_test_results_addr); uint32_t tunneler_test_results_size =
//     test_args::get_command_option_uint32(input_args, "--tunneler_test_results_size",
//     default_tunneler_test_results_size); uint32_t timeout_mcycles = test_args::get_command_option_uint32(input_args,
//     "--timeout_mcycles", default_timeout_mcycles); uint32_t rx_disable_data_check =
//     test_args::get_command_option_uint32(input_args, "--rx_disable_data_check", default_rx_disable_data_check);
//     uint32_t rx_disable_header_check = test_args::get_command_option_uint32(input_args, "--rx_disable_header_check",
//     default_rx_disable_header_check); bool tx_skip_pkt_content_gen = test_args::has_command_option(input_args,
//     "--tx_skip_pkt_content_gen"); uint32_t dump_stat_json = test_args::get_command_option_uint32(input_args,
//     "--dump_stat_json", default_dump_stat_json); std::string output_dir = test_args::get_command_option(input_args,
//     "--output_dir", std::string(default_output_dir)); uint32_t check_txrx_timeout =
//     test_args::get_command_option_uint32(input_args, "--check_txrx_timeout", default_check_txrx_timeout); uint32_t
//     tx_pkt_dest_size_choice = (uint8_t)test_args::get_command_option_uint32(
//         input_args, "--tx_pkt_dest_size_choice", default_tx_pkt_dest_size_choice);
//     uint32_t tx_data_sent_per_iter_low = test_args::get_command_option_uint32(input_args,
//     "--tx_data_sent_per_iter_low", default_tx_data_sent_per_iter_low); uint32_t tx_data_sent_per_iter_high =
//     test_args::get_command_option_uint32(input_args, "--tx_data_sent_per_iter_high",
//     default_tx_data_sent_per_iter_high); uint32_t fabric_command =
...
//     mcast_depth[RoutingDirection::E] = test_args::get_command_option_uint32(input_args, "--e_depth",
//     default_multicast); mcast_depth[RoutingDirection::W] = test_args::get_command_option_uint32(input_args,
//     "--w_depth", default_multicast); mcast_depth[RoutingDirection::N] =
//     test_args::get_command_option_uint32(input_args, "--n_depth", default_multicast);
//     mcast_depth[RoutingDirection::S] = test_args::get_command_option_uint32(input_args, "--s_depth",
//     default_multicast); bool mcast = false; for (const auto& [dir, depth] : mcast_depth) {
//         if (depth) {
...
//     // assert((pkt_dest_size_choices_t)tx_pkt_dest_size_choice ==
...
//     std::string board_type = test_args::get_command_option(input_args, "--board_type",
//     std::string(default_board_type));
