...
int main(int argc, char **argv) {
    ...
    try {
        ...
        // Q input
        auto c_in0_config = CircularBufferConfig(single_tile_size, {{CB::c_in0, tt::DataFormat::Float16_b}}).set_page_size(CB::c_in0, single_tile_size);
        auto cb_in0_id = CreateCircularBuffer(program, cores, c_in0_config);

        // K input
        auto c_in1_config = CircularBufferConfig(single_tile_size, {{CB::c_in1, tt::DataFormat::Float16_b}}).set_page_size(CB::c_in1, single_tile_size);
        auto cb_in1_id = CreateCircularBuffer(program, cores, c_in1_config);

        // V input
        auto c_in2_config = CircularBufferConfig(single_tile_size, {{CB::c_in2, tt::DataFormat::Float16_b}}).set_page_size(CB::c_in2, single_tile_size);
        auto cb_in2_id = CreateCircularBuffer(program, cores, c_in2_config);

        // identity scale input
        auto c_in5_config = CircularBufferConfig(single_tile_size, {{CB::c_in5, tt::DataFormat::Float16_b}}).set_page_size(CB::c_in5, single_tile_size);
        auto cb_in5_id = CreateCircularBuffer(program, cores, c_in5_config);

        // cb_qk_im
        auto c_intermed0_config = CircularBufferConfig(single_tile_size, {{CB::c_intermed0, tt::DataFormat::Float16_b}}).set_page_size(CB::c_intermed0, single_tile_size);
        auto cb_intermed0_id = CreateCircularBuffer(program, cores, c_intermed0_config);

        // cb_cur_sum
        auto c_intermed5_config = CircularBufferConfig(single_tile_size, {{CB::c_intermed5, tt::DataFormat::Float16_b}}).set_page_size(CB::c_intermed5, single_tile_size);
        auto cb_intermed5_id = CreateCircularBuffer(program, cores, c_intermed5_config);

        // Output
        auto c_out0_config = CircularBufferConfig(single_tile_size, {{CB::c_out0, tt::DataFormat::Float16_b}}).set_page_size(CB::c_out0, single_tile_size);
        ...
        auto reader_kernels_id = CreateKernel(
            program, "tt_eager/tt_dnn/op_library/sdpa/kernels/dataflow/reader_interleaved.cpp", cores,
        ...
        auto writer_kernels_id = CreateKernel(
            program, "tt_eager/tt_dnn/op_library/sdpa/kernels/dataflow/writer_interleaved.cpp", cores,
        ...
        auto compute_kernels_id = CreateKernel(
            program, "tt_eager/tt_dnn/op_library/sdpa/kernels/compute/sdpa.cpp", cores,
