...
bool find_device_with_neighbor_in_multi_direction(
    ...
    for (auto* device : devices) {
        ...
        for (auto [routing_direction, num_hops] : mcast_hops) {
            ...
            FabricNodeId curr_fabric_node_id = src_fabric_node_id;
            for (uint32_t i = 0; i < num_hops; i++) {
                auto neighbors = control_plane->get_intra_chip_neighbors(curr_fabric_node_id, routing_direction);
                if (neighbors.size() > 0) {
                    temp_end_fabric_node_ids.emplace_back(FabricNodeId(curr_fabric_node_id.mesh_id, neighbors[0]));
...
void RunTestLineMcast(
    ...
    auto routers = control_plane->get_routers_to_chip(sender_id, mcast_start_id);
    if (routers.size() == 0) {
        log_info(
            tt::LogTest,
...
void RunTestUnicastRaw(BaseFabricFixture* fixture, uint32_t num_hops, RoutingDirection direction) {
    ...
    if (!is_2d_fabric) {
        ...
        auto routers = control_plane->get_routers_to_chip(src_fabric_node_id, dst_fabric_node_id);
        if (routers.size() == 0) {
            log_info(
                tt::LogTest,
                "No fabric routers between Src MeshId {} ChipId {} - Dst MeshId {} ChipId {}",
