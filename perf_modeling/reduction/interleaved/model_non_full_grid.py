...
#   - Primary parameters are tile/core, core count and output storage
...
if __name__ == "__main__":
    ...
    # test_data = data[data['CORE COUNT'] <= 16]
    # data = data[data['CORE COUNT'] > 16]

    for keys, group in data.groupby(["INPUT STORAGE", "INPUT_0_DATATYPE", "DIM"]):
        df = group[["TILE / CORE", "CORE COUNT", "OUTPUT STORAGE", "DEVICE KERNEL DURATION [ns]"]].copy()
        ...
        # # optional model evaluation on data with core count less than 16
        # tst_X = test_data[(test_data['INPUT STORAGE'] == keys[0]) & (test_data['INPUT_0_DATATYPE'] == keys[1]) & (test_data['DIM'] == keys[2])]
        # tst_X = tst_X[['TILE / CORE', 'CORE COUNT', 'OUTPUT STORAGE','DEVICE KERNEL DURATION [ns]']].copy()
