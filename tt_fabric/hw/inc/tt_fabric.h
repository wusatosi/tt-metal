    ...
    inline uint32_t forward_data_from_fvc_buffer() {
        ...
        if constexpr (live == true) {
            ...
            while (words_remaining) {
                ...
                DPRINT << "forward_data_from_fvc_buffer(live): sending " << words_to_forward << " words" << ENDL();
            ...
            DPRINT << "forward_data_from_fvc_buffer(live): final sync word sent, total=" << total_words_to_forward
                   << ENDL();
            ...
            DPRINT << "forward_data_from_fvc_buffer(!live): forwarded " << total_words_to_forward << " words offline"
                   << ENDL();
        }
...
typedef struct fvc_producer_state {
    ...
    inline void advance_next_packet() {
        if (this->get_num_words_available() >= PACKET_HEADER_SIZE_WORDS) {
            ...
            if (tt_fabric_is_header_valid(&current_packet_header)) {
                ...
                DPRINT << "advance_next_packet: got a valid header" << ENDL();
            } else {
                this->packet_corrupted = true;
                DPRINT << "advance_next_packet: header invalid, marking packet_corrupted" << ENDL();
            }
        }
    }
    ...
    inline uint32_t pull_data_from_fvc_buffer() {
        ...
        if (packet_in_progress == 0) {
            ...
            if (current_packet_header.routing.flags == INLINE_FORWARD) {
                ...
                DPRINT << "pull_data_from_fvc_buffer: (INLINE_FORWARD) copied header, words_available="
                       << words_available << ENDL();
            } else {
                ...
                DPRINT << "pull_data_from_fvc_buffer: (FORWARD) setting up local_pull_request with wr_ptr="
                       << fvc_out_wrptr << " rd_ptr=" << fvc_out_rdptr << ENDL();
            }
            ...
            DPRINT << "pull_data_from_fvc_buffer: continuing packet_in_progress, words_available=" << words_available
                   << ENDL();
            ...
            if (packet_words_remaining) {
                if (words_available) {
                    ...
                    DPRINT << "pull_data_from_fvc_buffer: forwarded " << words_available << " more words" << ENDL();
                }
            } else if (fvc_pull_rdptr == fvc_out_rdptr) {
                // all data has been pulled and cleared from local buffer
                packet_in_progress = 0;
                curr_packet_valid = false;
                DPRINT << "pull_data_from_fvc_buffer: all data cleared, packet_in_progress done." << ENDL();
            }
        }
        ...
        if (packet_is_for_local_chip()) {
            if (current_packet_header.routing.flags == FORWARD && current_packet_header.session.command == ASYNC_WR) {
                if (packet_in_progress == 0) {
                    ...
                    DPRINT << "process_inbound_packet(ASYNC_WR): start writing data, removed header words="
                           << PACKET_HEADER_SIZE_WORDS << ENDL();
                    ...
                    if (packet_words_remaining) {
                        ...
                        DPRINT << "process_inbound_packet(ASYNC_WR): packet completed, no more words" << ENDL();
                    }
                }
                ...
                DPRINT << "process_inbound_packet(INLINE_FORWARD): atomic increment done, used "
                       << PACKET_HEADER_SIZE_WORDS << " header words" << ENDL();
            }
        } else {
            // multicast
            if (current_packet_header.routing.flags & MCAST_DATA) {
                ...
                DPRINT << "process_inbound_packet(MCAST_DATA): processed " << words_processed << " words" << ENDL();
            } else {
                // unicast
                words_processed = pull_data_from_fvc_buffer<fvc_mode>();
                DPRINT << "process_inbound_packet(unicast): processed " << words_processed << " words" << ENDL();
            }
        }
    ...
    inline uint32_t process_inbound_multicast_packet() {
        ...
        if (packet_is_for_local_chip()) {
            if (current_packet_header.routing.flags == FORWARD && current_packet_header.session.command == ASYNC_WR) {
                if (packet_in_progress == 0) {
                    ...
                    DPRINT << "process_inbound_multicast_packet(ASYNC_WR): removing header words="
                           << PACKET_HEADER_SIZE_WORDS << ENDL();
                    ...
                    if (packet_words_remaining) {
                        ...
                        DPRINT << "process_inbound_multicast_packet(ASYNC_WR): done writing packet" << ENDL();
                    }
                }
            } else if (current_packet_header.routing.flags == INLINE_FORWARD) {
                uint64_t noc_addr = ((uint64_t)current_packet_header.session.target_offset_h << 32) |
                ...
                DPRINT << "process_inbound_multicast_packet(INLINE_FORWARD): local atomic inc done." << ENDL();
            }
        }
        ...
        if (!need_to_send_east && !need_to_send_west) {
            ...
            DPRINT << "process_inbound_multicast_packet: no further east/west hops, local consumption +"
                   << words_processed << " words" << ENDL();
        ...
        DPRINT << "process_inbound_multicast_packet: " << pr_count
               << " requests sent east/west, total words processed now=" << words_processed << ENDL();
    ...
    inline void flush_async_writes() {
        ...
        DPRINT << "flush_async_writes: set fvc_pull_rdptr=" << fvc_pull_rdptr << ENDL();
    }
...
inline uint32_t pull_data_to_fvc_buffer(
    ...
    if (num_words_to_pull == 0) {
        ...
        DPRINT << "pull_data_to_fvc_buffer: no words to pull from remote (0). skipping" << ENDL();
    ...
    DPRINT << "pull_data_to_fvc_buffer: pulled " << num_words_to_pull << " words" << ENDL();
...
inline uint32_t move_data_to_fvc_buffer(
    ...
    if (fvc_consumer_state->get_num_words_free() < PACKET_HEADER_SIZE_WORDS) {
        DPRINT << "move_data_to_fvc_buffer: not enough space in consumer to move data" << ENDL();
        return 0;
    }
 ...
 */
bool wait_all_src_dest_ready(volatile router_state_t* router_state, uint32_t timeout_cycles) {
...
inline uint64_t tt_fabric_send_pull_request(uint64_t dest_addr, volatile local_pull_request_t* local_pull_request) {
    ...
    DPRINT << "tt_fabric_send_pull_request: queued new pull request at wr_index=" << dest_wr_index << ENDL();
...
inline void tt_fabric_init() {
    ...
    DPRINT << "tt_fabric_init: local xy_local_addr=0x" << (void*)xy_local_addr << ENDL();
}
