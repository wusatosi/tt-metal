...
void calculate_sum_x_squared() {
    ckernel::cb_wait_front(cb_input, num_inner);
    ckernel::cb_reserve_back(cb_rms_before_reduction_intermediate, onetile);
    ...
    for (uint32_t col = 0; col < num_inner; ++col) {
        ...
        ckernel:: copy_tile(cb_input, /* tile_idx */ col, /* register_idx */ working_register);
        ...
        if constexpr (do_mask_w) {
            if (col + 1 == num_inner) {
                ...
                ckernel:: copy_tile(cb_mask, /* tile_idx */ 0, /* register idx */ mask_register);
    ...
    ckernel:: pack_tile(accum_register, cb_rms_before_reduction_intermediate);

    ckernel::tile_regs_release();
    ckernel::cb_push_back(cb_rms_before_reduction_intermediate, onetile);
}
...
void calculate_sum_x_squared() {
    ...
    for (uint32_t col = 0; col < num_inner;) {
        ckernel::cb_wait_front(cb_input, block_size);
        for (uint32_t block_idx = 0; block_idx < block_size; ++block_idx, ++col) {
            auto working_register = col == 0 ? accum_register : tile_register;
            copy_tile_init(cb_input);
            ckernel:: copy_tile(cb_input, /* tile_idx */ block_idx, /* register_idx */ working_register);
        ...
        ckernel::cb_pop_front(cb_input, block_size);
    }
...
void calculate_input_multiplied_by_gamma_and_divided_by_rms() {
    ckernel::cb_wait_front(cb_gamma, num_inner);
    ckernel::cb_wait_front(cb_inverse_rms_after_reduction_intermediate, onetile);

    constexpr uint32_t mask_register = block_size;
    for (uint32_t col = 0; col < num_inner; col += block_size) {
        ckernel::cb_reserve_back(cb_output, block_size);
        ckernel::cb_reserve_back(cb_output_intermediate, block_size);
        ...
        for (uint32_t block_idx = 0; block_idx < block_size; ++block_idx) {
            ckernel:: pack_tile(block_idx, cb_output_intermediate);
        }
        ckernel::cb_push_back(cb_output_intermediate, block_size);
        ckernel::tile_regs_release();

        ckernel::cb_wait_front(cb_output_intermediate, block_size);
        ckernel:: tile_regs_acquire();
        ...
        for (uint32_t block_idx = 0; block_idx < block_size; ++block_idx) {
            ckernel:: pack_tile(block_idx, cb_output);
        }

        ckernel::tile_regs_release();
        ckernel::cb_push_back(cb_output, block_size);
        ckernel::cb_pop_front(cb_output_intermediate, block_size);
    }

    ckernel::cb_pop_front(cb_input, num_inner);
    ckernel::cb_pop_front(cb_inverse_rms_after_reduction_intermediate, onetile);
}
...
void MAIN {
    ...
    for (uint32_t row = 0; row < num_rows_per_core; ++row) {
        ...
        // reduce and sqrt
        {
            ckernel::cb_wait_front(cb_rms_before_reduction_intermediate, onetile);
            ckernel::cb_reserve_back(cb_rms_after_reduction_intermediate, onetile);
            ckernel:: tile_regs_acquire();
            ...
            ckernel::reduce_tile<PoolType::SUM, ReduceDim::REDUCE_ROW>(
                cb_rms_before_reduction_intermediate,
                cb_scaler,
                /* tile_idx */ 0,
                /* tile_idx */ 0,
                reduction_register);
            ckernel::reduce_revert_delta<ReduceDim::REDUCE_ROW>(cb_rms_before_reduction_intermediate);

            const uint32_t eps_register = 1U;
            reconfig_data_format_srca(cb_eps);
            copy_tile_init(cb_eps);
            ckernel:: copy_tile(cb_eps, /* tile_idx */ 0, /* register_idx */ eps_register);
            ...
            sqrt_tile_init();
            ckernel:: sqrt_tile(reduction_register);

            ckernel:: tile_regs_commit();
            ckernel::tile_regs_wait();

            pack_reconfig_data_format(cb_rms_after_reduction_intermediate);
            ckernel:: pack_tile(reduction_register, cb_rms_after_reduction_intermediate);

            ckernel::tile_regs_release();
            ckernel::cb_push_back(cb_rms_after_reduction_intermediate, onetile);
            ckernel::cb_pop_front(cb_rms_before_reduction_intermediate, onetile);
        }
        ...
        if constexpr (return_rms) {
            ...
            ckernel::cb_wait_front(cb_rms_after_reduction_intermediate, onetile);
            ckernel:: tile_regs_acquire();
            ckernel::cb_reserve_back(cb_rms_output, onetile);
            copy_tile_init(cb_rms_after_reduction_intermediate);
            ckernel:: copy_tile(cb_rms_after_reduction_intermediate, /* tile idx */ 0, temporary_register);
            ckernel:: tile_regs_commit();

            ckernel::tile_regs_wait();
            pack_reconfig_data_format(cb_rms_output);
            ckernel:: pack_tile(temporary_register, cb_rms_output);
            ckernel::tile_regs_release();
            ckernel::cb_push_back(cb_rms_output, onetile);
        }
        ...
        // reciprocal of rms intermediate
        {
            const uint32_t temporary_register = 0;
            ckernel::cb_wait_front(cb_rms_after_reduction_intermediate, onetile);
            ckernel::cb_reserve_back(cb_inverse_rms_after_reduction_intermediate, onetile);
            ckernel:: tile_regs_acquire();
            ...
            ckernel:: pack_tile(temporary_register, cb_inverse_rms_after_reduction_intermediate);

            ckernel::tile_regs_release();
            ckernel::cb_push_back(cb_inverse_rms_after_reduction_intermediate, onetile);
            ckernel::cb_pop_front(cb_rms_after_reduction_intermediate, onetile);
        }
...
#ifdef EVERYTHING_FITS_IN_L1
    ckernel::cb_pop_front(cb_gamma, num_inner);
#endif
    ckernel::cb_pop_front(cb_scaler, onetile);
    ckernel::cb_pop_front(cb_eps, onetile);

    if constexpr (do_mask_w) {
        ckernel::cb_pop_front(cb_mask, onetile);
    }
}
