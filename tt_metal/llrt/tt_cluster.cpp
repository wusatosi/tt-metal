...
void Cluster::initialize_blackhole_eth_connectivity() {
    ...
    // from https://tenstorrent.sharepoint.com/:x:/r/sites/SOC/Blackhole%20Documents/Blackhole%20-%20NOC%20Co-ordinates.xlsx?d=w449397eff6fc48abaed13762398c30dd&csf=1&web=1&e=bKt38y
    static constexpr std::array<uint32_t, 14> eth_id_to_noc0_x_mapping = {1, 16, 2, 15, 3, 14, 4, 13, 5, 12, 6, 11, 7, 10};
        ...
        for (const CoreCoord &physical_eth_core : physical_eth_cores) {
            ...
            while (port_status == 0) {
                ...
                if (elapsed > train_timeout_ms) {
                    TT_THROW("Device {}: Timeout ({} ms) waiting for logical ethernet core {} to finish training", chip_id, train_timeout_ms, logical_eth.str());
                }
                read_core(&port_status, sizeof(uint32_t), tt_cxy_pair(chip_id, physical_eth_core), eth_status_addr);
            }

            std::cout << "physical eth core " << physical_eth_core.str()
                      << " logical " << logical_eth.str()
            ...
            if (port_status == 1 or port_status == 2) {
                ...
                if (port_status == 1) {
                    ...
                    read_core(local_chip_info.data(), chip_infot_num_dwords * sizeof(uint32_t), tt_cxy_pair(chip_id, physical_eth_core), local_info_base_addr);
                    ...
                    std::cout << "\tasic location " << (uint32_t)asic_location << " board id " << std::hex << local_board_id << std::dec << std::endl;

                    this->chip_to_location[chip_id] = chip_identifier_t{.chip_location = asic_location, .board_id = local_board_id};
                }
            }
        }
    }
    ...
    for (const auto &chip_id : all_chips) {
        ...
        for (const CoreCoord &physical_eth_core : soc_desc.physical_ethernet_cores) {
            ...
            if (is_active_and_trained) {
                ...
                std::cout << "\tremote eth id " << (uint32_t)remote_eth_id
                          << " remote asic location " << (uint32_t)remote_asic_location
                          << " remote board id " << std::hex << remote_board_id << std::dec << std::endl;
                ...
                TT_FATAL(connected_chip_id != this->chip_to_location.size(),
                    "Expected to find connected chip for logical eth core {} on chip {}", logical_eth.str(), chip_id);
                ...
                CoreCoord remote_logical_eth = remote_soc_desc.get_logical_ethernet_core_from_physical(remote_physical_eth_core);
                ...
                std::cout << "\tremote remote_physical_eth_core " << remote_physical_eth_core.str()
                          << " remote_logical_eth " << remote_logical_eth.str()
                          << " remote eth channel " << remote_eth_channel << std::endl;
    ...
    for (chip_id_t c_id = 0; c_id < chip_to_location.size(); c_id++) {
        ...
        std::cout << "\tBoard id " << std::hex << this->chip_to_location[c_id].board_id << std::dec << " location " << (uint32_t)this->chip_to_location[c_id].chip_location << std::endl;
        ...
        for (const auto &active_eth_core : this->chip_to_logical_active_eths[c_id]) {
            ...
            if (connected_id_and_eth.has_value()) { // Eth cores that failed training will not have any connections
                ...
                auto remote_logical_eth = this->get_soc_desc(remote_chip).chan_to_logical_eth_core_map.at(remote_channel);
                std::cout << "\t\tconnected to chip " << remote_chip
                        << " at remote channel " << remote_channel
...
std::unordered_map<chip_id_t, std::vector<CoreCoord>> Cluster::get_ethernet_cores_grouped_by_connected_chips(
    ...
    if (arch_ == ARCH::BLACKHOLE) {
        ...
        for (uint32_t local_chip_chan = 0; local_chip_chan < local_eth_channels_connectivity.size(); local_chip_chan++) {
...
std::unordered_set<CoreCoord> Cluster::get_active_ethernet_cores(
    ...
    if (this->arch_ == ARCH::BLACKHOLE) {
        active_ethernet_cores.insert(this->chip_to_logical_active_eths[chip_id].begin(), this->chip_to_logical_active_eths[chip_id].end());
    } else {
        const auto &connected_chips = this->get_ethernet_cores_grouped_by_connected_chips(chip_id);
        for (const auto &[other_chip_id, eth_cores] : connected_chips) {
            for (const auto &eth_core : eth_cores) {
...
std::tuple<chip_id_t, CoreCoord> Cluster::get_connected_ethernet_core(std::tuple<chip_id_t, CoreCoord> eth_core) const {
    ...
    if (arch_ == ARCH::BLACKHOLE) {
        ...
        auto connected_chip_and_eth_channel = local_eth_channels_connectivity[local_eth_channel];
        TT_FATAL(connected_chip_and_eth_channel.has_value(),
            "Logical eth core {} is not an active eth core on chip {}", std::get<1>(eth_core).str(), std::get<0>(eth_core));
