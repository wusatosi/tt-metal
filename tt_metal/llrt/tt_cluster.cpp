...
void Cluster::detect_arch_and_target() {
    ...
    TT_FATAL(this->arch_ == tt::ARCH::Invalid, "Failed to get platform architecture");
...
const std::unordered_map<CoreCoord, int32_t>& Cluster::get_virtual_routing_to_profiler_flat_id(
...
void Cluster::open_driver(const bool& skip_driver_allocs) {
    ...
    barrier_params.tensix_l1_barrier_base =
        tt_metal::hal.get_dev_addr(tt_metal::HalProgrammableCoreType::TENSIX, tt_metal::HalL1MemAddrType::BARRIER);
    ...
    if (tt_metal::hal.get_arch() != tt::ARCH::GRAYSKULL) {
        barrier_params.eth_l1_barrier_base = tt_metal::hal.get_dev_addr(
        ...
        for (auto& core_desc : this->get_soc_desc(chip_id).physical_cores) {
            if (core_desc.second.type != CoreType::HARVESTED) {
                CoreCoord virtual_coords = this->get_virtual_coordinate_from_physical_coordinates(
                    chip_id, core_desc.first, core_desc.second.type);
                tt_cxy_pair virtual_core = tt_cxy_pair(chip_id, virtual_coords.x, virtual_coords.y);
                tt_cxy_pair umd_core = this->get_soc_desc(chip_id).convert_to_umd_coordinates(
                    tt_cxy_pair(chip_id, core_desc.first.x, core_desc.first.y));
...
void Cluster::generate_logical_to_virtual_coord_mapping() {
    for (auto chip_id : this->cluster_desc_->get_all_chips()) {
        ...
        for (auto x_coords : soc_desc.worker_log_to_routing_x) {
            CoreCoord phys_core =
                soc_desc.get_physical_core_from_logical_core(CoreCoord(x_coords.first, 0), CoreType::WORKER);
            CoreCoord virtual_coords =
                this->get_virtual_coordinate_from_physical_coordinates(chip_id, phys_core, CoreType::WORKER);
        ...
        for (auto y_coords : soc_desc.worker_log_to_routing_y) {
            CoreCoord phys_core =
                soc_desc.get_physical_core_from_logical_core(CoreCoord(0, y_coords.first), CoreType::WORKER);
            CoreCoord virtual_coords =
                this->get_virtual_coordinate_from_physical_coordinates(chip_id, phys_core, CoreType::WORKER);
            this->worker_logical_to_virtual_y_.at(board_type).insert({y_coords.first, virtual_coords.y});
        }
        for (std::size_t log_eth_core_y = 0; log_eth_core_y < soc_desc.physical_ethernet_cores.size();
             log_eth_core_y++) {
            CoreCoord logical_eth_core = {0, log_eth_core_y};
            CoreCoord virtual_coords = this->get_virtual_coordinate_from_physical_coordinates(
                chip_id, soc_desc.physical_ethernet_cores.at(log_eth_core_y), CoreType::ETH);
...
void Cluster::generate_virtual_to_profiler_flat_id_mapping() {
    ...
    for (auto chip_id : this->cluster_desc_->get_all_chips()) {
        ...
        for (const auto& core_to_profiler_id : soc_desc.physical_routing_to_profiler_flat_id) {
            if (std::find(
                    soc_desc.physical_workers.begin(), soc_desc.physical_workers.end(), core_to_profiler_id.first) !=
                soc_desc.physical_workers.end()) {
                this->virtual_routing_to_profiler_flat_id_.at(board_type)
                    .insert(
                        {this->get_virtual_coordinate_from_physical_coordinates(
                             chip_id, core_to_profiler_id.first, CoreType::WORKER),
                         core_to_profiler_id.second});
            } else {
                this->virtual_routing_to_profiler_flat_id_.at(board_type)
                    .insert(
                        {this->get_virtual_coordinate_from_physical_coordinates(
                             chip_id, core_to_profiler_id.first, CoreType::ETH),
                         core_to_profiler_id.second});
            }
        }
    }
...
CoreCoord Cluster::get_virtual_coordinate_from_logical_coordinates(
    ...
    if (core_type == CoreType::WORKER) {
        return CoreCoord(
            this->worker_logical_to_virtual_x_.at(board_type).at(logical_coord.x),
            this->worker_logical_to_virtual_y_.at(board_type).at(logical_coord.y));
...
CoreCoord Cluster::get_virtual_coordinate_from_physical_coordinates(
    chip_id_t chip_id, CoreCoord physical_coord, const CoreType& core_type) const {
    auto& soc_desc = this->get_soc_desc(chip_id);
    if ((not(core_type == CoreType::WORKER or core_type == CoreType::ETH)) or
        this->target_type_ == TargetDevice::Simulator) {
        return physical_coord;
    }
    tt_cxy_pair virtual_chip_coord =
        soc_desc.convert_to_umd_coordinates(tt_cxy_pair(chip_id, physical_coord.x, physical_coord.y));
