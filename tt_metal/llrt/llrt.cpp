...
static bool check_if_riscs_on_specified_core_done(chip_id_t chip_id, const CoreCoord &core, int run_state) {
    ...
    if (dispatch_core_type == tt_metal::HalProgrammableCoreType::ACTIVE_ETH) {
        ...
        tt::Cluster::instance().read_core(&l1_unreserved_base_val, sizeof(uint32_t), tt_cxy_pair(chip_id, core), eth_l1_mem::address_map::ERISC_L1_UNRESERVED_BASE);
        // tt::Cluster::instance().read_reg(&pc_addr, tt_cxy_pair(chip_id, core), 0xFFB14008);
        // std::cout << "Eth core " << core.str() << " l1 unreserved base " << std::hex << l1_unreserved_base_val << std::dec << std::endl;

        // uint32_t reset_val;
        // tt::Cluster::instance().read_reg(&reset_val, tt_cxy_pair(chip_id, core), 0xFFB121B0);
        // std::cout << "Active eth core " << core.str() << " in reset_cores reset val " << std::hex << reset_val << std::dec << std::endl;
    }
