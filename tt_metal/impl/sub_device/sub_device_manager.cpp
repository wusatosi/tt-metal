...
void SubDeviceManager::populate_noc_data() {
    ...
    for (uint32_t i = 0; i < num_sub_devices; ++i) {
        ...
        for (const auto& core_range : tensix_cores.ranges()) {
            ...
            this->noc_mcast_unicast_data_[idx++] = this->device_->get_translated_noc_multicast_encoding(noc_index, translated_core_range);
        ...
        for (const auto& core_range : eth_cores.ranges()) {
            ...
            for (const auto& core : core_range) {
                ...
                this->noc_mcast_unicast_data_[idx++] = this->device_->get_translated_noc_unicast_encoding(noc_index, translated_core);
            }
        }
