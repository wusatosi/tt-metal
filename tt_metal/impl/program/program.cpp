...
class Program_ {
   ...
   private:
    ...
    CBHandle add_circular_buffer_(const CoreRangeSet &core_range_set, const std::shared_ptr<CircularBuffer> &circular_buffer);
    CBHandle add_circular_buffer(const CoreRangeSet &core_range_set, const CircularBufferConfig &config);
    CBHandle add_circular_buffer(const CoreRangeSet &core_range_set, const CircularBufferConfig &config, const experimental::GlobalCircularBuffer &global_circular_buffer);
                ...
                for (auto y = range.start_coord.y; y <= range.end_coord.y; y++) {
                    for (auto x = range.start_coord.x; x <= range.end_coord.x; x++) {
                        ...
                        if (local_val != per_core_local_cb_indices_.end()) {
                            max_local_cb_end_index = std::max(max_local_cb_end_index, NUM_CIRCULAR_BUFFERS - (uint32_t)__builtin_clz(local_val->second.to_ulong()));
                        }
                        auto remote_val = per_core_remote_cb_indices_.find(core);
                        if (remote_val != per_core_remote_cb_indices_.end()) {
                            min_remote_cb_start_index = std::min(min_remote_cb_start_index, (uint32_t)__builtin_ctz(remote_val->second.to_ulong()));
                        }
                    }
                }
            }
...
CBHandle detail::Program_::add_circular_buffer_(const CoreRangeSet &core_range_set, const std::shared_ptr<CircularBuffer> &circular_buffer) {
    ...
    for (const CoreRange &core_range : core_range_set.ranges()) {
        for (auto x = core_range.start_coord.x; x <= core_range.end_coord.x; x++) {
            for (auto y = core_range.start_coord.y; y <= core_range.end_coord.y; y++) {
                ...
                auto add_buffer_indices = [&cb_indices](const std::unordered_set<uint8_t> &buffer_indices, std::bitset<NUM_CIRCULAR_BUFFERS> &target_cb_indices) {
                    for (uint32_t buffer_index : buffer_indices) {
                        // TT_ASSERT since we validate when constructing the config that it's within range
                        TT_ASSERT(buffer_index < NUM_CIRCULAR_BUFFERS, "Invalid circular buffer index: {} should be between 0 and {}", buffer_index, NUM_CIRCULAR_BUFFERS);
...
CBHandle detail::Program_::add_circular_buffer(const CoreRangeSet &core_range_set, const CircularBufferConfig &config, const experimental::GlobalCircularBuffer &global_circular_buffer) {
    TT_FATAL(this->compiled_.empty(), "Cannot add circular buffer to an already compiled program {}", this->id);
    std::shared_ptr<CircularBuffer> circular_buffer = std::make_shared<CircularBuffer>(core_range_set, config, global_circular_buffer);
    return add_circular_buffer_(core_range_set, circular_buffer);
}
...
CBHandle Program::add_circular_buffer(const CoreRangeSet &core_range_set, const CircularBufferConfig &config, const experimental::GlobalCircularBuffer &global_circular_buffer) {
...
void detail::Program_::set_remote_circular_buffer_init(Device *device, const std::shared_ptr<Kernel> &kernel) const {
    ...
    for (const auto &str : reserved_defines) {
        TT_FATAL(kernel_defines.find(str) == kernel_defines.end(), "{} is a reserved define and can't be manually set", str);
    }
    ...
    for (auto logical_cr : kernel->logical_coreranges()) {
        ...
        for (const auto &circular_buffer : cbs_on_core) {
            ...
            if (!circular_buffer->local_buffer_indices().empty()) {
                align_code += fmt::format("align_local_cbs_to_remote_cb<{}>({},{{", circular_buffer->local_buffer_indices().size(), remote_cb_index);
    ...
    if (!remote_cb_indices.empty()) {
        ...
        std::string update_code = fmt::format("update_remote_cb_configs_in_l1<{}>({{", remote_cb_indices.size());
...
uint32_t detail::Program_::finalize_cbs(uint32_t programmable_core_type_index, uint32_t base_offset) {
    ...
    for (auto& kg : kernel_groups) {
        max_local_end_index = std::max(max_local_end_index, (uint32_t)kg.launch_msg.kernel_config.max_local_cb_end_index);
        min_remote_start_index = std::min(min_remote_start_index, (uint32_t)kg.launch_msg.kernel_config.min_remote_cb_start_index);
    }
    ...
    uint32_t remote_cb_size = (NUM_CIRCULAR_BUFFERS - min_remote_start_index) * UINT32_WORDS_PER_REMOTE_CIRCULAR_BUFFER_CONFIG * sizeof(uint32_t);
