                ...
                if (kernel_id) {
                    ...
                    for (auto& [transfer_info, dests] :
                        ...
                        AddToDispatchLog(
                            &program,
                            fmt::format(
                                "  Queueing up CRTAs size {} to core range {} - {} cores",
                                size,
                                core_range,
                                core_range.size()));
                    }
...
void assemble_device_commands(
    ...
    AddToDispatchLog(
        &program,
        fmt::format("  Ringbuffer sizes: worker {}, ethernet {}", program_config_sizes[0], program_config_sizes[1]));
            ...
            for (const auto& transfer_info : transfer_info_vec) {
                for (const auto& dst_noc_info : transfer_info.dst_noc_info) {
                    ...
                    AddToDispatchLog(
                        &program,
                        fmt::format(
                            "  Queueing up multicast semaphores size {} to {}",
    ...
    if (num_multicast_cb_sub_cmds > 0) {
        ...
        for (const CoreRange& core_range : circular_buffers_unique_coreranges) {
            ...
            AddToDispatchLog(
                &program,
                fmt::format("  Queueing up multicast CBs size {} to {}", max_index * sizeof(uint32_t), core_range));
...
void reserve_space_in_kernel_config_buffer(
    ...
    if (device_id == kDeviceToPrint) {
        fmt::println(stderr, "Enqueue program {} ptr {} on device {}", program_id, program_ptr, device_id);
    }
