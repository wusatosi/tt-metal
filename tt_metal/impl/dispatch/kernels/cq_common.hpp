...
FORCE_INLINE void cq_noc_async_write_with_state(
...
#endif
        WATCHER_RING_BUFFER_PUSH(0x30000000 | ((uint32_t)(dst_addr >> NOC_ADDR_COORD_SHIFT) & NOC_COORDINATE_MASK));
    ...
    if constexpr (flags & CQ_NOC_FLAG_LEN) {
        ...
        WATCHER_RING_BUFFER_PUSH(0x40000000 | size);
    ...
    if constexpr (send) {
        ...
        WATCHER_RING_BUFFER_PUSH(0x50000000);
...
FORCE_INLINE void cq_noc_async_write_init_state(uint32_t src_addr, uint64_t dst_addr, uint32_t size = 0) {
    ...
    WATCHER_RING_BUFFER_PUSH(0x60000000);
    while (!noc_cmd_buf_ready(noc_index, NCRISC_WR_CMD_BUF)) {
    ...
    constexpr uint32_t noc_cmd_field =
        NOC_CMD_CPY | NOC_CMD_WR | NOC_CMD_VC_STATIC | NOC_CMD_STATIC_VC(vc) | (false ? NOC_CMD_VC_LINKED : 0x0) |
        (mcast ? ((multicast_path_reserve ? NOC_CMD_PATH_RESERVE : 0) | NOC_CMD_BRCST_PACKET) : 0x0) |
        (posted ? 0 : NOC_CMD_RESP_MARKED);

    WATCHER_RING_BUFFER_PUSH(0x70000000 | noc_cmd_field);
