...
void EnqueueProgramCommand::assemble_stall_commands(ProgramCommandSequence& program_command_sequence, bool prefetch_stall) {
    if (prefetch_stall) {
        ...
        program_command_sequence.uncached_bin_stall_command_sequence =
            HostMemDeviceCommand(uncached_cmd_sequence_sizeB);
        ...
        program_command_sequence.uncached_bin_stall_command_sequence.add_dispatch_wait_with_prefetch_stall(
        ...
        program_command_sequence.cached_bin_stall_command_sequence =
            HostMemDeviceCommand(cached_cmd_sequence_sizeB);
        program_command_sequence.cached_bin_stall_command_sequence.add_dispatch_wait(
                    ...
                    for (auto x = core_range.start_coord.x; x <= core_range.end_coord.x; x++) {
                        for (auto y = core_range.start_coord.y; y <= core_range.end_coord.y; y++) {
                            ...
                            unique_sub_cmds.emplace_back(CQDispatchWritePackedUnicastSubCmd{
                                .noc_xy_addr = this->device->get_noc_unicast_encoding(this->noc_index, virtual_core)});
                        }
                    }
                }
                ...
                if (common_rt_args.size() > 0) {
                    ...
                    if (core_type == CoreType::ETH) {
                        ...
                        for (auto& core_coord : kernel->logical_cores()) {
                            ...
                            CoreCoord virtual_core_coords = device->virtual_core_from_logical_core(core_coord, CoreType::ETH);
                            unicast_sub_cmd.emplace_back(CQDispatchWritePackedUnicastSubCmd{
                                .noc_xy_addr = this->device->get_noc_unicast_encoding(this->noc_index, virtual_core_coords)});
                        }
                    } else {
...
void EnqueueProgramCommand::assemble_device_commands(
    ...
    if (num_multicast_cb_sub_cmds > 0) {
        ...
        for (const CoreRange& core_range : circular_buffers_unique_coreranges) {
            ...
            multicast_cb_config_sub_cmds.emplace_back(CQDispatchWritePackedMulticastSubCmd{
                .noc_xy_addr = this->device->get_noc_multicast_encoding(
                    this->noc_index, CoreRange(virtual_start, virtual_end)),
    ...
    for (KernelGroup& kernel_group : program.get_kernel_groups(programmable_core_index)) {
        ...
        for (const CoreRange& core_range : kernel_group.core_ranges.ranges()) {
            CoreCoord virtual_start = device->virtual_core_from_logical_core(core_range.start_coord, kernel_group.get_core_type());
            CoreCoord virtual_end = device->virtual_core_from_logical_core(core_range.end_coord, kernel_group.get_core_type());
...
void EnqueueProgramCommand::write_program_command_sequence(
    ...
    uint32_t stall_fetch_size_bytes =
        (stall_first || stall_before_program) ? program_command_sequence.stall_command_sequence_ptr->size_bytes() : 0;
    ...
    if (total_fetch_size_bytes <= dispatch_constants::get(this->dispatch_core_type).max_prefetch_command_size()) {
        ...
        if (stall_first) {
            ...
            this->manager.cq_write(
                program_command_sequence.stall_command_sequence_ptr->data(), stall_fetch_size_bytes, write_ptr);
...
void EnqueueProgramCommand::process() {
    ...
    if (!is_cached) {
        ...
        program_command_sequence.stall_command_sequence_ptr = &(program_command_sequence.uncached_bin_stall_command_sequence);
        ...
        program.set_cached();
        program.set_binaries_on_device_status(device->id(), ProgramBinaryStatus::Committed);
    } else {
        ...
        if (program.binaries_on_device_status(device->id()) != ProgramBinaryStatus::Committed) {
            cached_program_command_sequence.stall_command_sequence_ptr = &(cached_program_command_sequence.uncached_bin_stall_command_sequence);
            program.set_binaries_on_device_status(device->id(), ProgramBinaryStatus::Committed);
        } else {
            cached_program_command_sequence.stall_command_sequence_ptr = &(cached_program_command_sequence.cached_bin_stall_command_sequence);
        }

        cached_program_command_sequence.stall_command_sequence_ptr->update_cmd_sequence(
...
void HWCommandQueue::enqueue_program(Program& program, bool blocking) {
    ...
    if (program.binaries_on_device_status(device->id()) == ProgramBinaryStatus::NotSent) {
        ...
        program.set_binaries_on_device_status(device->id(), ProgramBinaryStatus::InFlight);
    }
