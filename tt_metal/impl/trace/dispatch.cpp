...
void issue_trace_commands(
    ...
    for (const auto& [id, desc] : dispatch_md.trace_worker_descriptors) {
        ...
        const auto& num_noc_unicast_txns =
            desc.num_traced_programs_needing_go_signal_unicast ? 3 /* device->num_noc_unicast_txns(id) */ : 0;
        auto index = *id;
    ...
    for (const auto& [id, desc] : dispatch_md.trace_worker_descriptors) {
        ...
        if (desc.num_traced_programs_needing_go_signal_unicast) {
            expected_num_workers += 3; /* device->num_worker_cores(HalProgrammableCoreType::ACTIVE_ETH, id); */
        }
