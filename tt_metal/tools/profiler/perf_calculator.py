...
def calculate_max_sum(data, input_columns):
    ...
    for i in range(len(data[input_columns[0]])):
        ret_val += max(float(data[column][i]) for column in input_columns)
    return ret_val
...
def main(filepath, batch, e2e_ratio):
    ...
    print(f"E2E ratio: {e2e_ratio}")
    print("-------------------------------------------------------------")
    print("Calculating samples per second for measured kernel time")
    ...
    print("-------------------Device measured time-------------------")
    el = ["DEVICE KERNEL DURATION [ns]"]
    matmul_data_kernel = calculate_max_sum(matmul_data, el)
    print(f"Measured matmul time {matmul_data_kernel}")
    conv_data_kernel = calculate_max_sum(conv_data, el)
    print(f"Measured conv time {conv_data_kernel}")
    ...
    print("-------------------Utilization ration-------------------")
    print(f"Matmul utilization ratio: {matmul_ideal_execution_time /matmul_data_kernel}")
    print(f"Conv utilization ratio: {conv_ideal_execution_time/conv_data_kernel}")

    print("-------------------Model calculations-------------------")
    cs_model = (
