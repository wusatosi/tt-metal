...
bool ThreadPool::WorkerQueue::empty() const { return head_ == tail_; }

ThreadPool::WorkerQueue::Node* ThreadPool::WorkerQueue::pop_head() {
    ThreadPool::WorkerQueue::Node* old_head = head_;
    if (old_head == tail_) {
        ...
        workers_.emplace_back([this] {
            while (true) {
                std::function<void()> task;  // Task container for this thread
                {
                    ...
                    this->cv.wait(lock, [this] { return not tasks_.empty() or shutdown_; });
