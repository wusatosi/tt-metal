...
void MeshCommandQueue::enqueue_write_shards(
    ...
    futures.reserve(shard_data_transfers.size());

    auto dispatch_lambda =
        std::make_shared<std::packaged_task<void(uint32_t)>>(
            [&shard_data_transfers, &buffer, this](uint32_t thread_idx) mutable {
                auto& shard_data_transfer = shard_data_transfers[thread_idx];
    ...
    for (std::size_t thread_idx = 0; thread_idx < shard_data_transfers.size(); thread_idx++) {
        std::packaged_task<void()> writer([dispatch_lambda, thread_idx]() mutable { (*dispatch_lambda)(thread_idx); });
        futures.push_back(writer.get_future());
        boost::asio::post(thread_pool_, [task = std::move(writer)]() mutable { task(); });
    }
    // Barrier
    ...
    float wait_duration =
        std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start)
            .count();
    total_write_time += wait_duration;
    num_writes++;
    if (blocking) {
        this->finish();
    }
...
void MeshCommandQueue::enqueue_read_shards(
    ...
    std::vector<std::future<void>> futures;
    futures.reserve(shard_data_transfers.size());
    auto dispatch_lambda = std::make_shared<std::packaged_task<void(uint32_t)>>(
        [&shard_data_transfers, &buffer, this](uint32_t thread_idx) mutable {
            auto& shard_data_transfer = shard_data_transfers[thread_idx];
            auto device_shard_view = buffer->get_device_buffer(shard_data_transfer.shard_coord);
    ...
    for (std::size_t thread_idx = 0; thread_idx < shard_data_transfers.size(); thread_idx++) {
        std::packaged_task<void()> reader([dispatch_lambda, thread_idx]() mutable { (*dispatch_lambda)(thread_idx); });
        futures.push_back(reader.get_future());
        boost::asio::post(thread_pool_, [task = std::move(reader)]() mutable { task(); });
    }
    // Barrier
    for (auto& future : futures) {
        future.wait();
...
void MeshCommandQueue::write_program_cmds_to_subgrid(
    ...
    uint32_t num_cols = sub_grid.end_coord().coords()[1] - sub_grid.start_coord().coords()[1] + 1;
    ...
    auto dispatch_lambda =
        std::make_shared<std::function<void(uint32_t)>>([&chip_ids_in_workload,
                                                         &program_cmd_seq,
                                                         num_chips_per_col,
                                                         dispatch_core_type,
                                                         stall_first,
    ...
    for (uint32_t thread_idx = 0; thread_idx < num_cols; thread_idx++) {
        std::packaged_task<void()> program_dispatcher(
            [dispatch_lambda, thread_idx]() mutable { (*dispatch_lambda)(thread_idx); });
        futures.push_back(program_dispatcher.get_future());
        boost::asio::post(thread_pool_, [task = std::move(program_dispatcher)]() mutable { task(); });
    }
    // Barrier
    for (auto& future : futures) {
        future.wait();
    ...
    float wait_duration =
        std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start)
            .count();
    if (num_workloads >= 60) {
        total_workload_time += wait_duration;
    }
    num_workloads++;
}
