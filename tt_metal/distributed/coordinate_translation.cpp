...
const MeshContainer<PhysicalMeshCoordinate>& get_system_mesh_coordinate_translation_map() {
    static tt::stl::Indestructible<MeshContainer<PhysicalMeshCoordinate>> kTranslationMap([]() {
        ...
        const auto global_mesh_shape = control_plane->cumulative_shape_across_fabric_meshes();
        std::cout << "Global " << global_mesh_shape << std::endl;
        ...
        physical_coordinates.reserve(global_mesh_shape.mesh_size());
        for (auto mesh_id : mesh_ids) {
            std::cout << "Mesh ID: " << mesh_id << std::endl;
            auto local_mesh_shape = control_plane->get_physical_mesh_shape(mesh_id);
            std::cout << "Local Mesh Shape: " << local_mesh_shape << std::endl;
            for (int logical_chip_id = 0; logical_chip_id < local_mesh_shape.mesh_size(); ++logical_chip_id) {
                ...
                TT_FATAL(
                    ...
                    physical_chip_id,
                    mesh_id);
                physical_coordinates.push_back(
                    PhysicalMeshCoordinate(/*mesh_id=*/mesh_id, /*chip_id=*/physical_chip_id));
            }
        }
        return MeshContainer<PhysicalMeshCoordinate>(global_mesh_shape, std::move(physical_coordinates));
    }());
