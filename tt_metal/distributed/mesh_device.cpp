...
std::shared_ptr<MeshTraceBuffer>& MeshDevice::create_mesh_trace(uint32_t tid) {
    auto [trace, emplaced] = trace_buffer_pool_.emplace(tid, MeshTrace::create_empty_mesh_trace_buffer());
    TT_FATAL(emplaced, "Trace buffer with tid {} already exists", tid);
    return trace->second;
}

void MeshDevice::release_mesh_trace(uint32_t tid) { trace_buffer_pool_.erase(tid); }

std::shared_ptr<MeshTraceBuffer> MeshDevice::get_mesh_trace(uint32_t tid) {
...
void MeshDevice::begin_mesh_trace(uint8_t cq_id, uint32_t tid) {
    auto& mesh_trace_buffer = this->create_mesh_trace(tid);
    mesh_command_queues_[cq_id]->record_begin(tid, mesh_trace_buffer->desc);
}

void MeshDevice::end_mesh_trace(uint8_t cq_id, uint32_t tid) {
    auto trace_buffer = this->get_mesh_trace(tid);
