...
uint32_t BeginTraceCapture(MeshDevice* device, uint8_t cq_id) {
    uint32_t trace_id = MeshTrace::next_id();
    device->begin_mesh_trace(cq_id, trace_id);
    return trace_id;
}

void EndTraceCapture(MeshDevice* device, uint8_t cq_id, uint32_t trace_id) { device->end_mesh_trace(cq_id, trace_id); }
...
void ReleaseTrace(MeshDevice* device, const uint32_t trace_id) { device->release_mesh_trace(trace_id); }
