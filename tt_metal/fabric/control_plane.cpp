...
std::vector<chip_id_t> ControlPlane::get_mesh_physical_chip_ids(
    ...
    // TODO: if square mesh, we might need to pin another corner chip, or potentially have multiple possible
    // orientations
        ...
        if (mesh_ew_size % 2 == 0) {
            for (const auto& [dest_id, equal_dist_paths] : paths) {
                if ((equal_dist_paths.size() == 2) and (equal_dist_paths[0].size() == mesh_ew_size / 2 + 1)) {
                    ...
                    physical_chip_ids.insert(
                        physical_chip_ids.end(), equal_dist_paths[1].rbegin() + 1, equal_dist_paths[1].rend() - 1);
                    break;
                }
            }
        } else {
            ...
            for (const auto& [dest_id, equal_dist_paths] : paths) {
                if ((equal_dist_paths.size() == 1) and (equal_dist_paths[0].size() == mesh_ew_size / 2 + 1)) {
                    if (eth_connections.find(equal_dist_paths[0].back()) != eth_connections.end()) {
                        physical_chip_ids.insert(
                            physical_chip_ids.end(), equal_dist_paths[0].rbegin(), equal_dist_paths[0].rend() - 1);
                        break;
                    }
                }
            }
