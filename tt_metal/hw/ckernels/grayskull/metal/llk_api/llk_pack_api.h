...
inline void llk_pack(std::uint32_t tile_index, std::uint32_t output, std::uint32_t output_tile_index = 0) {
    ...
    _llk_pack_<DST_SYNC_MODE, out_of_order_output, untilize, is_fp32_dest_acc_en>(
        tile_index, pack_dst_format[output_id], pack_tile_addr);
}
...
inline void llk_pack_reduce_config_v2(uint32_t output) {
    ...
    if constexpr (at_kernel_start) {
        ...
        _llk_pack_hw_configure_<untilize>(
            pack_src_format[output_id], pack_dst_format[output_id], tile_size, relu_config.val);
    }
...
inline void llk_matmul_pack(
    ...
    const std::uint32_t output_tile_index = 0) {
