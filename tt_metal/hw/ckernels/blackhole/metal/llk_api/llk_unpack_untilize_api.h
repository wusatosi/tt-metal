...
inline void llk_unpack_untilize_hw_configure(const llk_unpack_A_params_t* unpack_untilize_params) {
    ...
    const uint32_t unpA_face_r_dim = ckernel::FACE_R_DIM;
...
inline void llk_unpack_untilize_init(std::uint32_t operand = 0) {
    ...
    TTI_RDCFG(
        ckernel::p_gpr_unpack::SR_UNPACK_UNTILIZER_STATE_0,
        UNP0_ADDR_CTRL_XY_REG_1_Ystride_ADDR32);  // Save unpack stride config
    TTI_RDCFG(
        ckernel::p_gpr_unpack::SR_UNPACK_UNTILIZER_STATE_1,
        THCON_SEC0_REG5_Tile_x_dim_cntx0_ADDR32);  // Save tile x dim per context
    TTI_RDCFG(
        ckernel::p_gpr_unpack::SR_UNPACK_UNTILIZER_STATE_2,
...
inline void llk_unpack_untilize_uninit(
    const std::uint32_t operand, const std::uint32_t face_r_dim = ckernel::FACE_R_DIM) {
    ...
    std::uint32_t unpA_ch1_x_stride = (uint)(unpack_dst_format[operand_id] & 0x3) == (uint)DataFormat::Float32   ? 4
                                      : (uint)(unpack_dst_format[operand_id] & 0x3) == (uint)DataFormat::Float16 ? 2
                                                                                                                 : 1;
    std::uint32_t unpA_ch1_y_stride = ckernel::FACE_C_DIM * ckernel::FACE_R_DIM * unpA_ch1_x_stride;
    ...
    TT_SETADCXX(ckernel::p_setadc::UNP_A, ckernel::FACE_R_DIM * ckernel::FACE_C_DIM - 1, 0x0);
    TTI_REG2FLOP(
        1,
        0,
        0,
        ...
        ckernel::p_gpr_unpack::FACE_DIM_16x16);
    ckernel::cfg_reg_rmw_tensix<THCON_SEC0_REG0_TileDescriptor_ADDR32 + 1, 0, 0xFFFF>(1);
