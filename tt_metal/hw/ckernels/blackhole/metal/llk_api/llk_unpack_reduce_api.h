...
inline void llk_unpack_reduce_hw_configure(
    ...
    constexpr bool within_face_16x16_transpose = (ckernel::ReduceDim::REDUCE_ROW == dim);
...
inline void llk_unpack_reduce_init(const std::uint32_t within_face_16x16_transpose = 0) {
    ...
    ckernel::cfg_reg_rmw_tensix<ALU_FORMAT_SPEC_REG1_SrcB_RMW>(unpB_dst_format);

    ckernel::cfg_reg_rmw_tensix<THCON_SEC1_REG0_TileDescriptor_ADDR32, 0, 0xf>(unpB_src_format);
    ckernel::cfg_reg_rmw_tensix<THCON_SEC1_REG2_Out_data_format_RMW>(unpB_dst_format);

    TTI_WRCFG(ckernel::p_gpr_unpack::L1_BUFFER_ADDR, ckernel::p_cfg::WRCFG_32b, THCON_SEC1_REG3_Base_address_ADDR32);
    TTI_WRCFG(
        ckernel::p_gpr_unpack::L1_BUFFER_ADDR, ckernel::p_cfg::WRCFG_32b, THCON_SEC1_REG3_Base_cntx1_address_ADDR32);
    TTI_NOP;
    TTI_NOP;
