...
inline void llk_unpack_tilize_hw_configure(const llk_unpack_A_params_t *unpack_tilize_params) {
    ...
    constexpr ckernel::StochRndType stoch_rnd_mode = ckernel::StochRndType::None;
...
inline void llk_unpack_tilize_uninit(const std::uint32_t operand, const std::uint32_t face_r_dim = ckernel::FACE_R_DIM) {
    // Revert X dim value to default.
    TT_SETADCXX(ckernel::p_setadc::UNP_A, face_r_dim * ckernel::FACE_C_DIM - 1, 0x0);
    TT_SETADCXX(ckernel::p_setadc::UNP_B, face_r_dim * ckernel::FACE_C_DIM - 1, 0x0);

    // Revert Z dim value back to default.
    const uint Tile_z_dim = get_operand_num_faces(operand);
    ckernel::cfg_reg_rmw_tensix<THCON_SEC0_REG0_TileDescriptor_ADDR32+1, 16, 0xffff0000>(Tile_z_dim);
    ...
    TTI_WRCFG(ckernel::p_gpr_unpack::FACE_DIM_16x16,0,THCON_SEC0_REG5_Tile_x_dim_cntx0_ADDR32);
    TTI_NOP;
}
...
inline void llk_unpack_tilizeA_B_mop_config(const bool narrow_tile = false, const std::uint32_t num_faces = 4) {
    ...
    ckernel::load_replay_buf(0, replay_buf_run_len, false, []{
        //Unpacks 1x16 row of datums to SrcA
        TTI_UNPACR(ckernel::SrcA, 0b01000000/*CH1_Y+=1*/, 0, 0, 0, 1, 0, ckernel::p_unpacr::RAREFYB_DISABLE, 0, 0, 0, 0, 1);
...
inline void llk_unpack_tilizeA_B_init(
    ...
    const std::uint32_t c_dim_size = ckernel::SCALE_DATUM_SIZE(unpack_src_format[operandA_id], ct_dim * ((num_faces==1) ? ckernel::FACE_C_DIM: ckernel::TILE_C_DIM)) >> 4;

    //This sets the scartch register that CFGSHIFTMASK instruction uses to increment the L1 address
    TT_SETDMAREG(0, LOWER_HALFWORD(c_dim_size), 0, LO_16(ckernel::p_gpr_unpack::TILE_OFFSET));
    TT_SETDMAREG(0, UPPER_HALFWORD(c_dim_size), 0, HI_16(ckernel::p_gpr_unpack::TILE_OFFSET));
    TTI_STALLWAIT(ckernel::p_stall::STALL_CFG, ckernel::p_stall::THCON);
    TTI_WRCFG(ckernel::p_gpr_unpack::TILE_OFFSET, 0, SCRATCH_SEC0_val_ADDR32);
    TTI_NOP;

    //Unpack 1 row of 1x16 at a time for SrcA
    ckernel::unpacker::config_unpacker_x_end<ckernel::p_setadc::UNP_A>(1);
    ...
    uint unpA_ch1_y_stride = ckernel::SCALE_DATUM_SIZE(unpack_dst_format[operandA_id], ckernel::FACE_C_DIM);
    ckernel::cfg_reg_rmw_tensix<UNP0_ADDR_CTRL_XY_REG_1_Ystride_RMW>(unpA_ch1_y_stride);
    ckernel::cfg_reg_rmw_tensix<THCON_SEC0_REG2_Haloize_mode_RMW>(0);
...
inline void llk_unpack_tilizeA_B(
    ...
    std::uint32_t unpA_face_r_dim = ckernel::FACE_R_DIM) {
    ...
    const std::uint32_t offset_address_a = ckernel::SCALE_DATUM_SIZE(unpack_src_format[operandA_id], tile_index_a) << 1;
    ...
    const std::uint32_t block_c_dim = block_ct_dim * ((num_faces==1) ? ckernel::FACE_C_DIM: ckernel::TILE_C_DIM) * face_r_dim;
    ...
    TTI_SETADCZW(ckernel::p_setadc::UNP_AB, 0, 0, 0, 0, 0b1111);
    ...
    for (std::uint32_t n = 0; n < num_faces; n++) {
        ...
        */
        std::uint32_t address_face_a = (n % 2 == 0) ? address_a : (address_a + (ckernel::SCALE_DATUM_SIZE(unpack_src_format[operandA_id], ckernel::FACE_C_DIM) >> 4));
        address_face_a += (n >= 2) ? ((ckernel::SCALE_DATUM_SIZE(unpack_src_format[operandA_id], block_c_dim)) >> 4) : 0;

        // Wait for free context
	ckernel::unpacker::wait_for_next_context(2);

        if constexpr (neginf_srcA) {
            TTI_UNPACR_NOP(ckernel::SrcA,0,0,0,0,0,0,ckernel::p_unpacr::UNP_CLRSRC_NEGINF, ckernel::p_unpacr::UNP_CLRSRC);
        }
	...
	ckernel::semaphore_post(ckernel::semaphore::UNPACK_SYNC);
        ...
        TTI_SETADCXY(ckernel::p_setadc::UNP_A, 0, 0, 0, 0, 0b1010);
        //Unpack SrcB 16x16 face & Set Data Valid

        //If reload_srcB, only first face needs to be loaded, otherwise CH0_Z+=1
        TTI_UNPACR(ckernel::SrcB, reload_srcB ? 0b0 : 0b1, 0, 0, 0, 1, 1, ckernel::p_unpacr::RAREFYB_DISABLE, 0, 0, 0, 0, 1);

        //Unpacks face_r_dim-1 rows of 1x16 datums to SrcA
        if (run_r_dim_loop) {
            ckernel_unpack_template::run(ckernel::instrn_buffer, face_r_dim-1, unp_cfg_context == 0 ? 0 : 0xffff);
        }

        //Unpack last SrcA row of a 16x16 face and SetDvalid
        TTI_UNPACR(ckernel::SrcA, 0b0, 0, 0, 0, 1, 1 /*Set Dvalid*/, ckernel::p_unpacr::RAREFYB_DISABLE, 0, 0, 0, 0, 1);
