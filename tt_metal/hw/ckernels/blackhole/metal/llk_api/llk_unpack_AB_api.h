...
inline void llk_unpack_AB_init(
    ...
    const std::uint32_t acc_to_dest = 0, const bool skip=false) {
    ...
    _llk_unpack_AB_init_<BType>(face_r_dim, num_faces, narrow_tile, transpose, acc_to_dest, skip);
}

template <BroadcastType BType = BroadcastType::NONE>
inline void llk_unpack_AB_skip(
    const std::uint32_t operandA,
    const std::uint32_t operandB,
    ...
    WAYPOINT("UABW");
    _llk_unpack_AB_skip_<BType>(address_a, address_b, transpose_of_faces > 0);
    WAYPOINT("UABD");
...
inline void llk_unpack_AB(
    ...
    std::uint32_t offset_address_a = get_local_cb_interface(operandA_id).fifo_page_size * tile_index_a;
    std::uint32_t address_a = base_address_a + offset_address_a;
    // just skipping the below still hangs
    std::uint32_t base_address_b = get_local_cb_interface(operandB_id).fifo_rd_ptr - 1;
    std::uint32_t offset_address_b = get_local_cb_interface(operandB_id).fifo_page_size * tile_index_b;
    std::uint32_t address_b = base_address_b + offset_address_b;
