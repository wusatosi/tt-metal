...
inline void llk_push_to_brisc(const std::int32_t operand, const std::int32_t num_tiles, const std::int32_t num_words) {
    ...
    // so we use get_local_cb_interface(output).tiles_received instead of tiles_received_ptr, because it is updated by TRISC2 and no additional synchronization is needed
