...
int main() {
    ...
    while (1) {
...

        {
            ...
            if (enables && end_cb_index != NUM_CIRCULAR_BUFFERS) {
                uint32_t tt_l1_ptr* remote_cb_l1_base =
                    (uint32_t tt_l1_ptr*)(kernel_config_base + launch_msg_address->kernel_config.remote_cb_offset);
                // Any atomics issued will be barriered within the fn
                experimental::setup_remote_cb_interfaces<true>(remote_cb_l1_base, end_cb_index, noc_index, noc_mode);
            }
