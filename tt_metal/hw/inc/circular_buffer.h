...
struct RemoteSenderCBInterface {
    ...
    // Used only for writing the final ptr value back to L1
    uint32_t fifo_wr_ptr_addr;
    ...
    // It's stored as pages_sent * num_receivers entries
    // Followed by pages_acked * num_receivers entries
...
static_assert(
    offsetof(RemoteSenderCBInterface, fifo_limit_page_aligned) == offsetof(RemoteReceiverCBInterface, fifo_limit_page_aligned),
...
static_assert(
    offsetof(RemoteSenderCBInterface, fifo_wr_ptr_addr) == offsetof(RemoteReceiverCBInterface, fifo_rd_ptr_addr),
    "fifo_wr_ptr_addr and fifo_rd_ptr_addr must be at the same offset in RemoteSenderCBInterface and RemoteReceiverCBInterface");
