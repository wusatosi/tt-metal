...
inline void setup_remote_cb_interfaces(
    uint32_t tt_l1_ptr* cb_l1_base, uint32_t start_cb_index, uint8_t noc, uint8_t nm) {
    ...
    for (uint32_t cb_id = NUM_CIRCULAR_BUFFERS - 1, end_id = start_cb_index - 1; cb_id != end_id; cb_id--) {
        ...
        if (is_sender) {
            ...
            // Only barrier atomics if we are updating remote over noc
            resize_remote_sender_cb_interface<update_remote_over_noc, update_remote_over_noc>(
                cb_id, page_size, noc, nm);
        } else {
            ...
            // Only barrier atomics if we are updating remote over noc
            resize_remote_receiver_cb_interface<update_remote_over_noc, update_remote_over_noc>(
                cb_id, page_size, noc, nm);
        }
