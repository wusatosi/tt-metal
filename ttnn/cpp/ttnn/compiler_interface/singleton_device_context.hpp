...
// Singleton class to manage the device context, ensuring the device remains active
// while running multiple graph traces without dispatching.
...
class SingletonDeviceContext {
   public:
    ...
    size_t get_compute_with_storage_grid_size() const {
        return m_device->compute_with_storage_grid_size().x * m_device->compute_with_storage_grid_size().y;
    }
