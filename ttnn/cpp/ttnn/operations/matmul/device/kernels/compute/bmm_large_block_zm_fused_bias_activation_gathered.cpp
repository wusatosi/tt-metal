...
void MAIN {
    ...
    const uint32_t start_core_x = get_compile_time_arg_val(17);
    const uint32_t start_core_y = get_compile_time_arg_val(18);
    const uint32_t NUM_CORES_X = get_compile_time_arg_val(19);

    volatile tt_l1_ptr uint32_t* unpadded_in0_shard_widths_in_tiles =
        (volatile tt_l1_ptr uint32_t*)(&kernel_compile_time_args[20]);
    constexpr uint32_t ct_args_idx = 20 + ring_size;
    ...
    // UNPACK(( DPRINT << "core_id " << (uint)core_id << " core_type " << core_type << " ring_idx " << ring_idx <<
    // ENDL() ));
