...
void kernel_main() {
    ...
    for (uint32_t n = start_n; n < N && num_tiles_read < dst_num_tiles; ++n, start_c = 0) {
        for (uint32_t c = start_c; c < C && num_tiles_read < dst_num_tiles; ++c, start_th = 0) {
            // Outer loop now iterates over columns.
            for (uint32_t tw = start_tw; tw < Wt && num_tiles_read < dst_num_tiles; ++tw) {
                DPRINT << "n: " << n << " c: " << c << " th: " << start_th << " tw: " << tw << ENDL();
                cb_reserve_back(cb_id_src, onetile);
                uint32_t l1_write_addr_src = get_write_ptr(cb_id_src);
                // Notice: the tile read index uses only the column offset.
                noc_async_read_tile(tile_offset + tw, src, l1_write_addr_src);
                noc_async_read_barrier();
                FILL_TILE_WITH_FIRST_ROW(cb_id_src);
                cb_push_back(cb_id_src, onetile);

                num_tiles_read += Ht - start_th;
            }
