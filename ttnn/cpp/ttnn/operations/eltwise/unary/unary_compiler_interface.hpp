...
namespace ttnn::compiler_interface::unary {
...
std::tuple<bool, size_t, size_t, size_t> unary_op_constraints(const OperandParams& input, const OperandParams& output) {
    ...
    size_t peak_cbs_per_core = 0;
    size_t peak_l1_tensors_per_core = 0;
    size_t output_l1_tensor_per_core = 0;

    try {
        auto op_trace = get_op_trace(input, output);

        peak_cbs_per_core = graph::extract_circular_buffers_peak_size_per_core(op_trace);

        auto interleave_storage_cores = SingletonDeviceContext::get_instance().get_compute_with_storage_grid_size();
        peak_l1_tensors_per_core =
            graph::extract_l1_buffer_allocation_peak_size_per_core(op_trace, interleave_storage_cores);
        output_l1_tensor_per_core =
            graph::extract_l1_output_buffer_allocation_size_per_core(op_trace, interleave_storage_cores);
    ...
    return std::make_tuple(constraint_valid, peak_cbs_per_core, peak_l1_tensors_per_core, output_l1_tensor_per_core);
}
