                ...
                if (shard_spec.has_value()) {
                    ...
                    std::tie(
                        ...
                        tt::tt_metal::split_work_to_cores(
                            compute_with_storage_grid_size, num_tiles / num_tiles_per_shard);
...
=======
    ...
    struct MultiCoreDramInterleaved {
        struct shared_variables_t {
            KernelHandle reader_kernel_id;
            KernelHandle compute_kernel_id;
            KernelHandle writer_kernel_id;
        ...
        static cached_program_t create(
            ...
            KernelHandle reader_id = tt::tt_metal::CreateKernel(
                program,
                "ttnn/cpp/ttnn/operations/multiplyadd/device/kernels/dataflow/reader_dram_interleaved.cpp",
                all_cores,
                ReaderDataMovementConfig());

            KernelHandle writer_id = tt::tt_metal::CreateKernel(
                program,
                "ttnn/cpp/ttnn/operations/multiplyadd/device/kernels/dataflow/writer_dram_interleaved.cpp",
                all_cores,
...
>>>>>>> Adding support for sharded tensors
                } else {
                    std::tie(
                        num_cores,
                        all_cores,
                ...
                CircularBufferConfig cb_src0_config =
                    CircularBufferConfig(
                        num_input_tiles * input_tile_size, {{src0_cb_index, tt::DataFormat::Float16_b}})
                ...
                CircularBufferConfig cb_src1_config =
                    CircularBufferConfig(
                        num_input_tiles * input_tile_size, {{src1_cb_index, tt::DataFormat::Float16_b}})
                ...
                CircularBufferConfig cb_src2_config =
                    CircularBufferConfig(
                        num_input_tiles * input_tile_size, {{src2_cb_index, tt::DataFormat::Float16_b}})
                ...
                CircularBufferConfig cb_dst0_config =
                    CircularBufferConfig(
                        num_input_tiles * input_tile_size, {{dst0_cb_index, tt::DataFormat::Float16_b}})
                        .set_page_size(dst0_cb_index, input_tile_size);
                CBHandle cb_dst0 = tt::tt_metal::CreateCircularBuffer(program, all_cores, cb_dst0_config);

                CircularBufferConfig cb_dst1_config =
                    CircularBufferConfig(
                        num_input_tiles * input_tile_size, {{dst1_cb_index, tt::DataFormat::Float16_b}})
...
=======
    struct MultiCoreHeightSharded {
        ...
        static cached_program_t create(
            ...
            uint32_t numTiles = inputTensor1.volume() / tt::constants::TILE_HW;
            uint32_t num_tiles_per_shard = 1;  // inputTensor1.shard_spec().value().shape[0] / tt::constants::TILE_HW;
            uint32_t num_shards = numTiles / num_tiles_per_shard;
            // num_tiles_per_shard = 0, numTiles = 56
            ...
            auto
                [num_cores,
                 ...
                 core_group_1,
                 core_group_2,
                 num_shards_per_core_group_1,
                 num_shards_per_core_group_2] =
                    tt::tt_metal::split_work_to_cores(compute_with_storage_grid_size, num_shards);

            CircularBufferConfig cb_src0_config =
                CircularBufferConfig(num_tiles_per_shard * inputTileSize, {{src0_cb_index, tt::DataFormat::Float16_b}})
                    .set_page_size(src0_cb_index, inputTileSize);
            CBHandle cb_src0 = tt::tt_metal::CreateCircularBuffer(program, all_cores, cb_src0_config);

            CircularBufferConfig cb_src1_config =
                CircularBufferConfig(num_tiles_per_shard * inputTileSize, {{src1_cb_index, tt::DataFormat::Float16_b}})
                    .set_page_size(src1_cb_index, inputTileSize);
            CBHandle cb_src1 = tt::tt_metal::CreateCircularBuffer(program, all_cores, cb_src1_config);

            CircularBufferConfig cb_src2_config =
                CircularBufferConfig(num_tiles_per_shard * inputTileSize, {{src2_cb_index, tt::DataFormat::Float16_b}})
                    .set_page_size(src2_cb_index, inputTileSize);
            CBHandle cb_src_intermediate = tt::tt_metal::CreateCircularBuffer(program, all_cores, cb_src2_config);

            CircularBufferConfig cb_dst0_config =
                CircularBufferConfig(num_tiles_per_shard * inputTileSize, {{dst0_cb_index, tt::DataFormat::Float16_b}})
                    .set_page_size(dst0_cb_index, inputTileSize);
            CBHandle cb_dst0 = tt::tt_metal::CreateCircularBuffer(program, all_cores, cb_dst0_config);

            CircularBufferConfig cb_dst1_config =
                CircularBufferConfig(num_tiles_per_shard * inputTileSize, {{dst1_cb_index, tt::DataFormat::Float16_b}})
                    .set_page_size(dst1_cb_index, inputTileSize);
            CBHandle cb_dst1 = tt::tt_metal::CreateCircularBuffer(program, all_cores, cb_dst1_config);

            KernelHandle reader_id = tt::tt_metal::CreateKernel(
                program,
                "ttnn/cpp/ttnn/operations/multiplyadd/device/kernels/dataflow/reader_height_sharded.cpp",
                all_cores,
                ReaderDataMovementConfig());

            KernelHandle writer_id = tt::tt_metal::CreateKernel(
                program,
                "ttnn/cpp/ttnn/operations/multiplyadd/device/kernels/dataflow/writer_height_sharded.cpp",
                all_cores,
                WriterDataMovementConfig());

            uint32_t num_shards_per_core = num_shards / num_cores;

            KernelHandle compute_id = tt::tt_metal::CreateKernel(
                program,
                "ttnn/cpp/ttnn/operations/multiplyadd/device/kernels/compute/fpu_height_sharded.cpp",
                all_cores,
                tt::tt_metal::ComputeConfig{.math_fidelity = MathFidelity::LoFi});

            for (uint32_t i = 0, num_shards_written = 0; i < num_cores; i++) {
                CoreCoord core = {i / num_cores_y, i % num_cores_y};
                uint32_t num_shards_per_core = 0;
                if (core_group_1.contains(core)) {
                    num_shards_per_core = num_shards_per_core_group_1;
                } else if (core_group_2.contains(core)) {
                    num_shards_per_core = num_shards_per_core_group_2;
                } else {
                    TT_ASSERT(false, "Core not in specified core ranges");
                }
                tt::tt_metal::SetRuntimeArgs(
                    ...
                    {src0_buffer->address(),
                     ...
                     num_tiles_per_shard,
                     num_shards_written});
                tt::tt_metal::SetRuntimeArgs(program, compute_id, core, {num_shards_per_core, num_tiles_per_shard});
                tt::tt_metal::SetRuntimeArgs(
                    program,
                    writer_id,
                    core,
                    {dst_buffer->address(), num_shards_per_core, num_tiles_per_shard, num_shards_written});

                num_shards_written += num_shards_per_core;
            }
    ...
    using program_factory_t = std::variant<MultiCoreDramInterleaved, MultiCoreHeightSharded>;

    static program_factory_t select_program_factory(
        const operation_attributes_t& operation_attributes, const tensor_args_t&) {
        return MultiCoreHeightSharded{};
    }
