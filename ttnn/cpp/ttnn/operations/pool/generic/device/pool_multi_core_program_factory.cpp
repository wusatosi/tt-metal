...
Pool2D::MultiCore::cached_program_t pool2d_multi_core_sharded_with_halo_v2_impl_new(
    ...
    // after reduction
    printf("shard spec: %d, %d\n", output.shard_spec().value().shape[0], output.shard_spec().value().shape[1]);
    uint32_t shard_width_padded = tt::round_up(output.shard_spec().value().shape[1], tt::constants::TILE_WIDTH);

    uint32_t out_cb_pagesize = output.shard_spec().value().shape[1] * out_nbytes / in_nblocks_c;
    // uint32_t out_cb_pagesize =
    //     shard_width_padded * out_nbytes / in_nblocks_c;  // there is just one row of channels after each reduction
    //     (or 1
