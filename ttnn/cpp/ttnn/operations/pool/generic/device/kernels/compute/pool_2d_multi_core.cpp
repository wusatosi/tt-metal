...
inline void tilize_in(uint32_t in_cb_id, uint32_t tiles_per_iteration, uint32_t out_cb_id) {
    tilize_init_short(in_cb_id, tiles_per_iteration, out_cb_id);

    cb_wait_front(in_cb_id, tiles_per_iteration);
    cb_reserve_back(out_cb_id, tiles_per_iteration);

    tilize_block(in_cb_id, tiles_per_iteration, out_cb_id);

    cb_push_back(out_cb_id, tiles_per_iteration);
    cb_pop_front(in_cb_id, tiles_per_iteration);
...
inline void reduce_h_fused(
    ...
    const uint32_t in_cb_id_tilized_0,
    ...
    for (uint32_t c_i = 0; c_i < num_output_tiles; ++c_i) {
        ...
        reduce_tile_with_faces(in_cb_id_tilized_0, in_scalar_cb_id, 0, 0, reduce_dst_idx++, num_faces_in_tile);
...
void MAIN {
    ...
    constexpr uint32_t in_cb_id_tilized_0 = get_compile_time_arg_val(16);
    ...
    for (uint32_t i = 0; i < nsticks_per_core; ++i) {
        ...
        for (uint32_t b_i = 0; b_i < in_nblocks_c - 1; ++b_i) {
            reduce_h_fused<max_tiles_per_iter, is_partial_tile, split_reader, window_size_hw>(
                in_cb_id_0, in_cb_id_1, in_cb_id_tilized_0, in_scalar_cb_id, i, out_cb_id);
        }
