...
inline uint32_t generate_reader_pattern_indices(
    ...
    if (!skip_top_partial_middle_aligned_row) {
        for (uint32_t top_partial_middle_row_width_i = 0;
             top_partial_middle_row_width_i < top_partial_middle_aligned_row_width;
             top_partial_middle_row_width_i += stride_w) {
    ...
    if (!skip_top_partial_right_aligned_row) {
        for (uint32_t top_partial_right_row_width_i = 0;
             top_partial_right_row_width_i < top_partial_right_aligned_row_width;
             top_partial_right_row_width_i += stride_w) {
    ...
    for (uint32_t top_partial_image_row_i = num_skip_rows_top_partial_image;
         top_partial_image_row_i < num_rows_top_partial_image;
         top_partial_image_row_i += stride_h) {
    ...
    for (uint32_t bottom_partial_image_row_i = num_skip_rows_bottom_partial_image;
         bottom_partial_image_row_i < num_rows_bottom_partial_image;
         bottom_partial_image_row_i += stride_h) {
        for (uint32_t row_width_i = 0; row_width_i < image_width; row_width_i += stride_w) {
    ...
    if (!skip_bottom_partial_left_aligned_row) {
        for (uint32_t bottom_partial_row_width_i = 0;
             bottom_partial_row_width_i < bottom_partial_left_aligned_row_width;
             bottom_partial_row_width_i += stride_w) {
...
void kernel_main() {
    uint32_t i = 0;
    uint32_t image_height = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t image_width = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t stride_h = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t stride_w = get_arg_val<uint32_t>(i);
    i += 1;

    // halo from prev core region args
    uint32_t halo_prev_read_enabled = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_prev_core_noc_x = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_prev_core_noc_y = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_prev_num_tiles = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_prev_start_addr = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_prev_addr_offset = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_prev_size_bytes = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t halo_prev_read_pattern_offset = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_prev_top_partial_middle_aligned_row_width = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_prev_skip_top_partial_middle_right_aligned_row = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t halo_prev_top_partial_right_aligned_row_width = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_prev_skip_top_partial_right_aligned_row = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t halo_prev_num_rows_top_partial_image = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_prev_num_skip_rows_top_partial_image = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t halo_prev_num_full_images = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t halo_prev_num_rows_bottom_partial_image = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_prev_num_skip_rows_bottom_partial_image = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t halo_prev_bottom_partial_left_aligned_row_width = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_prev_skip_bottom_partial_left_aligned_row = get_arg_val<uint32_t>(i);
    i += 1;

    // local region args
    uint32_t local_read_pattern_offset = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t local_top_partial_middle_aligned_row_width = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t local_skip_top_partial_middle_right_aligned_row = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t local_top_partial_right_aligned_row_width = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t local_skip_top_partial_right_aligned_row = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t local_num_rows_top_partial_image = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t local_num_skip_rows_top_partial_image = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t local_num_full_images = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t local_num_rows_bottom_partial_image = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t local_num_skip_rows_bottom_partial_image = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t local_bottom_partial_left_aligned_row_width = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t local_skip_bottom_partial_left_aligned_row = get_arg_val<uint32_t>(i);
    i += 1;

    // halo from next core region args
    uint32_t halo_next_read_enabled = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_next_core_noc_x = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_next_core_noc_y = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_next_num_tiles = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_next_start_addr = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_next_addr_offset = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_next_size_bytes = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t halo_next_read_pattern_offset = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_next_top_partial_middle_aligned_row_width = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_next_skip_top_partial_middle_right_aligned_row = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t halo_next_top_partial_right_aligned_row_width = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_next_skip_top_partial_right_aligned_row = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t halo_next_num_rows_top_partial_image = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_next_num_skip_rows_top_partial_image = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t halo_next_num_full_images = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t halo_next_num_rows_bottom_partial_image = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_next_num_skip_rows_bottom_partial_image = get_arg_val<uint32_t>(i);
    i += 1;

    uint32_t halo_next_bottom_partial_left_aligned_row_width = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t halo_next_skip_bottom_partial_left_aligned_row = get_arg_val<uint32_t>(i);
    i += 1;

    // args are for untilizing and tilizing
    uint32_t num_untilized_input_blocks = get_arg_val<uint32_t>(i);
    i += 1;  // input block contains only 1 row of tiles
    uint32_t num_tiles_untilized_input_block = get_arg_val<uint32_t>(i);
    i += 1;
    uint32_t num_output_tiles = get_arg_val<uint32_t>(i);
    i += 1;
    ...
    if (halo_prev_read_enabled) {
        ...
        noc_async_read(
            get_noc_addr(halo_prev_core_noc_x, halo_prev_core_noc_y, halo_prev_addr),
    ...
    if (halo_next_read_enabled) {
        ...
        noc_async_read(
            get_noc_addr(halo_next_core_noc_x, halo_next_core_noc_y, halo_next_addr),
    ...
    if (halo_prev_read_enabled) {
        ...
        reader_pattern_index = generate_reader_pattern_indices(
            image_height,
            image_width,
            stride_h,
            stride_w,
    ...
    // wait for untilized blocks from compute, drop rows based on reader_pattern and push to
    ...
    for (uint32_t untilized_input_block_i = 0; untilized_input_block_i < num_untilized_input_blocks;
        ...
        while (reader_pattern[reader_pattern_index] < 32 + untilize_block_offset) {
            // local read into untilized downsampled l1, read_address = l1_read_addr +
            // (reader_pattern[reader_pattern_index] * conv_act_c_bytes), read_size = conv_act_c_bytes, write_address =
            // untilize_downsampled_cb_l1_write_addr reader pattern indices are for the whole sharded input. Need to
            // subtract by block offset to read from the block which is 1 row of tiles.
            uint32_t read_address =
                untilize_block_l1_read_addr +
                ((uint32_t)((int32_t)reader_pattern[reader_pattern_index] - (int32_t)untilize_block_offset)
