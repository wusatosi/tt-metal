...
uint32_t get_bf16_pool_scalar(Pool2DType pool_type, uint32_t kernel_size_hw, std::optional<int32_t> divisor_override) {
    float value;
    switch (pool_type) {
        case Pool2DType::MAX_POOL2D: value = 1.; break;
        case Pool2DType::AVG_POOL2D:
            value = divisor_override.has_value() ? 1. / (float)divisor_override.value() : 1. / (float)kernel_size_hw;
            break;
