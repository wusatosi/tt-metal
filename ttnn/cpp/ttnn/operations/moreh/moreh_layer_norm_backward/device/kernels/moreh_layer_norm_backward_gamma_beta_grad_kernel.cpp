    ...
    for (uint32_t outer_idx = 0; outer_idx < num_cols_per_core; outer_idx++) {
        for (uint32_t inner_idx = 0; inner_idx < NCHt; inner_idx++) {
            ...
            if (do_mask_h && ((h_idx + 1) % origin_Ht == 0)) {
                ...
                ckernel:: copy_tile(cb_mask_h, 0, dst1);
        ...
        if (gamma_grad_has_value) {
            ...
            ckernel:: tile_regs_acquire();
            ckernel::cb_wait_front(cb_ydyadd, onetile);
            ckernel::cb_reserve_back(cb_dgamma, onetile);
            ...
            if (is_lastdim_layernorm || is_groupnorm) {
                ...
                ckernel::reduce_revert_delta(cb_dgamma);
            } else {
                // Just copy
                copy_tile_init_with_dt(cb_ydyadd);
                ckernel:: copy_tile(cb_ydyadd, 0, dst0);
            }
            ckernel:: tile_regs_commit();
            ...
            ckernel::cb_pop_front(cb_ydyadd, onetile);
            ckernel::cb_push_back(cb_dgamma, onetile);
            ckernel::tile_regs_release();
        }  // gamma_grad_has_value
        ...
        if (beta_grad_has_value) {
            ...
            ckernel::cb_pop_front(cb_dyadd, onetile);
            ckernel::cb_push_back(cb_dbeta, onetile);
            ckernel::tile_regs_release();
        }  // beta_grad_has_value
