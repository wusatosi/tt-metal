...
void MAIN {
    ...
    ckernel::cb_wait_front(cb_n_recip_n, 2);     // comes from the reader
    ...
    if (do_mask_h || do_mask_w) {
        ckernel::cb_wait_front(cb_mask_h_w, 2);  // comes from the reader
    }
    ...
    for (uint32_t ncht = 0; ncht < NCHt; ncht++) {
        ckernel::cb_wait_front(cb_mean, onetile);  // comes from the reader
        ckernel::cb_wait_front(cb_rstd, onetile);  // comes from the reader
        ...
        for (uint32_t wt = 0; wt < Wt; wt++) {
            ...
            ckernel::cb_reserve_back(cb_xmm, onetile);
            ...
            ckernel::cb_push_back(cb_xmm, onetile);
            ckernel::tile_regs_release();

            // Compute cb_y
            // (x - mean) * rstd and mask(optional)
            ckernel:: tile_regs_acquire();
            ckernel::cb_wait_front(cb_xmm, onetile);
            ...
            ckernel::cb_pop_front(cb_xmm, onetile);
            ckernel::cb_push_back(cb_y, onetile);
            ckernel::tile_regs_release();

            // Copy cb_dy to cb_dycopy
            ckernel::cb_reserve_back(cb_dycopy, onetile);
            if (gamma_has_value) {
                // Compute cb_dycopy
                // dycopy = dy * gamma and mask(optional)
                ckernel:: tile_regs_acquire();
                ckernel::cb_wait_front(cb_dy, onetile);     // comes from the reader
                ckernel::cb_wait_front(cb_gamma, onetile);  // comes from the reader
                ...
                ckernel::cb_pop_front(cb_dy, onetile);
                ckernel::cb_pop_front(cb_gamma, onetile);
                ckernel::cb_push_back(cb_dycopy, onetile);
                ckernel::tile_regs_release();
            } else {
                // Compute cb_dycopy
                ...
                ckernel:: copy_tile(cb_dy, 0, dst0);
            ...
            // Compute cb_dyadd
            ckernel::cb_wait_front(cb_dycopy, onetile);
            if (wt == 0) {
                ckernel:: tile_regs_acquire();
                ckernel::cb_reserve_back(cb_dyadd, onetile);

                copy_tile_init_with_dt(cb_dycopy);
                ckernel:: copy_tile(cb_dycopy, 0, dst0);
                ckernel:: tile_regs_commit();

                ckernel::tile_regs_wait();
                pack_tile_with_dt(dst0, cb_dyadd);

                ckernel::cb_push_back(cb_dyadd, onetile);
                ckernel::tile_regs_release();
            } else {
                ckernel:: tile_regs_acquire();
                ckernel::cb_wait_front(cb_dyadd, onetile);
                ckernel::cb_reserve_back(cb_dyadd, onetile);
                ...
                ckernel::cb_pop_front(cb_dyadd, onetile);
                ckernel::cb_push_back(cb_dyadd, onetile);
                ckernel::tile_regs_release();
            }
            ...
            // Compute cb_ydy
            ckernel:: tile_regs_acquire();
            ckernel::cb_wait_front(cb_y, onetile);
            ckernel::cb_reserve_back(cb_ydy, onetile);
            ...
            ckernel::cb_pop_front(cb_y, onetile);
            ckernel::cb_pop_front(cb_dycopy, onetile);
            ckernel::cb_push_back(cb_ydy, onetile);
            ckernel::tile_regs_release();

            // Compute cb_ydyadd
            if (wt == 0) {
                ckernel:: tile_regs_acquire();
                ckernel::cb_wait_front(cb_ydy, onetile);
                ckernel::cb_reserve_back(cb_ydyadd, onetile);

                copy_tile_init_with_dt(cb_ydy);
                ckernel:: copy_tile(cb_ydy, 0, dst0);
                ckernel:: tile_regs_commit();

                ckernel::tile_regs_wait();
                pack_tile_with_dt(dst0, cb_ydyadd);

                ckernel::cb_pop_front(cb_ydy, onetile);
                ckernel::cb_push_back(cb_ydyadd, onetile);
                ckernel::tile_regs_release();
            } else {
                ckernel:: tile_regs_acquire();
                ckernel::cb_wait_front(cb_ydy, onetile);
                ckernel::cb_wait_front(cb_ydyadd, onetile);
                ckernel::cb_reserve_back(cb_ydyadd, onetile);
                ...
                ckernel::cb_pop_front(cb_ydy, onetile);
                ckernel::cb_pop_front(cb_ydyadd, onetile);
                ckernel::cb_push_back(cb_ydyadd, onetile);
                ckernel::tile_regs_release();
            }
        }  // Wt loop
        ...
        ckernel::cb_pop_front(cb_dyadd, onetile);
        ckernel::cb_push_back(cb_dysum, onetile);
        ckernel::tile_regs_release();
        ...
        ckernel::cb_pop_front(cb_ydyadd, onetile);
        ckernel::cb_push_back(cb_ydysum, onetile);
        ckernel::tile_regs_release();
        ...
        ckernel:: tile_regs_acquire();
        ckernel::cb_reserve_back(cb_recip_nrstd, onetile);
        ...
        ckernel::cb_push_back(cb_recip_nrstd, onetile);
        ckernel::tile_regs_release();

        // Compute cb_dx
        // ((n * dy - Sum[dy]) - (y * Sum[y * dy])) * (rstd / n)
        ckernel::cb_wait_front(cb_dysum, onetile);
        ckernel::cb_wait_front(cb_ydysum, onetile);
        ckernel::cb_wait_front(cb_recip_nrstd, onetile);
        for (uint32_t wt = 0; wt < Wt; wt++) {
            // Copy cb_dy to cb_dycopy
            ckernel::cb_reserve_back(cb_dycopy, onetile);
            if (gamma_has_value) {
            ...
            ckernel:: tile_regs_acquire();
            ckernel::cb_wait_front(cb_dycopy, onetile);
            ckernel::cb_reserve_back(cb_ndy, onetile);
            ...
            ckernel::cb_pop_front(cb_dycopy, onetile);
            ckernel::cb_push_back(cb_ndy, onetile);
            ckernel::tile_regs_release();
            ...
            ckernel:: tile_regs_acquire();
            ckernel::cb_wait_front(cb_ndy, onetile);
            ckernel::cb_reserve_back(cb_ndymdysum, onetile);
            ...
            ckernel::cb_pop_front(cb_ndy, onetile);
            ckernel::cb_push_back(cb_ndymdysum, onetile);
            ckernel::tile_regs_release();
            ...
            ckernel:: tile_regs_acquire();
            ckernel::cb_wait_front(cb_y, onetile);
            ckernel::cb_reserve_back(cb_yydysum, onetile);
            ...
            ckernel::cb_pop_front(cb_y, onetile);
            ckernel::cb_push_back(cb_yydysum, onetile);
            ckernel::tile_regs_release();
            ...
            ckernel:: tile_regs_acquire();
            ckernel::cb_wait_front(cb_ndymdysum, onetile);
            ckernel::cb_wait_front(cb_yydysum, onetile);
            ckernel::cb_reserve_back(cb_tmp4, onetile);
            ...
            ckernel::cb_pop_front(cb_ndymdysum, onetile);
            ckernel::cb_pop_front(cb_yydysum, onetile);
            ckernel::cb_push_back(cb_tmp4, onetile);
            ckernel::tile_regs_release();

            // Compute cb_dx
            // ((n * dy - Sum[dy]) - (y * Sum[y * dy])) * (rstd / n)
            ckernel:: tile_regs_acquire();
            ckernel::cb_wait_front(cb_tmp4, onetile);
            ckernel::cb_reserve_back(cb_dx, onetile);
            ...
            ckernel::cb_pop_front(cb_tmp4, onetile);
            ckernel::cb_push_back(cb_dx, onetile);
            ckernel::tile_regs_release();
        }  // Wt loop
        ckernel::cb_pop_front(cb_recip_nrstd, onetile);
        ckernel::cb_pop_front(cb_dysum, onetile);
        ckernel::cb_pop_front(cb_ydysum, onetile);

        ckernel::cb_pop_front(cb_mean, onetile);
        ckernel::cb_pop_front(cb_rstd, onetile);
    }  // NCHt loop
    ckernel::cb_pop_front(cb_scaler, onetile);
    ckernel::cb_pop_front(cb_n_recip_n, 2);

    if (do_mask_h || do_mask_w) {
        ckernel::cb_pop_front(cb_mask_h_w, 2);
    }
}  // void MAIN
}  // namespace NAMESPACE
