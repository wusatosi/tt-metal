...
void MAIN {
    ...
    for (uint32_t ncht = 0; ncht < NCHt; ncht++) {
        ...
        // y = (x - mean) * rstd
        ckernel::cb_reserve_back(cb_y, Wt);
        ...
        ckernel::cb_reserve_back(cb_dycopy, Wt);
        for (uint32_t wt = 0; wt < Wt; wt++) {
            if (gamma_has_value) {
                // Compute cb_dycopy
        ...
        ckernel::cb_wait_front(cb_dycopy, Wt);
        for (uint32_t wt = 0; wt < Wt; wt++) {
            if (wt == 0) {
                ckernel:: tile_regs_acquire();
        ...
        ckernel::cb_pop_front(cb_dycopy, Wt);
        ckernel::cb_pop_front(cb_y, Wt);
    ...
    if (do_mask_h || do_mask_w) {
        ckernel::cb_wait_front(cb_mask_h_w, 2);
    }
}  // void MAIN
}  // namespace NAMESPACE
