...
void MAIN {
    ...
    for (uint32_t idx = 0; idx < num_input_tiles_per_core; ++idx) {
        ...
        ckernel::cb_wait_front(cb_y, onetile);   // comes from the reader
        ...
        ckernel::cb_reserve_back(cb_tmp5, onetile);
        ...
        ckernel::cb_push_back(cb_tmp5, onetile);

        // 1 / y^p
        power_and_recip_tile_to_cb(cb_y, cb_tmp0, cb_tmp1, cb_decimal, cb_tmp2, cb_recip_ypow, p, p_is_negative);

        // (x^(p - 1) * y * dy) / y^p -> cb_dx
        ckernel::cb_wait_front(cb_tmp5, onetile);
        ckernel::cb_wait_front(cb_recip_ypow, onetile);
        ckernel::cb_reserve_back(cb_tmp4, onetile);
        ...
        ckernel::cb_pop_front(cb_tmp5, onetile);
        ckernel::cb_pop_front(cb_recip_ypow, onetile);
        ckernel::cb_push_back(cb_tmp4, onetile);
