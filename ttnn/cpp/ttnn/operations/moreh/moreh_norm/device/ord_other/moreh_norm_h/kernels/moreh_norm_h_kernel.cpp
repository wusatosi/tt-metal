...
void MAIN {
    ...
    if (do_mask_h) {
        ckernel::cb_wait_front(cb_mask_h, onetile);  // comes from the reader
    }
    for (uint32_t col_idx = 0; col_idx < num_cols_per_core; ++col_idx) {
        ...
        ckernel::cb_pop_front(cb_cal, onetile);
        ckernel::cb_push_back(cb_reduce, onetile);

        ckernel:: tile_regs_acquire();

        ckernel::cb_wait_front(cb_reduce, onetile);
        ckernel::cb_reserve_back(cb_y, onetile);

        copy_tile_init_with_dt(cb_reduce);
        ckernel:: copy_tile(cb_reduce, 0, dst0);
#ifdef MINUS_INF
        negative_tile_init();
        negative_tile(dst0);
#endif
        ...
        ckernel::cb_pop_front(cb_reduce, onetile);
