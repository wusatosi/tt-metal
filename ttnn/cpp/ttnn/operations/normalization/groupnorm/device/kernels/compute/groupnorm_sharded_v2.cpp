...
// tilize in0 -> in
...
#ifdef READER_REPACK
        ckernel::cb_wait_front(cb_in_rm, per_core_N);
#endif
        ckernel::cb_reserve_back(cb_in, per_core_N);
        tilize_block(cb_in_rm, per_core_N, cb_in);
        ckernel::cb_push_back(cb_in, per_core_N);
        ckernel::cb_pop_front(cb_in_rm, per_core_N);
    }
    tilize_uninit(cb_in_rm, cb_in);
    ckernel::cb_wait_front(cb_in, per_core_MN);
#else
    ...
    for (uint32_t b = 0; b < batch; ++b) {
        ...
        for (uint32_t g = 0; g < group; ++g) {
            ...
            ckernel::cb_reserve_back(cb_x, block_hw);
            ckernel::cb_wait_front(cb_input_mask, block_w);
            ...
            ckernel::tile_regs_release();
            ckernel::cb_push_back(cb_ex_partial, 1);
            ...
            if constexpr (is_mcast_sender and num_cores_per_mcast_group > 1) {
                ...
                ckernel::cb_reserve_back(cb_ex, 1);
                ckernel:: tile_regs_acquire();
                ...
                ckernel:: tile_regs_commit();
                ckernel::tile_regs_wait();
                ckernel:: pack_tile(dst0, cb_ex_global);
                ckernel::tile_regs_release();
                ckernel::reduce_revert_delta(cb_ex_global);
            ...
            ckernel::cb_reserve_back(cb_xmm, block_hw);
            ...
            for (uint32_t i = 0; i < block_h; i++) {
                ...
                ckernel::cb_pop_front(cb_x, block_w);
            }
            ckernel::cb_pop_front(cb_ex_global, 1);
            ckernel::cb_push_back(cb_xmm, block_hw);
            ...
            ckernel::cb_reserve_back(cb_x, block_hw);
            ckernel::cb_wait_front(cb_xmm, block_hw);
            for (uint32_t i = 0; i < block_h; i++) {
                index_subblock_w_offset = 0;
                for (uint32_t j = 0; j < num_subblocks_w; ++j) {
                    ckernel:: tile_regs_acquire();
                ...
                ckernel::cb_pop_front(cb_xmm, block_w);
            }
            ckernel::cb_pop_front(cb_input_mask, block_w);
            ckernel::cb_push_back(cb_x, block_hw);
            ...
            ckernel::tile_regs_release();
            ckernel::cb_push_back(cb_ex_partial, 1);
            ckernel::cb_pop_front(cb_xmm, block_hw);
            ckernel::reduce_revert_delta(cb_ex_partial);
            ...
            ckernel::cb_wait_front(cb_eps, 1);
            ckernel::cb_wait_front(cb_ex_global, 1);
            ...
            ckernel::cb_pop_front(cb_x, block_hw);
            ckernel::cb_wait_front(cb_xmm, block_hw);
            ...
            for (uint32_t w = 0; w < block_w_curr; ++w) {
                ...
                for (uint32_t i = 0; i < block_h; ++i) {
                    ...
                    if (copy_or_add == true) {
                        ckernel:: copy_tile(cb_xmm, index_xmm, dst0);
                    } else {
    ...
    ckernel::cb_push_back(cb_out, per_core_MN);
    ckernel::cb_pop_front(cb_in, per_core_MN);

    if constexpr (do_gamma) {
        index_h_offset = 0;
        mul_bcast_rows_init_short(cb_out, cb_gamma);
        ckernel::cb_reserve_back(cb_outgamma, per_core_MN);
        ckernel::cb_wait_front(cb_gamma, per_core_N);
        for (uint32_t i = 0; i < per_core_M; ++i) {
            for (uint32_t j = 0; j < per_core_N; ++j) {
                ...
                ckernel:: tile_regs_commit();
                ckernel::tile_regs_wait();
                ckernel:: pack_tile(dst0, cb_outgamma);
                ckernel::tile_regs_release();
            }
            index_h_offset += per_core_N;
        }
        ckernel::cb_push_back(cb_outgamma, per_core_MN);
        ckernel::cb_pop_front(cb_out, per_core_MN);
        ckernel::cb_wait_front(cb_outgamma, per_core_MN);
    }

    if constexpr (do_beta) {
        index_h_offset = 0;
        add_bcast_rows_init_short(cb_inbeta, cb_beta);
        ckernel::cb_reserve_back(cb_outbeta, per_core_MN);
        ckernel::cb_wait_front(cb_beta, per_core_N);
        for (uint32_t i = 0; i < per_core_M; ++i) {
            for (uint32_t j = 0; j < per_core_N; ++j) {
                ckernel:: tile_regs_acquire();
                uint32_t index = j + index_h_offset;
                ...
                ckernel:: tile_regs_commit();
                ckernel::tile_regs_wait();
                ckernel:: pack_tile(dst0, cb_outbeta);
                ckernel::tile_regs_release();
            }
            index_h_offset += per_core_N;
        }
        ckernel::cb_push_back(cb_outbeta, per_core_MN);
        ckernel::cb_pop_front(cb_inbeta, per_core_MN);
        ckernel::cb_wait_front(cb_outbeta, per_core_MN);
    }

#ifdef UNTILIZE_OUT
    // untilize
    untilize_init_short(cb_untilize_in);
    ckernel::cb_wait_front(cb_untilize_in, per_core_MN);
    for (uint32_t m = 0; m < per_core_M; ++m) {
        ckernel::cb_reserve_back(cb_untilize_out, per_core_N);
        untilize_block(cb_untilize_in, per_core_N, cb_untilize_out);
        ckernel::cb_push_back(cb_untilize_out, per_core_N);
        ckernel::cb_pop_front(cb_untilize_in, per_core_N);
    }
