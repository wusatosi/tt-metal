...
void MAIN {
    ...
    for (uint32_t ncht = 0; ncht < NCHt; ncht++) {
        ...
        ckernel::cb_wait_front(cb_stats, stats_tiles_cols);
        ckernel::cb_reserve_back(cb_stats_reduced, stats_tile_stride);
#ifdef RMSNORM
        ckernel::cb_reserve_back(cb_var, 1);
#endif
        ACQ();
        ...
        ckernel:: pack_tile(0, cb_stats_reduced);
        ...
        ckernel:: pack_tile(1, cb_stats_reduced);
#else
        ckernel:: pack_tile(0, cb_var);
#endif
        REL();
        ckernel::cb_push_back(cb_stats_reduced, stats_tile_stride);
        ckernel::cb_pop_front(cb_stats, stats_tiles_cols);
#ifdef RMSNORM
        ckernel::cb_push_back(cb_var, 1);
#endif

        ckernel::reduce_revert_delta(cb_stats_reduced);
        ...
        ckernel::cb_reserve_back(cb_mean_squared, onetile);
        ckernel::cb_wait_front(cb_stats_reduced, stats_tile_stride);
        ACQ();
        mul_tiles(cb_stats_reduced, cb_stats_reduced, 1, 1, 0);
        ckernel:: pack_tile(0, cb_mean_squared);
        REL();

        ckernel::cb_push_back(cb_mean_squared, 1);
        ...
        ckernel::cb_wait_front(cb_mean_squared, 1);
        ACQ();
        sub_tiles(cb_stats_reduced, cb_mean_squared, 0, 0, 0);
        ckernel:: pack_tile(0, cb_var);
        REL();
        ckernel::cb_push_back(cb_var, 1);
        ckernel::cb_pop_front(cb_mean_squared, 1);
        ...
        for (uint32_t wt = 0; wt < Wt; wt += blk) {
            ckernel::cb_wait_front(cb_inp, blk);
            ckernel::cb_reserve_back(cb_x_minus_mean, blk);
            ACQ();
            for (uint32_t wtr = 0; wtr < blk; wtr++) {
                sub_tiles_bcast_cols(cb_inp, cb_stats_reduced, wtr, 1, wtr);
                ckernel:: pack_tile(wtr, cb_x_minus_mean);
            }
            REL();
            ckernel::cb_push_back(cb_x_minus_mean, blk);
            ckernel::cb_pop_front(cb_inp, blk);
        }
#endif

        // free up CBs
        ckernel::cb_pop_front(cb_stats_reduced, stats_tile_stride);

        /*
         * 1/sqrt(var + eps)
         */
        ckernel::cb_wait_front(cb_var, 1);
        ckernel::cb_reserve_back(cb_recip_sqrt_var, 1);
        ...
        recip_tile_init();
        recip_tile(0);
        ckernel:: pack_tile(0, cb_recip_sqrt_var);
        REL();
        ckernel::cb_push_back(cb_recip_sqrt_var, 1);
        ckernel::cb_pop_front(cb_var, 1);
        ...
        ckernel::cb_wait_front(cb_recip_sqrt_var, 1);
        for (uint32_t wt = 0; wt < Wt; wt += blk) {
            ckernel::cb_wait_front(cb_norm_x_input, blk);
            ckernel::cb_reserve_back(cb_x_normed, blk);
            ACQ();
            for (uint32_t wtr = 0; wtr < blk; wtr++) {
                mul_tiles_bcast_cols(cb_norm_x_input, cb_recip_sqrt_var, wtr, 0, wtr);
                ckernel:: pack_tile(wtr, cb_x_normed);
            }
            REL();
            ckernel::cb_push_back(cb_x_normed, blk);
            ckernel::cb_pop_front(cb_norm_x_input, blk);
        }
        ckernel::cb_pop_front(cb_recip_sqrt_var, 1);
        ...
        ckernel::cb_wait_front(cb_gamma, Wt);
        mul_bcast_rows_init_short(cb_x_normed, cb_gamma);
        for (uint32_t wt = 0; wt < Wt; wt += blk) {
            ckernel::cb_wait_front(cb_x_normed, blk);
            ckernel::cb_reserve_back(cb_times_gamma_out, blk);
            ACQ();
            for (uint32_t wtr = 0; wtr < blk; wtr++) {
                mul_tiles_bcast_rows(cb_x_normed, cb_gamma, wtr, wt + wtr, wtr);
                ckernel:: pack_tile(wtr, cb_times_gamma_out);
            }
            REL();
            ckernel::cb_push_back(cb_times_gamma_out, blk);
            ckernel::cb_pop_front(cb_x_normed, blk);
        }
        ...
        if constexpr (do_gamma and do_beta) {
            ...
            ckernel::cb_wait_front(cb_beta, Wt);
            add_bcast_rows_init_short(cb_times_gamma_out, cb_beta);
            for (uint32_t wt = 0; wt < Wt; wt += blk) {
                ckernel::cb_wait_front(cb_times_gamma_out, blk);
                ckernel::cb_reserve_back(cb_out, blk);
                ACQ();
                for (uint32_t wtr = 0; wtr < blk; wtr++) {
                    add_tiles_bcast_rows(cb_times_gamma_out, cb_beta, wtr, wt + wtr, wtr);
                    ckernel:: pack_tile(wtr, cb_out);
                }
                REL();
                ckernel::cb_push_back(cb_out, blk);
                ckernel::cb_pop_front(cb_times_gamma_out, blk);
            }
        }
    }
    ckernel::cb_pop_front(cb_eps, 1);
    ckernel::cb_pop_front(cb_reduce, 1);
    ckernel::cb_pop_front(cb_gamma, Wt);
    if constexpr (do_beta) {
        ckernel::cb_pop_front(cb_beta, Wt);
    }
}
}  // namespace NAMESPACE
