...
ALWI void calc_numeric_stable(uint32_t cb_in, uint32_t cb_bcast_scaler, uint32_t cb_max, uint32_t cb_out) {
    ...
    for (uint32_t w = 0; w < block_w; w++) {
        ...
        ckernel::reduce_tile<PoolType::MAX, ReduceDim::REDUCE_ROW>(cb_in, cb_bcast_scaler, w, bcast_scaler0, 0);
    }
    ...
    for (uint32_t j = 0; j < num_subblocks_w; j++) {
        ACQ();
        ckernel::cb_reserve_back(cb_out, subblock_w);
        ...
        for (uint32_t w = 0; w < subblock_w; w++) {
            ...
            ckernel:: pack_tile(w, cb_out);
        }
        ckernel::cb_push_back(cb_out, subblock_w);
        REL();
        index_subblock_w_offset += subblock_w;
    }
    ckernel::cb_pop_front(cb_in, block_w);
    ckernel::cb_pop_front(cb_max, 1);
    ckernel::cb_wait_front(cb_out, block_w);
}
...
void MAIN {
    ...
    for (uint32_t i = 0; i < block_h; i++) {
        ...
        for (uint32_t j = 0; j < num_subblocks_w; j++) {
            ACQ();
            ckernel::cb_reserve_back(cb_scale_mask, subblock_w);
            for (uint32_t w = 0; w < subblock_w; w++) {
                index = w + index_subblock_w_offset;
                mul_tiles_bcast_scalar(cb_in0, cb_fused_scale, index, 0, w);
                ckernel:: pack_tile(w, cb_scale_mask);
            }
            ckernel::cb_push_back(cb_scale_mask, subblock_w);
            REL();
            index_subblock_w_offset += subblock_w;
        }
        ...
        // fused attn
        ckernel::cb_wait_front(cb_scale_mask, block_w);

#ifndef SHARDED_CAUSAL_MASK
        ckernel::cb_wait_front(cb_fused_attn, block_w);
#endif
...
#endif
            ckernel::cb_reserve_back(cb_x, subblock_w);
            for (uint32_t w = 0; w < subblock_w; w++) {
#ifndef NUMERIC_STABLE
                exp_tile<EXP_APPROX>(w);
#endif
                ckernel:: pack_tile(w, cb_x);
            }
            ckernel::cb_push_back(cb_x, subblock_w);
            REL();
            index_subblock_w_offset += subblock_w;
        }
        ckernel::cb_pop_front(cb_scale_mask, block_w);

// add numeric_stable
// fuse exp with sub tiles
#ifdef NUMERIC_STABLE
        ckernel::cb_wait_front(cb_x, block_w);
        calc_numeric_stable<block_w, num_subblocks_w, subblock_w>(cb_x, cb_bcast_scaler, cb_max, cb_exps);
#endif

#ifdef CAUSAL_MASK
        ckernel::cb_pop_front(cb_fused_attn, block_w);
#endif
        ...
        for (uint32_t j = 0; j < num_subblocks_w; j++) {
            ...
            for (uint32_t w = 0; w < subblock_w; w++) {
                index = w + index_subblock_w_offset;
                ckernel:: copy_tile(cb_in0, index, w);
            }
            ckernel::cb_reserve_back(cb_exps, subblock_w);
            for (uint32_t w = 0; w < subblock_w; w++) {
                exp_tile<EXP_APPROX>(w);
                ckernel:: pack_tile(w, cb_exps);
            }
            ckernel::cb_push_back(cb_exps, subblock_w);
            REL();
            index_subblock_w_offset += subblock_w;
        }
        ckernel::cb_pop_front(cb_in0, block_w);
        ...
        ckernel::cb_wait_front(cb_exps, block_w);
        ckernel::cb_wait_front(cb_bcast_scaler, 1);
        ckernel::cb_reserve_back(cb_recipsumexps, 1);
        for (uint32_t w = 0; w < block_w; w++) {
            constexpr uint32_t bcast_scaler0 = 0;
        ...
        for (uint32_t j = 0; j < num_subblocks_w; j++) {
            ACQ();
            ckernel::cb_reserve_back(cb_out0, subblock_w);
            for (uint32_t w = 0; w < subblock_w; w++) {
                index = w + index_subblock_w_offset;
                mul_tiles_bcast<BroadcastType::COL>(cb_exps, cb_recipsumexps, index, 0, w);
                ckernel:: pack_tile(w, cb_out0);
            }
            ckernel::cb_push_back(cb_out0, subblock_w);
            REL();
            index_subblock_w_offset += subblock_w;
        }
        ckernel::cb_pop_front(cb_recipsumexps, 1);
        ckernel::cb_pop_front(cb_exps, block_w);
    }
}
}  // namespace NAMESPACE
