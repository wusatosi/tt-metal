...
ALWI void batchnorm_bcast_tiles(
    ...
    for (uint32_t i = 0; i < onetile; ++i) {
        ckernel:: copy_tile(cb_eps, i, i * 2 + 1);
    ...
    ckernel::cb_wait_front(cb_bcast, onetile);  // input - batch_mean
    ckernel::cb_wait_front(cb_den, onetile);    // (input - batch_mean)/(sqrt(batch_var + eps)) = result
    ...
    for (uint32_t j = tile_start; j < freq; ++j) {
        ...
        for (uint32_t i = 0; i < onetile; ++i) {
            ckernel:: copy_tile(cb_other, i, i * 2);
        }
        sub_binary_tile_init();
        copy_tile_to_dst_init_short_with_dt(cb_other, cb_bcast);
        for (uint32_t i = 0; i < onetile; ++i) {
            ckernel:: copy_tile(cb_bcast, i, i * 2 + 1);
        ...
        for (uint32_t i = 0; i < onetile; ++i) {
            ckernel:: copy_tile(cb_den, i, i * 2 + 1);
            mul_binary_tile(i * 2, i * 2 + 1);

            ckernel:: pack_tile(i * 2, cb_affine_or_out);
        }
        ckernel:: tile_regs_commit();
        ckernel::tile_regs_release();
        ...
        if (bias_has_value) {  // result = result + bias
            ...
            for (uint32_t i = 0; i < onetile; ++i) {
                ckernel:: copy_tile(cb_tmp_1, i, i * 2);
            }
            add_binary_tile_init();
            copy_tile_to_dst_init_short_with_dt(cb_tmp_1, cb_bias);
            for (uint32_t i = 0; i < onetile; ++i) {
                ckernel:: copy_tile(cb_bias, i, i * 2 + 1);
                add_binary_tile(i * 2, i * 2 + 1);

                ckernel:: pack_tile(i * 2, cb_output_0);
            }
            ckernel:: tile_regs_commit();
            ckernel::tile_regs_release();
