...
void MAIN {
    ...
    for (uint32_t i = 0; i < input_height; ++i) {
        ...
        UNPACK((DPRINT << "UNPACK chunk_count_addr " << (uint32_t)chunk_addr_ptr<<" content " <<(uint32_t)(*chunk_addr_ptr) <<ENDL() ));
        MATH((DPRINT << "MATH chunk_count_addr " << (uint32_t)chunk_addr_ptr<<" content "<<(uint32_t)(*chunk_addr_ptr) <<ENDL() ));
        PACK((DPRINT << "PACK chunk_count_addr " << (uint32_t)chunk_addr_ptr<<" content "<<(uint32_t)(*chunk_addr_ptr) <<ENDL() ));
