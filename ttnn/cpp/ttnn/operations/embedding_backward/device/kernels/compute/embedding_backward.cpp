...
void MAIN {
    ...
    for (uint32_t i = 0; i < input_height; ++i) {
        ckernel::cb_wait_front(cb_grad, max_tiles_per_core);

        // Get chunk_count from reader
        volatile uint32_t* chunk_addr_ptr;
        ckernel::cb_get_tile(cb_chunk_count_scratch, 0, &chunk_addr_ptr);
        uint32_t chunk_count = chunk_addr_ptr[4];  // Need to shift because read ptr is off by 1 << 4 in BBE
        ckernel::cb_release_tile(cb_chunk_count_scratch);

        for (uint32_t chunk = 0; chunk < chunk_count; ++chunk) {  // chunk_count
            ckernel::cb_wait_front(cb_mask, 1);
            // get cb_index pointer from unpack to math thread
            volatile uint* idx_addr_ptr;
            uint32_t tile_to_get = 0;
            ckernel::cb_get_tile(cb_mask, tile_to_get, &idx_addr_ptr);
            uint32_t idx_addr = reinterpret_cast<uint32_t>(idx_addr_ptr);

            ckernel::cb_wait_front(cb_out_intermed, max_tiles_per_core);

            ckernel::cb_reserve_back(cb_out, max_tiles_per_core);

            for (uint32_t hidden_dim = 0; hidden_dim < tiles_per_core; hidden_dim++) {
                ckernel:: tile_regs_acquire();
                ckernel::tile_regs_wait();

                ckernel:: copy_tile(cb_grad, hidden_dim, 0);

                ckernel:: copy_tile(cb_out_intermed, hidden_dim, 1);

                reshuffle_rows_tile_init();
                reshuffle_rows_tile(0, idx_addr);

                ckernel:: pack_tile(1, cb_out, hidden_dim);  // reshuffle puts output into Tile 1 in DEST

                ckernel:: tile_regs_commit();
                ckernel::tile_regs_release();
            }

            ckernel::cb_push_back(cb_out, max_tiles_per_core);
            ckernel::cb_pop_front(cb_out_intermed, max_tiles_per_core);

            ckernel::cb_release_tile(cb_mask);
            ckernel::cb_pop_front(cb_mask, 1);
        }

        ckernel::cb_pop_front(cb_grad, max_tiles_per_core);
    }
}
}  // namespace NAMESPACE
