...
std::unordered_map<CoreCoord, std::vector<PageStride>> get_core_page_ranges_conv_crop(
...
enum class ConvCropStridesInRange { ALL_STRIDES, FIRST_HALF, SECOND_HALF };

std::vector<uint32_t> get_runtime_args_for_given_ranges_conv_crop(
    ...
    const ConvCropStridesInRange conv_crop_strides_in_range = ConvCropStridesInRange::ALL_STRIDES) {
    ...
    for (uint32_t range_id = starting_range; range_id < ending_range; range_id++) {
        ...
        uint32_t start_data;
        if (conv_crop_strides_in_range == ConvCropStridesInRange::ALL_STRIDES) {
            num_strides = ps.num_strides;
            start_core_x = ps.start_core.x;
            start_core_y = ps.start_core.y;
            start_data = ps.start_data;
        } else {
            if (conv_crop_strides_in_range == ConvCropStridesInRange::FIRST_HALF) {
...
operation::ProgramWithCallbacks conv_crop_multicore_generic(const Tensor& input, Tensor& output) {
    auto device = input.device();
    auto output_core_to_page_range_pair = get_core_page_ranges_conv_crop(input.buffer(), output.buffer());
    ...
    for (const auto& core : cores) {
        ...
        auto runtime_args_0 = get_runtime_args_for_given_ranges_conv_crop(
        ...
        auto runtime_args_1 = get_runtime_args_for_given_ranges_conv_crop(
            physical_core_coords,
            page_stride_vector,
            output_page_offset,
            input.buffer()->address(),
...
struct WidthShardedRuntimeArgsConvCrop {
    uint32_t write_size;
...
std::tuple<std::vector<std::vector<WidthShardedRuntimeArgsConvCrop>>, uint32_t, uint32_t, uint32_t>
compute_width_sharded_conv_crop_runtime_args(
    ...
    using WidthShardedRuntimeArgsConvCropForSingleCore = std::vector<WidthShardedRuntimeArgsConvCrop>;
    ...
    std::vector<WidthShardedRuntimeArgsConvCropForSingleCore> runtime_args_for_each_core;
    ...
    for (const auto& core : local_cores) {
        WidthShardedRuntimeArgsConvCropForSingleCore core_args;
...
template <bool is_reader>
operation::ProgramWithCallbacks conv_crop_multi_core_same_height(const Tensor& input, Tensor& output) {
    auto device = input.device();
...
operation::ProgramWithCallbacks conv_crop_multi_core(const Tensor& input, Tensor& output) {
    if (input.memory_config().memory_layout == TensorMemoryLayout::HEIGHT_SHARDED &&
        ...
        if (output.memory_config().buffer_type == BufferType::L1) {
            ...
            return conv_crop_multi_core_same_width<false>(input, output);
        }
    } else if (
        ...
        if (output.memory_config().buffer_type == BufferType::L1) {
            return conv_crop_multi_core_same_height<true>(input, output);
        } else {
            return conv_crop_multi_core_same_height<false>(input, output);
        }
    } else {
        return conv_crop_multicore_generic(input, output);
    }
}
