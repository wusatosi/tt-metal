...
operation::ProgramWithCallbacks copy_multi_core_with_sub_core_grids(
    const Tensor& input, const Tensor& output, const CoreRangeSet& sub_core_grids, bool backwards) {
    tt::tt_metal::Program program{};
    ...
    auto core_grid = sub_core_grids;
    auto num_cores = core_grid.num_cores();
    auto num_units_per_core = num_units / num_cores;
    ...
    if (convert_dtype) {
        ...
        tt::tt_metal::CircularBufferConfig output_cb_config =
            tt::tt_metal::CircularBufferConfig(
                num_output_units * aligned_output_unit_size, {{output_cb_index, output_cb_data_format}})
                .set_page_size(output_cb_index, aligned_output_unit_size);
        auto cb_output = tt::tt_metal::CreateCircularBuffer(program, core_grid, output_cb_config);
    }
    ...
    if (convert_dtype) {
        std::vector<uint32_t> compute_kernel_args_group_1 = {num_units_per_core};
        auto eltwise_unary_kernel_group_1 = tt::tt_metal::CreateKernel(
            program,
            "ttnn/cpp/ttnn/deprecated/tt_dnn/kernels/compute/eltwise_copy.cpp",
            core_grid,
