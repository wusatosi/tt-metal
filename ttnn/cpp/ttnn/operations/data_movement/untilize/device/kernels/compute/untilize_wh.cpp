...
void MAIN {
    ...
    const uint32_t cb_id_in0 = get_compile_time_arg_val(3);
    const uint32_t cb_id_out = get_compile_time_arg_val(4);
    ...
    for (uint32_t b = 0; b < block_size_col * third_dim; ++b) {
        ...
        untilize_block(cb_id_in0, block_size_row, cb_id_out);
