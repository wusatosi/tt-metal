...
void kernel_main() {
    ...
    ckernel::cb_reserve_back(cb_id_out, num_unpadded_output_rows);
    ...
    for (uint32_t i = 0; i < batches_of_8; i++) {
        ckernel::cb_wait_front(cb_id_untilize_out, 8);
        ...
        ckernel::cb_pop_front(cb_id_untilize_out, 8);
    }

    ckernel::cb_wait_front(cb_id_untilize_out, remaining_tiles);
    uint64_t noc_l1_read_addr = get_noc_addr(get_read_ptr(cb_id_untilize_out));
    ...
    noc_async_read_barrier();
    ckernel::cb_pop_front(cb_id_untilize_out, remaining_tiles);

    ckernel::cb_push_back(cb_id_out, num_unpadded_output_rows);
}
