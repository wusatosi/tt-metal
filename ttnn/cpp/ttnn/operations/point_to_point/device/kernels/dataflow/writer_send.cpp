...
void kernel_main() {
    ...
    const auto receiver_base_address = get_arg_val<uint32_t>(0);
    ...
    const bool dst_is_forward = get_arg_val<uint32_t>(4);
    const auto page_size_bytes = get_arg_val<uint32_t>(5);
    const auto payload_size_bytes = get_arg_val<uint32_t>(6);
    const auto max_pages_per_packet = get_arg_val<uint32_t>(7);
    const auto receive_semaphore_addr = get_arg_val<uint32_t>(8);
    ...
    const uint64_t packet_base_addr = get_write_ptr(packet_cb_id);
    ...
    for (uint32_t page_idx = page_idx_start, packet_page_count = 0; page_idx < page_idx_end;
        ...
        const uint64_t packet_addr = packet_base_addr + packet_page_count * aligned_page_size_bytes;
        DPRINT << packet_addr << " " << packet_base_addr << " " << packet_page_count << " " << aligned_page_size_bytes
               << "\n";
        // tt_memmove<true, false, false, 0>(packet_addr, src_page_ptr, page_size_bytes);
        //  !TODO async copies
        cb_pop_front(sender_cb_id, 1);
        if (packet_page_count == curr_pages_per_packet) {
    ...
    const uint64_t receive_sem_noc_addr = get_noc_addr(my_x[noc_idx], my_y[noc_idx], receive_semaphore_addr);
    auto sem_header_ptr = reinterpret_cast<PACKET_HEADER_TYPE*>(sem_header_addr);
    ...
    // connection_direction.send_payload_flush_blocking_from_address(sem_header_addr, packet_header_size_bytes);
