...
ttnn::device_operation::CachedProgram<PointToPointOp::SendReceive::shared_variables_t> send_program_factory(
    ...
    constexpr auto num_packet_headers_storable = 3;  // ?
    ...
    for (auto c : corerange_to_cores(all_cores, std::nullopt)) {
        ...
        page_idx_end = std::min(page_idx_end + increment, input_num_pages);

        const std::vector<uint32_t> reader_runtime_args = {
            input_tensor.buffer()->address(), page_idx_start, page_idx_end};
