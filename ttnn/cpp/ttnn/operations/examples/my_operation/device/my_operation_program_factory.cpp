...
namespace ttnn::operations : examples {
    ...
    MyDeviceOperation::MyDeviceProgramFactory::create(
        ...
        // buffer and program definitions
        ...
        tt::DataFormat cb_data_format_output =
        ...
        // split work to cores
        // different core groups allow for cores to have differing number of tiles to process
    ...
    tt:
        ...
        auto compute_kernel_1 = tt::tt_metal::CreateKernel(
            program,
            "ttnn/cpp/ttnn/operations/examples/my_operation/device/kernels/compute/my_compute.cpp",
            core_group_1,
        ...
        for (uint32_t i = 0, num_tiles_written = 0; i < all_cores; i++) {
            ...
            tt::tt_metal::SetRuntimeArgs(
                ...
                reader_kernel,
                core,
                {cb_src0->address(), cb_src1->address(), num_tiles_per_core, num_tiles_written});
            tt::tt_metal::SetRuntimeArgs(
                program, writer_kernel, core, {cb_output->address(), num_tiles_per_core, num_tiles_written});
        }

        // return program
        return {
            std::move(program), {
                .reader_kernel_id = reader_kernel, .writer_kernel_id = writer_kernel,
                .compute_kernel_id_1 = compute_kernel_1, .compute_kernel_id_2 = compute_kernel_2,
            }
        }
    }
