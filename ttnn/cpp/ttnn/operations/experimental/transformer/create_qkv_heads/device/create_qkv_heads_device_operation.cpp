...
std::vector<tt::tt_metal::LegacyShape> CreateQKVHeadsDeviceOperation::compute_output_shapes(
    const std::vector<Tensor>& input_tensors) const {
    std::vector<tt::tt_metal::LegacyShape> output_shape_vec;
    const auto& input_tensor = input_tensors.at(0);
    const auto input_shape = input_tensor.get_legacy_shape();

    const auto q_output_shape =
        tt::tt_metal::LegacyShape{input_shape[0], this->num_q_heads, input_shape[2], this->head_dim};
    const auto v_output_shape =
        tt::tt_metal::LegacyShape{input_shape[0], this->num_kv_heads, input_shape[2], this->head_dim};
