...
std::pair<uint32_t, uint32_t> get_min_priority(uint32_t priority_addr, uint32_t other_priority_addr) {
    ...
    return std::make_pair(min_priority, min_other_priority);
}

template <uint32_t cb_scratch_id, uint32_t B, uint32_t priority_stick_size>
std::pair<uint32_t, uint32_t> read_min_priority_from_scratch() {
    uint32_t cum_wait = 0;
    uint32_t min_priority = UINT32_MAX;
    for (uint32_t i = 0; i < B; i++) {
        cum_wait += 1;
...
//     uint32_t priority_cb_wr_ptr = get_write_ptr(cb_scratch_id);
//     uint32_t other_priority_cb_wr_ptr = get_write_ptr(cb_scratch_id) + B*priority_scalar_bytes;
//     for (uint32_t i = 0; i < B; i++) {
//         uint64_t priority_noc_addr = get_noc_addr(i, priority_addr_gen);
//         uint64_t other_priority_noc_addr = get_noc_addr(i, other_priority_addr_gen);
//         noc_async_read(priority_noc_addr, priority_cb_wr_ptr, priority_scalar_bytes*B);
//         noc_async_read(other_priority_noc_addr, other_priority_cb_wr_ptr, priority_scalar_bytes*B);
//         DPRINT << "priority_cb_wr_ptr: " << priority_cb_wr_ptr << ENDL();
//         DPRINT << "other_priority_cb_wr_ptr: " << other_priority_cb_wr_ptr << ENDL();
//         priority_cb_wr_ptr += priority_scalar_bytes;
//         other_priority_cb_wr_ptr += priority_scalar_bytes;
//     }

//     noc_async_read_barrier();

//     volatile tt_l1_ptr uint32_t* priority_ptr = reinterpret_cast<volatile tt_l1_ptr
//     uint32_t*>(get_write_ptr(cb_scratch_id));

//     DPRINT << "priority_ptr: " << get_write_ptr(cb_scratch_id) << ENDL();
//     DPRINT << "Memory at priority_ptr[0]: " << priority_ptr[0] << ENDL();
//     DPRINT << "Memory at priority_ptr[1]: " << priority_ptr[1] << ENDL();
//     DPRINT << "Memory at priority_ptr[B]: " << priority_ptr[B] << ENDL();

//     uint32_t min_priority = UINT32_MAX;
//     uint32_t min_other_priority = UINT32_MAX;
//     for (uint32_t i = 0; i < B; i++) {
//         uint32_t cur_priority = priority_ptr[i];
//         uint32_t cur_other_priority = priority_ptr[i+B];
//         DPRINT << "cur_priority: " << cur_priority << ENDL();
//         DPRINT << "cur_other_priority: " << cur_other_priority << ENDL();
//         if (cur_priority < min_priority) {
//             min_priority = cur_priority;
