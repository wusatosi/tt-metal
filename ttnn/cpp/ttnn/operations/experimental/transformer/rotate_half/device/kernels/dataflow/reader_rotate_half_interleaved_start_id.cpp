...
void kernel_main() {
    ...
    ckernel::cb_reserve_back(cb_id_in_scalar, onetile);
    ...
    ckernel::cb_push_back(cb_id_in_scalar, onetile);
    ...
    for (uint32_t i = 0; i < num_rows; i++) {
        for (uint32_t j = 0; j < half_row_size; j++) {
            ckernel::cb_reserve_back(cb_id_in_no_mul, onetile);
            ...
            ckernel::cb_push_back(cb_id_in_no_mul, onetile);
            in_no_mul_curr_id++;

            ckernel::cb_reserve_back(cb_id_in_mul, onetile);
            uint32_t in1_l1_write_addr = get_write_ptr(cb_id_in_mul);
            noc_async_read_tile(in_mul_curr_id, s, in1_l1_write_addr);
            noc_async_read_barrier();
            ckernel::cb_push_back(cb_id_in_mul, onetile);
            in_mul_curr_id++;
        }
