...
void kernel_main() {
    ...
    for (uint32_t i = 0; i < num_rows; i++) {
        for (uint32_t j = 0; j < half_row_width; j++) {
            ckernel::cb_wait_front(cb_id_out_no_mul, onetile);
            ...
            ckernel::cb_pop_front(cb_id_out_no_mul, onetile);
            out_no_mul_curr_id++;

            ckernel::cb_wait_front(cb_id_out_mul, onetile);
            uint32_t out_mul_l1_read_addr = get_read_ptr(cb_id_out_mul);
            noc_async_write_tile(out_mul_curr_id, s, out_mul_l1_read_addr);
            noc_async_write_barrier();
            ckernel::cb_pop_front(cb_id_out_mul, onetile);
            out_mul_curr_id++;
        }
