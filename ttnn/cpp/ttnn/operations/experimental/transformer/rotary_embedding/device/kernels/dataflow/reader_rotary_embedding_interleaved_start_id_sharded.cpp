...
void kernel_main() {
    ...
    ckernel::cb_reserve_back(input_cb_id, num_rows * Wt);
    ckernel::cb_push_back(input_cb_id, num_rows * Wt);
    ...
    ckernel::cb_reserve_back(scalar_cb_id, onetile);
    ...
    ckernel::cb_push_back(scalar_cb_id, onetile);
    ...
    for (uint32_t i = 0; i < num_rows; ++i) {
        ckernel::cb_reserve_back(rotated_input_cb_id, Wt);
        ...
        noc_async_read_barrier();
        ckernel::cb_push_back(rotated_input_cb_id, Wt);

#ifndef DECODE_MODE
        for (uint32_t j = 0; j < Wt; ++j) {
            ckernel::cb_reserve_back(sin_cb_id, onetile);
            uint32_t sin_l1_write_addr = get_write_ptr(sin_cb_id);
            noc_async_read_tile(cos_sin_curr_id, s2, sin_l1_write_addr);
            noc_async_read_barrier();
            ckernel::cb_push_back(sin_cb_id, onetile);

            ckernel::cb_reserve_back(cos_cb_id, onetile);
            uint32_t cos_l1_write_addr = get_write_ptr(cos_cb_id);
            noc_async_read_tile(cos_sin_curr_id, s1, cos_l1_write_addr);
            noc_async_read_barrier();
            ckernel::cb_push_back(cos_cb_id, onetile);
            cos_sin_curr_id++;
        }
        ht++;
        if (ht == Ht) {
