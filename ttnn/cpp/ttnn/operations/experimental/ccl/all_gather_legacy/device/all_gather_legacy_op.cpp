...
#include "all_gather_legacy_op.hpp"
...
void AllGatherLegacy::validate_with_output_tensors(
    ...
    TT_FATAL(
        ...
        "Unsupported input tensor shape and configuration");
...
std::vector<ttnn::TensorSpec> AllGatherLegacy::compute_output_specs(const std::vector<Tensor>& input_tensors) const {
    const auto& input_tensor = input_tensors[0];
...
tt::tt_metal::operation::MeshWorkloadWithCallbacks AllGatherLegacy::create_mesh_workload(
...
tt::tt_metal::operation::ProgramWithCallbacks AllGatherLegacy::create_program_at(
...
tt::tt_metal::operation::Hash AllGatherLegacy::compute_program_hash(const std::vector<Tensor>& input_tensors) const {
    ...
    return tt::tt_metal::operation::hash_operation<AllGatherLegacy>(
        this->dim,
        this->num_links,
        this->ring_size,
...
Tensor all_gather_legacy_impl(
    ...
    TT_FATAL(
        ...
        "all_gather_legacy op is only supported for Fast Dispatch");
    uint32_t num_devices = devices.size();
    TT_FATAL(num_devices > 1, "all_gather_legacy op will only work for num_devices > 1, but has {}", num_devices);
