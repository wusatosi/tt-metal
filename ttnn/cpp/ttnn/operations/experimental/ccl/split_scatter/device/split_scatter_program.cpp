...
std::tuple<CoreRangeSet, std::vector<CoreCoord>> select_worker_cores(
    ...
    if (persistent_fabric_mode) {
        ...
        if (available_cores.num_cores() < num_workers_preferred) {
            log_warning(
                tt::LogOp,
                "SplitScatter is being launched on a subdevice with fewer worker cores available than ideal. Ideally "
                "{} "
...
// For ring split-scatter, we can send sub-sections of input tensor in opposite directions
// For linear split-scatter though, we must ensure we send full tensors in BOTH directions
...
operation::ProgramWithCallbacks split_scatter_multi_core_with_workers(
    ...
    const auto [sender_worker_core_range, sender_worker_cores] =
        select_worker_cores(num_links, num_workers_per_link, enable_persistent_fabric_mode, device);
    ...
    // ring_index)
    uint32_t partition_size = input_tensor.shape()[dim] / ring_size;

    auto input_tensor_slicer = ttnn::ccl::GenericWrappedTensorSlicerV2(
        input_tensor,
        dim,             // Split along the last dimension (dim = 3)
        ring_index,      // Partition index (device index)
        partition_size,  // Partition size (e.g., 128 for 2 devices)
        num_links        // Number of workers per link
    );
