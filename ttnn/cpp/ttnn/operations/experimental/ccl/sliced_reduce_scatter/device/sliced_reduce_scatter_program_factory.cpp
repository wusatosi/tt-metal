...
tt::tt_metal::operation::ProgramWithCallbacks sliced_reduce_scatter_async_minimal(
    ...
    auto worker_sender_reader_kernel_id = tt::tt_metal::CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/ccl/sliced_reduce_scatter/device/kernels/"
        "interleaved_sliced_reduce_scatter_reader.cpp",
    ...
    writer_kernel_config.compile_args = {
        ...
        sliced_reduce_scatter_detail::PACKET_HEADER_BUFFER_SIZE,  // num_packet_headers_storable
    ...
    auto worker_sender_writer_kernel_id = tt::tt_metal::CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/ccl/sliced_reduce_scatter/device/kernels/"
        "interleaved_sliced_reduce_scatter_writer.cpp",
        sender_worker_core_range,
    ...
    auto receiver_writer_kernel_id = tt::tt_metal::CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/ccl/sliced_reduce_scatter/device/kernels/"
        "interleaved_sliced_reduce_scatter_receiver_writer.cpp",
    ...
    auto receiver_reader_kernel_id = tt::tt_metal::CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/ccl/sliced_reduce_scatter/device/kernels/"
        "interleaved_sliced_reduce_scatter_receiver_reader.cpp",
        receiver_worker_core_range,
    ...
    // Create receiver reduce kernel
    const uint32_t num_packets = tt::div_up(out_col_tiles, pages_per_packet) * out_row_tiles;
    auto receiver_reduce_kernel_config = tt::tt_metal::ComputeConfig{};
    ...
    auto receiver_reduce_kernel_id = tt::tt_metal::CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/ccl/sliced_reduce_scatter/device/kernels/"
        "reduction.cpp",
        receiver_worker_core_range,
        receiver_reduce_kernel_config);
    ...
    */
    ...
    // Simple stride for dim=3 scatter_dim
    uint32_t input_shard_col_tiles = in_col_tiles / ring_size;
    uint32_t input_shard_row_tiles = in_row_tiles;
    uint32_t input_col_device_stride = input_shard_col_tiles;
    ...
    uint32_t sender_writer_global_semaphore_args_idx;
    uint32_t receiver_reader_global_semaphore_args_idx;
    for (uint32_t link = 0; link < num_links; link++) {
        const uint32_t receiver_core_x = device->worker_core_from_logical_core(receiver_worker_cores[link]).x;
        const uint32_t receiver_core_y = device->worker_core_from_logical_core(receiver_worker_cores[link]).y;
        ...
        for (const auto& semaphore : semaphore) {
            semaphore_addresses.push_back(semaphore.address());
        }
        log_info(tt::LogOp, "semaphore_addresses: {}", semaphore_addresses);
    ...
    auto override_runtime_arguments_callback =
        [worker_sender_reader_kernel_id,
         ...
         sender_worker_cores,
         sender_writer_global_semaphore_args_idx,
         receiver_reader_global_semaphore_args_idx,
         receiver_worker_cores](
            ...
            auto semaphore = static_cast<const ttnn::SlicedReduceScatterAsync*>(operation)->semaphore;
            ...
            for (const auto& core : sender_worker_cores) {
                ...
                for (uint32_t i = 0; i < semaphore.size(); i++) {
                    worker_writer_sender_runtime_args[sender_writer_global_semaphore_args_idx + i] =
                        semaphore[i].address();
                }
            }
            // receiver
            for (uint32_t i = 0; i < receiver_worker_cores.size(); i++) {
                ...
                for (uint32_t i = 0; i < semaphore.size(); i++) {
                    receiver_reader_runtime_args[receiver_reader_global_semaphore_args_idx + i] =
                        semaphore[i].address();
                }
            }
        };
