...
void kernel_main() {
    ...
    while (tiles_read < num_tiles_to_read) {
        ...
        uint64_t noc0_dest_noc_addr = get_noc_addr(core_noc_x[core_id], core_noc_y[core_id], reduction_input_addr);

        // Within-shard offset
        // noc0_dest_noc_addr += shard_tile_id * tensor0_page_size;

        DPRINT << "noc0_dest_noc_addr " << noc0_dest_noc_addr << ENDL();
    ...
    // auto* pkt_hdr = reinterpret_cast<volatile PACKET_HEADER_TYPE*>(packet_header_buffer_seminc);
