...
void MAIN {
    ...
    const uint32_t num_packets = slice_num_pages / tiles_per_packet / num_batches;
    ...
    for (uint32_t b = 0; b < num_batches; b++) {
        for (uint32_t i = 0; i < ring_size - 1; i++) {  // Don't reduce on the first slice
            ...
            for (uint32_t packet_id = 0; packet_id < num_packets; packet_id++) {
                ...
                cb_wait_front(intermediate_cb, tiles_per_packet);
                ...
                cb_pop_front(intermediate_cb, tiles_per_packet);
