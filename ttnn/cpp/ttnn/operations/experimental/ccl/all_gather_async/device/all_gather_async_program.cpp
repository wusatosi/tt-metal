...
tt::tt_metal::operation::ProgramWithCallbacks all_gather_async_multi_core_with_workers(
    ...
    for (std::size_t link = 0; link < num_links; link++) {
        ...
        //     // 5, increment the termination semaphore for local device for local teardown only for the drain sync
        //     core auto termination_infos = local_fabric_handle->generate_local_chip_fabric_termination_infos(device);
