...
#include "llama_prefill_reduce_scatter_device_operation.hpp"
...
CoreRangeSet get_worker_cores(const CoreRangeSet& available_cores, const uint32_t num_workers, bool row_wise = false) {
...
LlamaPrefillReduceScatterDeviceOperation::LlamaPrefillReduceScatterAdd::cached_mesh_workload_t
LlamaPrefillReduceScatterDeviceOperation::LlamaPrefillReduceScatterAdd::create_mesh_workload(
...
ttnn::device_operation::CachedProgram<
    LlamaPrefillReduceScatterDeviceOperation::LlamaPrefillReduceScatterAdd::shared_variables_t>
LlamaPrefillReduceScatterDeviceOperation::LlamaPrefillReduceScatterAdd::create_at(
    const operation_attributes_t& operation_attributes,
    ...
    const uint32_t cluster_axis = 1;  // operation_attributes.cluster_axis;

    const uint32_t ring_devices = (cluster_axis == 0) ? mesh_view.num_rows() : mesh_view.num_cols();
    // TT_FATAL(ring_devices > 1, "reduce_scatter async op will only work for ring_devices > 1, but has {}",
    // ring_devices);

    auto target_device = mesh_device->get_device(mesh_coordinate);

    const uint32_t ring_size = ring_devices;  // operation_attributes.ring_devices;
    const uint32_t num_devices = ring_size;

    uint32_t ring_index = 0;  // Initialize device index

    std::vector<IDevice*> devices = (cluster_axis == 0) ? mesh_view.get_devices_on_column(mesh_coordinate[1])
                                                        : mesh_view.get_devices_on_row(mesh_coordinate[0]);

    std::optional<IDevice*> forward_device = devices.at(0);
    std::optional<IDevice*> backward_device = devices.at(ring_size - 1);
    ...
    uint32_t num_links = 1;  // operation_attributes.num_links;

    // std::string device_order = rs_prefill::device_order_array_string(ring_size, ring_index);

    // std::map<std::string, std::string> reader_defines = {{"DEVICE_ORDER", device_order}};

    const auto& input_shape = input_tensor.get_logical_shape();
    const auto dim = 3;  // operation_attributes.dim;
    ...
    uint32_t num_packets_to_send = tile_num_magic_num;
    uint32_t num_packets_to_send_per_worker = tile_num_magic_num;
    ...
    //     (input_shard_cores_per_device * input_tiles_per_core_width + num_pages_per_packet - 1) /
    //     num_pages_per_packet;
    auto packet_worker_cores_grid = rs_prefill::get_worker_cores(sub_device_cores, 1);

    auto available_cores = sub_device_cores.subtract(packet_worker_cores_grid);

    auto sender_core_grid = rs_prefill::get_worker_cores(available_cores, num_workers_per_link * num_links);
    auto all_cores_grid = packet_worker_cores_grid.merge(sender_core_grid);
    // auto all_cores_grid = CoreRangeSet(
    //     CoreRange(CoreCoord(0, 0), CoreCoord(0, 1)));

    // auto schedule = rs_prefill::distribute_work_evenly(
    //     input_shard_cores_per_device,
    //     num_workers_per_link * num_links,
    //     input_tiles_per_core_width,
    //     num_pages_per_packet);
    // auto schedule_string = rs_prefill::schedule_to_string(schedule);
    ...
    static constexpr auto num_packet_headers_storable = tile_num_magic_num;
    ...
    // uint32_t max_shards_per_worker = rs_prefill::max_shards_per_worker(schedule);
    ...
    tt::tt_metal::CircularBufferConfig fabric_sender_cb_config =
        tt::tt_metal::CircularBufferConfig(
            tile_num_magic_num * input_page_size, {{fabric_sender_cb_index, cb_data_format}})
            .set_page_size(fabric_sender_cb_index, input_page_size);

    tt::tt_metal::CircularBufferConfig fabric_receiver_cb_config =
        tt::tt_metal::CircularBufferConfig(
            tile_num_magic_num * num_devices * input_page_size, {{fabric_receiver_cb_index, cb_data_format}})
    ...
    tt::tt_metal::CircularBufferConfig accumulator_cb_config =
        tt::tt_metal::CircularBufferConfig(
            buffering_factor * num_pages_per_packet * tile_num_magic_num * input_page_size * num_devices,
    ...
    auto packet_worker_cores = corerange_to_cores(packet_worker_cores_grid, std::nullopt);
    ...
    // auto total_num_read_txns = rs_prefill::get_num_entries_in_schedule(schedule);
    ...
    // }
    // reader_defines["INPUT_CORE_XY"] = rs_prefill::cores_to_string(to_worker_cores(input_cores, ncores_input));
    // reader_defines["OUTPUT_CORE_XY"] = rs_prefill::cores_to_string(to_worker_cores(output_cores, ncores_output));
    // reader_defines["PACKET_WORKER_CORES"] = rs_prefill::cores_to_string(to_worker_cores(packet_worker_cores));
    ...
    tt::tt_metal::KernelHandle unary_reader_kernel_id = tt::tt_metal::CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/ccl/llama_prefill_reduce_scatter/device/kernels/dataflow/"
        "reader_llama_prefill_reduce_scatter.cpp",
        all_cores_grid,
    ...
    // bool skip_write_back = output_cores == packet_worker_cores and num_pages_per_packet ==
    // output_tiles_per_core_width; if (skip_write_back) {
    //     writer_defines["SKIP_WRITE_BACK"] = "1";
    // }
    tt::tt_metal::KernelHandle unary_writer_kernel_id = tt::tt_metal::CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/ccl/llama_prefill_reduce_scatter/device/kernels/dataflow/"
        "writer_llama_prefill_reduce_scatter.cpp",
        all_cores_grid,
        tt_metal::DataMovementConfig{
    ...
    bool forward_fabric_connection = true;
    bool backward_fabric_connection = true;
    ...
    for (auto core : all_cores) {
        ...
        if (sender_core_grid.contains(core)) {
            ...
            auto sender_total_num_pages = tile_num_magic_num;
            ...
            reader_runtime_args[is_linear_input_packet_start_idx] = local_page;
...
void LlamaPrefillReduceScatterDeviceOperation::LlamaPrefillReduceScatterAdd::override_runtime_arguments(
