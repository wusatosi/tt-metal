...
void MAIN {
    ...
    constexpr uint32_t num_pages_per_packet = 1;  // get_compile_time_arg_val(4);

    // noc_semaphore_wait((uint32_t*)receiver_semaphore_address, 1);

    // Derived compile-time constants
    constexpr uint32_t total_pages = 1;  // num_devices * num_pages_per_packet;
    ...
    add_tiles(input_tensor_cb_id, fabric_receiver_cb_id, 0, 0, 0);
    //     }
    // }
