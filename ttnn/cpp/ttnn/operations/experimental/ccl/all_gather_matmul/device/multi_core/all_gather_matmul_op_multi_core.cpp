...
tt::tt_metal::ProgramDescriptor experimental::all_gather_matmul_multi_core_with_workers(
    ...
    // Matmul
    std::optional<tt::tt_metal::ProgramDescriptor> matmul_program;
    ...
    std::visit(
        [&](const auto& config) {
            ...
            if (std::is_same_v<ProgramConfigType, operations::matmul::MatmulMultiCoreReuseMultiCastProgramConfig>) {
                matmul_program = operations::matmul::matmul_multi_core_reuse_mcast_2d_optimized_helper(
                    program,
                    all_gather_output_tensor,
                    weight_tensor,
                    bias,
                ...
                matmul_program = operations::matmul::matmul_multi_core_reuse_mcast_1d_optimized_helper(
                    program,
                    all_gather_output_tensor,
                    weight_tensor,
                    bias,
