...
constexpr uint32_t get_barrier_read_threshold() {
    ...
    return (64 * 1024 / num_readers) / payload_size;  // should be magic number and compile time argument
}
    ...
    for (uint32_t src_core = 0; src_core < num_src_cores; src_core++) {
        ...
        if (src_noc_x >= end_x) {
            ...
            if (src_noc_y >= end_y) {
                src_noc_y = start_y;
                if (noc_index == 0) {
                    dst_address = get_write_ptr(dst_cb_id);
                } else {
                    dst_address = get_write_ptr(dst_cb_id) + dst_stride / 2;
                }
            }
        }
    }
