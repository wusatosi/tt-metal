...
#include "group_norm_program_factory.hpp"

namespace ttnn::operations::experimental {

void SDXLGroupNorm::validate_with_output_tensors(
...
std::vector<ttnn::TensorSpec> SDXLGroupNorm::compute_output_specs(const std::vector<Tensor>& input_tensors) const {
    return {input_tensors.at(0).get_tensor_spec()};
}

std::vector<Tensor> SDXLGroupNorm::create_output_tensors(
    const std::vector<Tensor>& input_tensors, const std::vector<std::optional<Tensor>>& output_tensors) const {
    return {input_tensors.at(0)};
}

tt::tt_metal::operation::ProgramWithCallbacks SDXLGroupNorm::create_program(
    const std::vector<Tensor>& input_tensors, std::vector<Tensor>& output_tensors) const {
    const auto& input_tensor = input_tensors.at(0);
    return detail::plusone_single_core_tmp(input_tensor, sub_core_grids);
}
