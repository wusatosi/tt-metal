...
// No need to transpose in0 if in1 is not repeat_interleaved
        ...
        ckernel::cb_reserve_back(cb_in0_transposed, onetile);
        ckernel:: pack_tile(0, cb_in0_transposed);

        ckernel:: tile_regs_commit();
        ckernel::tile_regs_release();
        ckernel::cb_push_back(cb_in0_transposed, onetile);
        ckernel::cb_pop_front(cb_id_in0, onetile);

        ckernel::cb_wait_front(cb_in0_transposed, onetile);
#endif
#endif

        for (uint32_t in1_block = 0; in1_block < in1_num_blocks; in1_block++) {
            // Transpose in1
            ckernel::cb_wait_front(cb_id_in1, onetile);
            ckernel:: tile_regs_acquire();
            ckernel::tile_regs_wait();
...
// If input b is not repeat_interleaved, then no need to transpose, bcast row
            ...
            ckernel::cb_reserve_back(cb_id_out, onetile);
            ckernel:: pack_tile(0, cb_id_out);

            ckernel:: tile_regs_commit();
            ckernel::tile_regs_release();
            ckernel::cb_push_back(cb_id_out, onetile);
            ckernel::cb_pop_front(cb_id_in1, onetile);
#else
            ...
            ckernel::cb_reserve_back(cb_in1_transposed, onetile);
            ckernel:: pack_tile(0, cb_in1_transposed);

            ckernel:: tile_regs_commit();
            ckernel::tile_regs_release();
            ckernel::cb_push_back(cb_in1_transposed, onetile);
            ckernel::cb_pop_front(cb_id_in1, onetile);
            ...
            for (uint32_t tile_row_id = 0; tile_row_id < num_rows_in_one_tile; tile_row_id++) {
                ...
                ckernel::cb_wait_front(cb_in1_bcast_row, onetile);
                ckernel:: tile_regs_acquire();
                ckernel::tile_regs_wait();
                ...
                ckernel::cb_reserve_back(cb_out_transposed, onetile);
                ckernel:: pack_tile(0, cb_out_transposed);

                ckernel:: tile_regs_commit();
                ckernel::tile_regs_release();
                ckernel::cb_push_back(cb_out_transposed, onetile);
#ifndef REPEAT_IN0
                ckernel::cb_pop_front(cb_in0_transposed, onetile);
#endif
                ckernel::cb_pop_front(cb_in1_bcast_row, onetile);

                // Transpose output back
                ckernel::cb_wait_front(cb_out_transposed, onetile);
                ckernel:: tile_regs_acquire();
                ckernel::tile_regs_wait();
                ...
                ckernel::tile_regs_release();
                ckernel::cb_push_back(cb_id_out, onetile);
                ckernel::cb_pop_front(cb_out_transposed, onetile);
