...
FORCE_INLINE void pack_block_rows_into_tiles(uint32_t cb_in, uint32_t cb_out, uint32_t num_tiles) {
    ...
    ckernel::cb_wait_front(cb_in, num_tiles);
    ckernel::cb_reserve_back(cb_out, NUM_TILES_IN_TILIZED_CHUNK);

    untilize_block(cb_in, NUM_TILES_IN_TILIZED_CHUNK, cb_out);

    ckernel::cb_push_back(cb_out, NUM_TILES_IN_TILIZED_CHUNK);
    ckernel::cb_pop_front(cb_in, num_tiles);
...
FORCE_INLINE void pack_block_tiles_into_rows(uint32_t cb_in, uint32_t cb_out, uint32_t num_tiles) {
    ...
    ckernel::cb_wait_front(cb_in, NUM_TILES_IN_TILIZED_CHUNK);
    ckernel::cb_reserve_back(cb_out, num_tiles);

    tilize_block(cb_in, NUM_TILES_IN_TILIZED_CHUNK, cb_out);

    ckernel::cb_push_back(cb_out, num_tiles);
    ckernel::cb_pop_front(cb_in, NUM_TILES_IN_TILIZED_CHUNK);
...
FORCE_INLINE void mul(uint32_t cb_a, uint32_t cb_b, uint32_t cb_out) {
    ...
    ckernel::cb_wait_front(cb_a, 1);
    ckernel::cb_wait_front(cb_b, 1);
    ...
    ckernel::cb_pop_front(cb_a, 1);
    ckernel::cb_pop_front(cb_b, 1);
}
...
FORCE_INLINE void copy(uint32_t cb_in, uint32_t cb_out, uint32_t num_input_units = 1) {
    ...
    ckernel::cb_wait_front(cb_in, num_input_units);
    ckernel::cb_reserve_back(cb_out, 1);

    ckernel:: tile_regs_acquire();
    ckernel:: copy_tile(cb_in, 0, 0);
    ckernel:: tile_regs_commit();
    ckernel::tile_regs_wait();
    ckernel:: pack_tile(0, cb_out);
    ckernel::tile_regs_release();
...
void MAIN {
    ...
    for (uint32_t tilized_chunk_idx = 0; tilized_chunk_idx < num_chunks_per_row; tilized_chunk_idx++) {
        ...
        ckernel::cb_pop_front(cb_h_in, remaining_tiles_in_chunk);
    }
    ...
    for (uint32_t row_idx = 0; row_idx < total_tiles_per_col; row_idx++) {
        for (uint32_t tilized_chunk_idx = 0; tilized_chunk_idx < num_chunks_per_row; tilized_chunk_idx++) {
            ...
            ckernel::cb_pop_front(cb_h_acc, 1);
