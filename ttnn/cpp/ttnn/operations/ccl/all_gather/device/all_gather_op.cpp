...
void AllGather::validate(const operation_attributes_t& attr, const tensor_args_t& input_tensor) {
    ...
    TT_FATAL(attr.num_links > 0, "Error, num_links should be more than 0 but has {}", attr.num_links);
    TT_FATAL(
        attr.num_links <= input_tensor.device()->compute_with_storage_grid_size().y,
        "Worker cores used by links are parallelizaed over rows");
    TT_FATAL(
        attr.receiver_device_id.has_value() || attr.sender_device_id.has_value(),
...
ttnn::TensorSpec AllGather::compute_output_specs(
    const AllGather::operation_attributes_t& attr, const AllGather::tensor_args_t& input_tensor) {
    auto output_shape = input_tensor.get_logical_shape();
    output_shape[attr.dim] *= attr.ring_size;

    TensorSpec spec(
        output_shape,
        tt::tt_metal::TensorLayout(
            input_tensor.get_dtype(), input_tensor.get_tensor_spec().page_config(), attr.output_mem_config));
    if (attr.output_mem_config.is_sharded()) {
        TensorSpec(
            output_shape,
            tt::tt_metal::TensorLayout(
                input_tensor.get_dtype(), input_tensor.get_tensor_spec().page_config(), attr.output_mem_config));
...
Tensor AllGather::create_output_tensors(
    const AllGather::operation_attributes_t& attr, const AllGather::tensor_args_t& input_tensor) {
    return create_device_tensor(compute_output_specs(attr, input_tensor), input_tensor.device());
}

tt::tt_metal::ProgramDescriptor AllGather::create_program(
    const AllGather::operation_attributes_t& attr,
    const AllGather::tensor_args_t& input_tensor,
    ...
    return all_gather_multi_core_with_workers(
        ...
        attr.ring_size,
        attr.ring_index,
        attr.receiver_device_id,
        attr.sender_device_id,
        attr.topology,
        attr.user_defined_num_workers,
        attr.user_defined_num_buffers_per_channel);
}

std::tuple<AllGather::operation_attributes_t, AllGather::tensor_args_t> AllGather::invoke(
    const AllGather::operation_attributes_t& attr, const Tensor& input_tensor) {
    return {attr, input_tensor};
}
...
Tensor all_gather(
    ...
    auto all_gather_struct = ttnn::ccl::all_gather_detail::create_all_gather_struct(
        input_tensor,
        gather_dim,
        num_links,
        memory_config,
        ...
        devices,
        ccl_topology);
    auto output_tensor = ttnn::prim::all_gather(all_gather_struct, input_tensor);
...
Tensor all_gather(
    ...
    auto sender_device_id =
        is_last_chip_in_counter_clockwise_direction ? std::nullopt : get_chip_id(device_index + num_devices - 1);
