...
EdmLineFabricOpInterface::EdmLineFabricOpInterface(
    ...
    for (size_t i = 0; i < device_pairs.size(); i++) {
        ...
        uint32_t size = local_device->get_ethernet_sockets(remote_device->id()).size();
        ...
        for (auto core : local_device->get_ethernet_sockets(remote_device->id())) {
        ...
        edm_builders.clear();
        for (const auto& core : reordered_ethernet_cores) {
