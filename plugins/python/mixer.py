...
ICAPS = Gst.Caps(Gst.Structure("video/x-raw", format="RGBA", width=320, height=240, framerate=Gst.Fraction(30, 1)))

OCAPS = Gst.Caps(Gst.Structure("video/x-raw", format="RGBA", width=320, height=240, framerate=Gst.Fraction(30, 1)))
...
class Videomixer(GstBase.Aggregator):
    __gstmetadata__ = ("Videomixer", "Video/Mixer", "Python video mixer", "Mathieu Duponchelle")

    __gsttemplates__ = (
        Gst.PadTemplate.new_with_gtype(
            "sink_%u", Gst.PadDirection.SINK, Gst.PadPresence.REQUEST, ICAPS, GstBase.AggregatorPad.__gtype__
        ),
        Gst.PadTemplate.new_with_gtype(
            "src", Gst.PadDirection.SRC, Gst.PadPresence.ALWAYS, OCAPS, GstBase.AggregatorPad.__gtype__
        ),
    )
