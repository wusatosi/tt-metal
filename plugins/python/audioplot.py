...
AUDIO_FORMATS = [f.strip() for f in GstAudio.AUDIO_FORMATS_ALL.strip("{ }").split(",")]
...
class AudioPlotFilter(GstBase.BaseTransform):
    __gstmetadata__ = ("AudioPlotFilter", "Filter", "Plot audio waveforms", "Mathieu Duponchelle")

    __gsttemplates__ = (
        Gst.PadTemplate.new("src", Gst.PadDirection.SRC, Gst.PadPresence.ALWAYS, OCAPS),
        Gst.PadTemplate.new("sink", Gst.PadDirection.SINK, Gst.PadPresence.ALWAYS, ICAPS),
    )
    __gproperties__ = {
        "window-duration": (
            float,
    ...
    def do_transform(self, inbuf, outbuf):
        if not self.h:
            (self.h,) = self.ax.plot(
                np.array(self.ringbuffer),
                lw=0.5,
                color="k",
                path_effects=[pe.Stroke(linewidth=1.0, foreground="g"), pe.Normal()],
            )
        else:
    ...
    def do_generate_output(self):
        ...
        res, data = self.converter.convert(GstAudio.AudioConverterFlags.NONE, info.data)
    ...
    def do_fixate_caps(self, direction, caps, othercaps):
        if direction == Gst.PadDirection.SRC:
            ...
            so.fixate_field_nearest_fraction("framerate", DEFAULT_FRAMERATE_NUM, DEFAULT_FRAMERATE_DENOM)
    ...
    def do_set_caps(self, icaps, ocaps):
        ...
        self.convert_info.set_format(GstAudio.AudioFormat.S32, in_info.rate, in_info.channels, in_info.position)
        self.converter = GstAudio.AudioConverter.new(
            GstAudio.AudioConverterFlags.NONE, in_info, self.convert_info, None
        )

        self.fig = plt.figure()
        dpi = self.fig.get_dpi()
        self.fig.patch.set_alpha(0.3)
        self.fig.set_size_inches(out_info.width / float(dpi), out_info.height / float(dpi))
        ...
        self.frame_duration = Gst.util_uint64_scale_int(Gst.SECOND, out_info.fps_d, out_info.fps_n)
        self.next_time = self.frame_duration

        self.agg.draw()
        self.background = self.fig.canvas.copy_from_bbox(self.ax.bbox)

        self.samplesperbuffer = Gst.util_uint64_scale_int(
            in_info.rate * in_info.channels, out_info.fps_d, out_info.fps_n
        )
        self.next_offset = self.samplesperbuffer
        self.cur_offset = 0
        self.buf_offset = 0
